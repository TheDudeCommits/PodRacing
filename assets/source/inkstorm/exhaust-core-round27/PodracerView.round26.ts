...
const exhaustFragment = /* glsl */ `
  ...
  void main() {
    ...
    // A chase camera can look almost directly down the nozzle axis. The
    // closed base needs its own luminous core so it cannot read as an empty
    // cyan ring with terrain showing through the engine mouth.
    float nozzleCore = (1.0 - step(0.003, along))
      * (1.0 - smoothstep(0.62, 0.82, length(vLocal.xz)));
    color = mix(color, mix(uHot, vec3(0.98, 0.99, 1.0), 0.68), nozzleCore);
    alpha = max(alpha, nozzleCore * 0.98);
