...
## Current V26 interface

These are actual local game captures from `index-BwYDA7ip.js` (SHA-256 `700dc1c6404afe7cce90ec5977eeef72e7a98291d173eceac2c33f6f8db11270`). They show implemented screens, not generated targets. [Native receipt](../../output/gauntlet/round35-broadcast-native-v26/receipt.json) passes 17 captures and control/layout checks with zero errors and closed owned browsers/servers. The [fresh V26 critic](../../output/critics/blind-hud-v26/cli-critic-attempt1/review.md) scores HUD **7/7.5**, Build **7/6.5**, map **7.5/8** and garage **8/7.5** (style/readability), below 8 in both. Build's default last description and synergy area remain crowded/clipped despite individual scroll reachability; V27 addresses this issue. The actual totals audit passes all 30 rows. Functional passes do not establish reference parity.

![Actual V26 HUD during driving](../../output/gauntlet/round35-broadcast-native-v26/06-desktop-live-race.png)

![Actual V26 Build screen](../../output/gauntlet/round35-broadcast-native-v26/03-desktop-build.png)

![Actual V26 destination map](../../output/gauntlet/round35-broadcast-native-v26/02-desktop-map.png)

![Actual V26 garage](../../output/gauntlet/round35-broadcast-native-v26/01-desktop-garage.png)

The latest independently reviewed crash is V26, **5/10 FAIL8**. Shorter pressure, curling fire and varied smoke are implemented, but the soft blast and limited contact/motion still fall short. [Actual crash review](../../output/playwright/round35-combat-v26/blind-critic/cli-critic-attempt1/review.md). V27 authored flame artwork and support-pivot settling are private work in progress. No isolated FPS measurement exists for V26; older measured performance is recorded separately in the [active handover](ROUND35_UI_COMBAT_HANDOVER.md).
