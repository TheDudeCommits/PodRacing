...
const receipt:any={outcome:'FAIL',errors:[],captures:[],commands:[],port,maxRaceWallSeconds:180,
...
async function installDriver(){
 const state=await snapshot(),course=createProceduralPodraceCourse({heightAt:sampleTerrainHeight},Number((state.game.course as any).seed));
 ...
 await page!.evaluate(({samples,totalLength,tune})=>{
  ...
  const frame=(wallMs:number)=>{
   ...
   for(const event of state.galactic?.recentEvents??[]){const key=JSON.stringify(event);if(!seen.has(key)){seen.add(key);append(observation.events,{...event,observedWallMs:wallMs},12000);}}
 ...
 },{samples,totalLength:course.totalLength,tune});
}
...
function analyze(observation:any){
 ...
 const wreck=events.find((e:any)=>e.type==='wreck'&&e.racerId!=='player'&&e.takedownBy==='player');
 ...
 return{outcome:issues.length?'FAIL':'PASS',issues,wreck,takedown,actualVictim,cue,estimatedCueStartMs:start,before,slow,after,cameraBaseline,
...
try{
 ...
 await installDriver();await page.locator('[data-action="start-race"]:visible').click();receipt.commands.push('Select actual Open Expedition and Start once. Course-following virtual pad holds Heat Lance(button5/E), deploys mines(button3/F) for rivals within45m behind, and pulses shield(button2/Q) for120ms on normal cooldown. No boost, pickup waits or review API actions.');
 const started=Date.now();let foundAt:number|null=null;
 while(Date.now()-started<180000){
  await delay(60);const status=await page.evaluate(()=>{const qa=(window as any).__outgoingQA;return{wreck:qa.observation.events.find((e:any)=>e.type==='wreck'&&e.racerId!=='player'&&e.takedownBy==='player'),phase:document.querySelector<HTMLElement>('.pod-hud')?.dataset.phase};});
  if(status.wreck&&foundAt===null){foundAt=Date.now();receipt.firstOutgoingWreck=status.wreck;receipt.captures.push({path:`${output}/outgoing-takedown.png`,snapshot:await snapshot()});await page.screenshot({path:`${output}/outgoing-takedown.png`});console.log('OUTGOING_WRECK',JSON.stringify(status.wreck));}
  if(foundAt!==null&&Date.now()-foundAt>=2200)break;
  if(status.phase==='finished'){receipt.stopReason='Race finished';break;}
 }
 receipt.stopReason??=foundAt===null?'180-second limit without a player-attributed enemy wreck':'First player-attributed enemy wreck plus return observation';
 await page.evaluate(()=>(window as any).__outgoingQA.stop());receipt.after=await snapshot();receipt.observation=await page.evaluate(()=>(window as any).__outgoingQA.observation);receipt.analysis=analyze(receipt.observation);
...
}catch(error){receipt.error=String(error);if(page){try{await page.evaluate(()=>(window as any).__outgoingQA?.stop());receipt.failure=await snapshot();receipt.observation=await page.evaluate(()=>(window as any).__outgoingQA?.observation??null);await page.screenshot({path:`${output}/failure.png`});}catch{}}}
