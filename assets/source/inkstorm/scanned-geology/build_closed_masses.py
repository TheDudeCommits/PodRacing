...
def build(asset,index):
    ...
    for v in coords:
        ...
        angle=u*1.40
        vertices.append((math.sin(angle)*(40-depth*15),-math.cos(angle)*(50-depth*20),h*120))
    n=len(vertices)
    for v in list(vertices):
        h=v[2]/120
        vertices.append((v[0]*(.90+.08*h)+2*math.sin(h*3.2),-v[1]*.88+1.8*math.sin(h*9+index),(v[2]-60)*.97+60))
    for face in bm.faces:
        ids=[v.index for v in face.verts]
        color=[tuple(linear(c) for c in tuple(loop[color_layer])[:3]) for loop in face.loops]
        faces.append(ids);smooth.append(True);colors.extend([(*c,1) for c in color])
        faces.append([i+n for i in reversed(ids)]);smooth.append(True);colors.extend([(*(v*.90 for v in c),1) for c in reversed(color)])
    ...
    for vi in boundary_vertices:
        ...
        for k in [1,2,3,4]:
            t=k/5;v=front.lerp(back,t)
            v.x*=.99+.025*math.sin(v.z*.079+index+k)
            v.x+=(1 if v.x>0 else -1)*(1.8+2.2*math.sin(v.z*.17+v.y*.23+index))*math.sin(math.pi*t)
            v.y+=2.8*math.sin(v.z*.15+v.x*.11)*math.sin(math.pi*t)
            crown=max(0,min(1,(v.z/120-.72)/.25))
            v.z+=(2.2*math.sin(v.x*.095+index)+crown*12*(1-min(1,abs(v.x)/45)))*math.sin(math.pi*t)
    ...
    for edge in boundary:
        ...
        ra=[a]+[side_indices[(a,k)] for k in [1,2,3,4]]+[a+n];rb=[b]+[side_indices[(b,k)] for k in [1,2,3,4]]+[b+n]
        for k in range(5):
            ids=[rb[k],ra[k],ra[k+1],rb[k+1]]
            faces.append(ids);smooth.append(False)
            # Broad warm broken planes on authored closure, not scan edge colors
            # stretched into continuous stripes across the whole cliff depth.
            for vi,edge_color in zip(ids,[cb,ca,ca,cb]):
                p=vertices[vi]
                value=.5+.5*math.sin(p[0]*.137+p[1]*.19+math.sin(p[2]*.09)*3+index)
                base=(.45+value*.23,.145+value*.14,.074+value*.055)
                c=tuple(base[j]*.80+edge_color[j]*.20 for j in range(3))
                colors.append((*c,1))
    bm.free()
