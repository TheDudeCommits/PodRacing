 ...
 * `sample`. In opt-in cadenceAware mode, its second argument must be the actual
 * RAF timestamp: a separate rolling window measures callback intervals, which
 * reveal missed presentation opportunities but are not GPU timing. Report
 * Three.js renderer information once after each complete frame graph with
 * `observeRendererInfo`. Degradation requires a sustained overload;
 * recovery is intentionally much slower to prevent visible quality pumping.
 */
export class PerformanceGovernor {
  readonly profiler: RollingFrameProfiler;
  readonly cadenceProfiler: RollingFrameProfiler | null;
  ...
  private readonly cadenceDegradeThresholdMs: number;
  private readonly cadenceRecoverThresholdMs: number;

  private adaptiveEnabledValue = true;
  private nextTransitionMs = 0;
  private lastNowMs = 0;
  private lastCadenceNowMs: number | null = null;
  ...
  constructor(options: PerformanceGovernorOptions = {}) {
    ...
    this.cadenceDegradeThresholdMs = clamp(
      finiteOr(options.cadenceDegradeThresholdMs, 25),
      PERFORMANCE_TARGET_FRAME_MS,
      100,
    );
    this.cadenceRecoverThresholdMs = clamp(
      finiteOr(options.cadenceRecoverThresholdMs, 22.5),
      1,
      this.cadenceDegradeThresholdMs - 0.5,
    );
    ...
    this.cadenceProfiler = options.cadenceAware ? new RollingFrameProfiler({
      windowSize: options.windowSize,
      budgetMs: this.cadenceDegradeThresholdMs,
      vsyncFrameMs: this.cadenceDegradeThresholdMs,
      // Long active intervals still signal overload. Values above this bounded
      // histogram range saturate at 1 s rather than being treated as pauses.
    ...
    this.telemetryValue = {
      ...
      cadence: this.cadenceProfiler?.telemetry ?? null,
      ...
      cadenceAware: this.cadenceProfiler !== null,
      cadenceDegradeThresholdMs: this.cadenceDegradeThresholdMs,
      cadenceRecoverThresholdMs: this.cadenceRecoverThresholdMs,
      cadenceWarmedUp: false,
      overBudgetSamples: 0,
  ...
  sample(frameMs: number, nowMs: number): Readonly<PerformanceDecision> {
    ...
    const cadenceAccepted = this.sampleCadence(nowMs);
    const cadence = this.cadenceProfiler?.telemetry;
    ...
    const frameWarmed = frame.windowSamples >= Math.min(
      this.profiler.windowSize,
      this.warmupSamples,
    );
    telemetry.cadenceWarmedUp = this.cadenceProfiler !== null
      && (cadence?.windowSamples ?? 0) >= Math.min(
        this.cadenceProfiler.windowSize,
        this.warmupSamples,
      );
    telemetry.warmedUp = frameWarmed && (
      this.cadenceProfiler === null || (cadenceAccepted && telemetry.cadenceWarmedUp)
    );
    ...
    const rendererOverBudget = this.cadenceProfiler === null && rendererWarmed
      && renderer.pressure > 1
    ...
    const rendererHasHeadroom = this.cadenceProfiler !== null
      || !rendererWarmed || renderer.pressure <= 0.82;
    const cadenceOverBudget = cadence !== undefined && (
      cadence.emaFrameMs > this.cadenceDegradeThresholdMs
      || (
        cadence.p95FrameMs > this.cadenceDegradeThresholdMs + 2
        && cadence.overBudgetRatio >= 0.35
      )
    );
    const cadenceHasHeadroom = cadence === undefined || (
      cadence.emaFrameMs <= this.cadenceRecoverThresholdMs
      && cadence.p95FrameMs <= this.cadenceDegradeThresholdMs
      && cadence.overBudgetRatio <= 0.05
    );

    if (frameOverBudget || rendererOverBudget || cadenceOverBudget) {
      telemetry.overBudgetSamples += 1;
      telemetry.underBudgetSamples = 0;
    } else if (frameUnderBudget && rendererHasHeadroom && cadenceHasHeadroom) {
    ...
    if (
      ...
      const reason: GovernorDecisionReason = rendererOverBudget
        ? 'renderer-budget'
        : frameOverBudget ? 'frame-budget' : 'cadence-budget';
  ...
  private sampleCadence(nowMs: number): boolean {
    if (this.cadenceProfiler === null || !Number.isFinite(nowMs)) return false;
    if (this.lastCadenceNowMs === null) {
      this.lastCadenceNowMs = nowMs;
      return false;
    }
    const interval = nowMs - this.lastCadenceNowMs;
    if (interval <= 0) return false;
    this.lastCadenceNowMs = nowMs;
    this.cadenceProfiler.sample(interval);
    return true;
  }

  private resetCadence(): void {
    this.lastCadenceNowMs = null;
    this.cadenceProfiler?.reset();
    this.telemetryValue.cadenceWarmedUp = false;
    if (this.cadenceProfiler !== null) this.telemetryValue.warmedUp = false;
  }
