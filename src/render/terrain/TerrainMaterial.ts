...
TerrainRegionPalette terrainRegionPalette(float index) {
  ...
  if (index < 0.5) {
    ...
    palette.shadow = vec3(54.0, 40.0, 77.0) / 255.0;
    palette.darkSand = vec3(135.0, 76.0, 79.0) / 255.0;
    palette.midSand = vec3(228.0, 151.0, 99.0) / 255.0;
    palette.sunSand = vec3(255.0, 192.0, 128.0) / 255.0;
    ...
    palette.haze = vec3(185.0, 146.0, 170.0) / 255.0;
  } else if (index < 3.5) {
  ...
  // Region literals are authored in sRGB; the first palette is already
  // linearized by Three.Color uniforms. Convert the others before lighting.
  if (index >= 0.5) {
    palette.shadow = pow(palette.shadow, vec3(2.2));
    palette.darkSand = pow(palette.darkSand, vec3(2.2));
    palette.midSand = pow(palette.midSand, vec3(2.2));
    palette.sunSand = pow(palette.sunSand, vec3(2.2));
    palette.crest = pow(palette.crest, vec3(2.2));
    palette.mineral = pow(palette.mineral, vec3(2.2));
    palette.sparkle = pow(palette.sparkle, vec3(2.2));
    palette.haze = pow(palette.haze, vec3(2.2));
  }
  return palette;
}
...
void main() {
  ...
  // Directional sand strokes give close ground a stable scale at racing speed.
  float stroke = sin(vTerrainWorldPosition.x * .23 + vTerrainWorldPosition.z * .69 + sin(vTerrainWorldPosition.x*.055)*2.1);
  float fineStroke = sin(vTerrainWorldPosition.x * 1.42 + vTerrainWorldPosition.z * 2.96);
  float detailFade = 1.-smoothstep(90.,380.,distanceToCamera);
  color *= 1.0 - (smoothstep(.65,.92,stroke)*.03 + smoothstep(.82,.96,fineStroke)*.012)*detailFade;
  // Authored crust carries the foreground scale; a second rotated, broader
  // sample breaks tile repetition without changing the shared terrain surface.
  if (uSandPaintReady > .5) {
    vec2 groundUv = vTerrainWorldPosition.xz;
    vec3 crust = texture(uSandPaint, groundUv * .026).rgb;
    vec3 broad = texture(uSandPaint, mat2(.8,-.6,.6,.8) * groundUv * .007 + .37).rgb;
    float grain = clamp(dot(crust,vec3(.30,.59,.11)) / .19, .30, 1.65);
    float patches = clamp(dot(broad,vec3(.30,.59,.11)) / .19, .50, 1.4);
    float paintFade = 1. - smoothstep(220.,950.,distanceToCamera);
    color *= mix(1., mix(.62,1.23,grain*.62) * mix(.9,1.1,patches*.6), paintFade*.62);
  }
  ...
  // Scenery casts coherent shade across the analytic ground and racing surface.
  float sunVisibility = min(inkstormSunVisibility(vTerrainWorldPosition + vec3(0.,.5,0.)), inkstormRacerSunVisibility(vTerrainWorldPosition,normal));
  color = mix(color * vec3(.29,.25,.48), color, mix(.15,1.,sunVisibility));
  ...
  // pale sand belt. Flat shelves and untouched procedural dunes stay sandy.
  float cliff = smoothstep(.24,.58,slope)
    * max(smoothstep(8.,42.,abs(vTerrainAuthoredOffset)),
          smoothstep(.15,.45,vTerrainAuthoredGrade));
  if (cliff>.001) {
    vec3 rock=inkstormRockAlbedo(vTerrainWorldPosition,normal,uRockPaint,uRockBeds,uRockPaintReady);
    rock=inkstormRockLight(rock,inkstormRockNormal(vTerrainWorldPosition,normal,rock),sunDirection,sunVisibility);
    // Rock haze and shade are applied exactly once, identically to mesh rocks.
    // Sand keeps its existing region palette and atmosphere.
