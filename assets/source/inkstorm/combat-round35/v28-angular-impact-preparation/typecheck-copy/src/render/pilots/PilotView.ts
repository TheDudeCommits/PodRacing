...
export class PilotView extends Group {
  ...
  private createMaterials(): PilotMaterials {
    const materials: PilotMaterials = {
      ...
      helmet: new CelMaterial({
        name: `${this.variant.id}:pilot-painted-helmet`,
        palette: this.variant.suitPalette,
        wear: .2,
        specularPower: 42,
        specularCutoff: .42,
  ...
  private buildHead(head: Group): void {
    ...
    const headScale = this.variant.proportions.headScale * (this.detail === 'distant' ? 1.16 : 1.1);
    if (this.variant.helmetStyle !== 'goggles') this.addPart(
      head,
      // Keep exposed faces as shallow forward masks. The hero's enclosed
      // racing visor covers this area, so it does not submit a hidden face.
    ...
    const helmet = this.addPart(
      ...
      new SphereGeometry(
        0.31,
        this.detail === 'hero' ? 14 : 8,
        this.detail === 'hero' ? 8 : 5,
        0,
        Math.PI * 2,
        0,
        Math.PI * 0.78,
      ...
      'helmet-dome',
      [0, 0.08, -0.018],
      [headScale, .94 * headScale, 1.15 * headScale],
    );
    helmet.rotation.x = -0.08;
    if (this.detail === 'hero') {
      // One slim longitudinal paint stripe crosses crown and rear shell. It
      // breaks the circular rear highlight without turning the whole head into
      // a warm luminous disc. Both arcs share one geometry and one submission.
      const arcs = [Math.PI * .5, Math.PI * 1.5].map(phi => new SphereGeometry(
        .315, 3, 8, phi - .105, .21, .045, Math.PI * .73,
      ));
      const stripe = mergeGeometries(arcs, false)!;
      for (const arc of arcs) arc.dispose();
      this.addPart(head, stripe, this.materials.accent, 'helmet-painted-center-stripe',
        [0, .08, -.018], [headScale, .94 * headScale, 1.15 * headScale], [-.08, 0, 0], false);
    }
    ...
    switch (this.variant.helmetStyle) {
      case 'goggles':
        this.addPart(
          head,
          new SphereGeometry(.321, this.detail === 'hero' ? 12 : 8, 4,
            .20, Math.PI - .40, Math.PI * .35, Math.PI * .25),
          this.materials.visor,
          'split-goggles',
          [0, .08, -.012],
          [headScale, .94 * headScale, 1.15 * headScale],
