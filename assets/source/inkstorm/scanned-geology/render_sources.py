...
try:
    ...
    camera_data=bpy.data.cameras.new('Inkstorm Scanned Source Preview Camera')
    camera=bpy.data.objects.new('Inkstorm Scanned Source Preview Camera',camera_data)
    scene.collection.objects.link(camera)
    created.append(camera)
    camera.location=(5,-8,6)
    camera.rotation_euler=(Vector((0,0,1))-camera.location).to_track_quat('-Z','Y').to_euler()
    camera_data.type='ORTHO'
    camera_data.ortho_scale=7
    camera_data.clip_end=1000
    scene.camera=camera
    for name,rotation,energy,color in [('Inkstorm Scanned Source Preview Sun',(.4,-.55,-.6),3.2,(1,.84,.65)),('Inkstorm Scanned Source Preview Fill',(.9,.3,2.1),1.25,(.64,.75,1))]:
    ...
    world=bpy.data.worlds.new('Inkstorm Scanned Source Preview World')
    ...
    targets=[('PH-rock_face_02-unmodified-source','source-02'),('PH-rock_face_01-unmodified-source','source-01')]
    for name,label in targets:
        ...
        camera.location=center+Vector((dimension*1.15,-dimension*1.7,dimension*.82))
        camera.rotation_euler=(center-camera.location).to_track_quat('-Z','Y').to_euler()
        camera.data.ortho_scale=dimension*1.55
