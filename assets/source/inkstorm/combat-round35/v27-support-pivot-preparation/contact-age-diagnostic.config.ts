...
const preparation = `${project}/assets/source/inkstorm/combat-round35/v27-support-pivot-preparation`;
...
export default defineConfig({
  plugins: [{
    name: 'v27-private-support-sources', enforce: 'pre',
    transform(code, id) {
      if (!id.endsWith('/tests/combat/TeemtoGroundFootprint.test.ts')) return null;
      return code.replace('expect(f.halfLength).toBeGreaterThan(2)', 'expect(f.halfLength, `${name}/frame=${frame}/age=${2.15-e.galactic!.wreck.timer}/halfWidth=${f.halfWidth}`).toBeGreaterThan(2)');
    },
  ...
  test: {
    ...
    include: [`${project}/tests/combat/TeemtoGroundFootprint.test.ts`, `${project}/tests/combat/TeemtoGroundEdges.test.ts`],
  },
});
