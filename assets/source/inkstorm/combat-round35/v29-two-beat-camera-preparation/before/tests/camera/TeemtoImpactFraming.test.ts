...
function fittedConstraints(rig: CinematicCamera, pose: WreckVisualPose) {
  ...
  for (const p of pose.impactFraming!.bounds) {
    ...
    horizontalMax = Math.max(horizontalMax, x + .74 * horizontal * z);
    horizontalMin = Math.min(horizontalMin, x - .74 * horizontal * z);
  ...
  const horizontalDistance = Math.max(2, (horizontalMax - horizontalMin) / (1.48 * horizontal));
...
function assertActualCluster(art: ReturnType<typeof fixture>, pose: WreckVisualPose): void {
  ...
  for (const name of engineNames) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      expect(Math.min(...cluster.bounds.map(point => point.distanceTo(corner))), `${name}: independently transformed source box corner`).toBeLessThan(1e-7);
      expect(Math.min(...pose.bounds.map(point => point.distanceTo(corner))), `${name}: full safety corner retained`).toBeLessThan(1e-7);
      expected.push(corner.clone());
    }
  }
  for (const witness of [pose.rupture?.position, pose.groundContact?.position]) if (witness) {
    expect(Math.min(...cluster.bounds.map(point => point.distanceTo(witness))), 'authored witness retained').toBeLessThan(1e-7);
    expected.push(witness);
  }
  expect(cluster.bounds.length).toBeGreaterThanOrEqual(expected.length);
  expect(cluster.bounds.length).toBeLessThanOrEqual(32);
}
    ...
    try {
      for (let frame = 1; frame <= 1067; frame++) {
        ...
        for (const [index, rig] of rigs.entries()) {
          ...
          safeCluster(rig, cluster?.bounds ?? pose.bounds, label, cluster ? .74 : .78);
          ...
          if (checkpoints.has(frame)) {
            ...
            try { for (const mode of ['side', 'chase'] as const) {
              ...
              safeCluster(endpoint, cluster.bounds, `${label}/${mode}`);
