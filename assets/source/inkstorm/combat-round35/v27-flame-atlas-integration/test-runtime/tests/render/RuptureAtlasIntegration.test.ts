...
import { GalacticEffectsView } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v27-flame-atlas-integration/test-runtime/src/render/galactic/GalacticEffectsView.ts';
import { type RuptureAtlasOptions } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v27-flame-atlas-integration/test-runtime/src/render/galactic/RuptureFlameAtlas.ts';
...
describe('rooted artwork on the existing authored plates', () => {
  ...
  it('maps all four measured roots to the same plate center with shared pixels/world despite unequal rectangles and pulse/stretch', () => {
    ...
    try {
      ...
      for (const age of [0, .08, .25, .6, 1.6]) {
        ...
        for (let i = 0; i < view.explosionPlates.count; i++) {
          ...
          expect(metresPerPixelX).toBeCloseTo(1 / 120, 7); expect(metresPerPixelY).toBeCloseTo(1 / 120, 7);
