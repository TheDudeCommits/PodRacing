...
async function checkSebulba() {
  ...
  try {
    ...
    assert(receipt.requests.every(request => /\/(teemto-(hero-v4c|rival)|sebulba-hero)\.glb$/.test(request.url)), 'An unused catalogue or Sebulba rival variant was loaded');
...
try {
  ...
  await failedPage.route('**/assets/inkstorm/vehicles/teemto-hero-v4c.glb', failAsset);
  ...
  await failedPage.unroute('**/assets/inkstorm/vehicles/teemto-hero-v4c.glb', failAsset);
  stage = 'retry';
  ...
  assert(receipt.requests.every((request) => /\/teemto-(hero-v4c|rival)\.glb$/.test(request.url)), 'Unselected catalogue models were loaded');
