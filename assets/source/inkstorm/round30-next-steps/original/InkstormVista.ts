...
export interface VistaLandform extends VistaFootprint {
  ...
  /** Narrow middle-distance spires, separately budgeted from earlier forms. */
  compositionLayer?: 'middle';
}
...
// Three small source spires interrupt the newly lowered shoulders. High/LOD
// bounds match the buttress grounding contract, but use separate existing
// batches and a bounded 10,248 / 5,409 triangle budget for this whole group.
const LAUNCH_COMPOSITION_FORMS = [
...
export function getInkstormVistaPlan(course: PodraceCourse, heightAt: HeightAt): InkstormVistaPlan {
  ...
  if (basinAnchor) {
    ...
    for (const form of LAUNCH_COMPOSITION_FORMS) {
      ...
      const z = basinAnchor.z + basinAnchor.tangentZ * forward - basinAnchor.rightZ * right;
      const radius = Math.hypot(40 * size, 50 * size);
      const footprint = { id: `launch-composition-${form.id}`, x, z, radius };
      if (!isClear(footprint, segments, LAUNCH_SHOULDER_CLEARANCE)) continue;
      if (LAUNCH_INDUSTRIAL_BENCHES.some(bench => Math.hypot(
        Math.max(0, Math.abs(forward - bench.forward) - bench.halfForward - 8),
        Math.max(0, Math.abs(right - bench.right) - bench.halfRight - 8),
      ...
      landforms.push({ ...placement, forward, family: 'fractured-spire',
        settlement: false, compositionLayer: 'middle' });
    }
