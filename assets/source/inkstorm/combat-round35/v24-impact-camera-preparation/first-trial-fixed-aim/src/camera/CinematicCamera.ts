...
export class CinematicCamera {
  ...
  private combatDirection(subject: CameraSubject, forward: Vector3, right: Vector3): void {
    ...
    if (authored && impactFrame) {
      ...
      // one modest 20-degree offset away from that direction, on the same
      // tear side. Lock it for this identity: no orbit as the pieces settle.
  ...
  private fitCombatBounds(subject: CameraSubject, direction: Vector3, minimumDistance = 0): void {
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      for (const point of points) {
        ...
        // Authored cluster: x in +/-.74; legacy full assembly: +/-.78.
        // Vertical caption/matte protection stays y in [-.58,.70].
        // The .06 upward screen bias reserves caption space below the craft.
        distance = Math.max(distance, z + Math.abs(x) / ((impactFrame ? .74 : .78) * horizontal),
