...
function analyze(observation, test) {
  ...
  if (test.id === 'solo-chase') {
    ...
    require(breakup.samples >= 3 && breakup.allTeemto && breakup.allAvailable && breakup.allActive,
      'Teemto engine breakup was not available and active across at least three non-birth wreck-shot samples.');
  }
...
async function runCase(test) {
  ...
  try {
    ...
    result.recoveryCompletion = { phase: result.after.game.wreckPhase, camera: result.after.camera,
      breakupActive: result.after.game.vehiclePresentation.racers[0].breakupActive, travelledMetres: travelled, speed: result.after.game.speed };
