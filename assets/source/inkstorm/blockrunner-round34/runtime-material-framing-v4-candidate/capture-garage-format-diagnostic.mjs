...
const receipt = { scope: 'Diagnostic: original WebP and losslessPNG read sequentially from the SAME preserved canvas before any next render. toDataURL observed; returned original WebP unchanged. No image processing, source change or performance claim.', port, errors: [], views: [], outcome: 'FAIL' };
let browser;
...
async function save(page, id) {
  ...
  const sameCanvas = await page.evaluate(source => window.__sameCanvasImages.findLast(item => item.webp === source), portrait.source);
  if (sameCanvas) {
    const png = Buffer.from(sameCanvas.png.split(',')[1], 'base64');
    await writeFile(`${output}/${id}-same-canvas.png`, png);
    portrait.sameCanvasPng = { bytes: png.length, sha256: sha(png) };
  }
...
try {
  ...
  await page.addInitScript(() => {
    ...
    window.__sameCanvasImages = [];
    HTMLCanvasElement.prototype.toDataURL = function(type, quality) {
      const result = original.call(this, type, quality);
      if (type === 'image/webp' && this.width >= 500) {
        window.__sameCanvasImages.push({ webp: result, png: original.call(this, 'image/png') });
        if (window.__sameCanvasImages.length > 24) window.__sameCanvasImages.shift();
      }
      return result;
    };
  });
  ...
  receipt.narrowLayout = await page.evaluate(() => ({ width: innerWidth, scrollWidth: document.documentElement.scrollWidth,
    startVisible: !!document.querySelector('[data-action="start-race"]'),
