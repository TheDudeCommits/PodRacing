...
OUTPUT = HERE / 'measured-anatomy-analysis-v2.json'
...
def welded_analysis(mesh, epsilon):
    ...
    coincidence = collections.defaultdict(list)
    original_degenerate = []
    for index, original_face in enumerate(mesh['faces']):
        face = tuple(representatives[i] for i in original_face)
        coincidence[tuple(sorted(face))].append((index, face))
        a, b, c = [mesh['positions'][i] for i in original_face]
        ab, ac = [b[k]-a[k] for k in range(3)], [c[k]-a[k] for k in range(3)]
        cross = (ab[1]*ac[2]-ab[2]*ac[1], ab[2]*ac[0]-ab[0]*ac[2], ab[0]*ac[1]-ab[1]*ac[0])
        if sum(v*v for v in cross) <= 1e-24:
            original_degenerate.append(index)
    duplicate_groups = []
    for key, occurrences in coincidence.items():
        if len(occurrences) < 2:
            continue
        parity_counts = collections.Counter(sum(face[i] > face[j] for i in range(3) for j in range(i+1,3)) % 2
                                            for _, face in occurrences)
        original_position_groups = {tuple(sorted(mesh['positions'][i] for i in mesh['faces'][index]))
                                    for index, _ in occurrences}
        is_exact = len(original_position_groups) == 1
        box = bounds([points[i] for i in key])
        duplicate_groups.append({'sourceTriangleIndices': [index for index, _ in occurrences],
                                 'multiplicity': len(occurrences), 'windingParityCounts': dict(parity_counts),
                                 'hasOpposedWinding': len(parity_counts) > 1,
                                 'exactPositionCoincidence': is_exact,
                                 'bounds': box})
    duplicate_summary = {'coincidentGroupCount': len(duplicate_groups),
                          'trianglesInCoincidentGroups': sum(g['multiplicity'] for g in duplicate_groups),
                          'redundantTrianglesIfOneKeptPerGroup': sum(g['multiplicity']-1 for g in duplicate_groups),
                          'opposedWindingGroups': sum(g['hasOpposedWinding'] for g in duplicate_groups),
                          'sameWindingOnlyGroups': sum(not g['hasOpposedWinding'] for g in duplicate_groups),
                          'exactPositionGroups': sum(g['exactPositionCoincidence'] for g in duplicate_groups),
                          'multiplicityHistogram': dict(collections.Counter(g['multiplicity'] for g in duplicate_groups)),
                          'originalNearZeroAreaTrianglesAt1eMinus12DoubleArea': len(original_degenerate),
                          'legRegionGroupsZMaxAtMost2p11': sum(g['bounds'][1][2] <= 2.11 for g in duplicate_groups),
                          'legRegionTrianglesInGroups': sum(g['multiplicity'] for g in duplicate_groups if g['bounds'][1][2] <= 2.11)}
    def find(index):
    ...
    return {'epsilonSourceMeters': epsilon, 'originalVertices': len(mesh['positions']),
            ...
            'components': component_rows, 'coincidenceSummary': duplicate_summary,
            'coincidentTriangleGroups': duplicate_groups}
...
print(json.dumps({'output': str(OUTPUT), 'sourceUnchanged': True,
                  ...
                  'pilot': [{'epsilon': p['epsilonSourceMeters'], 'components': p['componentCount'],
                             ...
                             'coincidence': p['coincidenceSummary']} for p in pilot]}, indent=2))
