...
vec2 inkstormWorkshopLight() {
  ...
  vec3 aim;
    ...
    // Each fixture is under the existing front beam, not above the opaque roof.
    aim = vec3(0., -1., -.24);
    if (vLocal.x < -21.) {
      center = vec3(-36., 11.38, -2.); range = 21.; roof = 11.39;
      receiverCenter = vec2(-39., -2.); receiverHalf = vec2(15.9, 10.4);
    } else if (vLocal.x < 18.) {
      center = vec3(1., 16.38, -6.); range = 25.; roof = 16.39;
      receiverCenter = vec2(-2., -4.5); receiverHalf = vec2(16.9, 11.9);
    } else {
      center = vec3(41., 13.18, 1.); range = 22.; roof = 13.19;
