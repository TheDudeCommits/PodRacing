...
describe('Owned porous rear-contact cloud cards', () => {
  ...
  it('keeps bounded resources, exact owner/sequence replacement and recovery/reset/disposal clearing', () => {
    ...
    let disposed = false;
    try {
      expect(GALACTIC_EFFECT_DRAW_CALL_BUDGET).toBe(9); expect(GALACTIC_EFFECT_CAPACITY.contactDust).toBe(40); expect(GALACTIC_EFFECT_CAPACITY.contactDustCards).toBe(80);
