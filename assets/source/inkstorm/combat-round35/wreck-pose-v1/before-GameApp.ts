...
export class GameApp {
  ...
  private render(dt: number): void {
    ...
    for (let racerIndex = 0; racerIndex < this.racerViews.length; racerIndex += 1) {
      ...
      if (!view || !entry) { view?.syncVisibility(); continue; }
      ...
      this.contactShadows.update(
        racerIndex,
        displayX,
        this.race.terrain.heightAt(displayX, displayZ),
        displayZ,
        displayYaw,
  ...
  private masteryStartOptions(): MasteryStartOptions {
    ...
    return {
      ...
      courseSeed: this.currentCourseSeed,
      directorSeed: this.race.state.director.seed,
      vehicleClass,
      ...
      sectorLabels: masterySectorLabels(this.race.course),
    };
  }

  private selectMasteryEvent(id: string): void {
    ...
    this.rebuildRaceForCourse(event.id === 'open-expedition' ? this.nextCourseSeed() : event.seed);
  ...
  private beginRaceCountdown(): void {
    ...
    const event = this.mastery.selectedEvent;
    // The hangar, Save course and released grid share one prepared route.
    const seed = event.id === 'open-expedition' ? this.currentCourseSeed : event.seed;
    this.rebuildRaceForCourse(seed);
    this.race.setTotalLaps(this.selectedLaps);
    this.finishStartingGrid();
  }
  ...
  private readonly handleHudAction = (action: RaceHudAction): void => {
    switch (action.type) {
      ...
      case 'save-course':
        if (this.room.lobby.role === 'solo') this.mastery.saveCourse(this.currentCourseSeed);
        this.nextHudFrame = 0;
        break;
      case 'toggle-ghost':
      ...
      case 'restart-championship':
        if (this.room.lobby.role === 'solo') this.selectMasteryEvent(this.mastery.restartChampionship().id);
        break;
  ...
  private leaveOnlineRoom(): void {
    ...
    this.room.leave();
    this.restartRace(vehicleClass);
    this.rebuildRaceForCourse(this.mastery.selectedEvent.seed);
    this.race.selectPlayerVehicle(vehicleClass);
  }
  ...
  private applyGuestAuthoritativeState(
    ...
    this.consumeGalacticEffects(galacticEvents, time);
