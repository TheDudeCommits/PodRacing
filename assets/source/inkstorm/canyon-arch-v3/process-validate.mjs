...
function geometryReceipt(p){
  ...
  assert.equal(degenerate,0);assert.equal(components,9);assert(volume>0);assert.equal(boxIntersections,0);
  ...
  assert(triangles<=45000);assert(min[0]>=-83.76&&max[0]<=86.93);
...
const source=await readFile(path.join(stage,'canyon-arch-v3-source.glb'));
...
primitive.setMaterial(doc.createMaterial('canyon-arch-v3-stone').setBaseColorFactor([1,1,1,1]).setMetallicFactor(0).setRoughnessFactor(.93));
previous.dispose();
await doc.transform(dedup(),prune());
const bytes=await io.writeBinary(doc),report=await validator.validateBytes(bytes,{uri:'canyon-arch-v3.glb',maxIssues:1000});
...
await writeFile(path.join(stage,'canyon-arch-v3.glb'),bytes);
assert.equal(hash(await readFile(oldPath)),oldHash);
const receipt={asset:'canyon-arch-v3.glb',status:'isolated candidate; no public/runtime integration',source:'original Blender MCP composition of CC0 Poly Haven boulder_01 LOD volumes',
  sourceScript:'scripts/blender/build-canyon-arch-v3.py',sourceSha256:hash(source),sha256:hash(bytes),bytes:bytes.length,
