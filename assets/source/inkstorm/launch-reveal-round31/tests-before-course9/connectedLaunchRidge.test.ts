...
import { launchRidgeRound18 } from './launchRidgeRound18';
...
describe('connected physical launch ridges', () => {
  it('preserves all 532296 round18 launch-grid lane/shoulder/normal samples bit for bit', async () => {
    ...
    expect(samples).toHaveLength(launchRidgeRound18.samples);
    const bytes = new Uint8Array(new Float64Array(samples).buffer);
    const digest = new Uint8Array(await crypto.subtle.digest('SHA-256', bytes));
    expect(Array.from(digest, value => value.toString(16).padStart(2, '0')).join('')).toBe(launchRidgeRound18.sha256);
  });
