def require_paint():
    ...
    for name,expected in STATE['finishGraphSignatures'].items():assert finish_graph_record(bpy.data.materials[name])==expected, 'Finish node graph drift'
    return scene
...
owned_world=None
stage=None
report={'stage':'IVORY_ORIGINAL_SURFACE_FINISH_V1_MATCHED_RENDER','view':RENDER['view'],'status':'STARTED','renderPassed':False,
...
try:
    ...
    try:
        ...
        require_histories();require_palette();require_paint()
        report['allSixHistoriesAndFinishMasksGraphsExact']=True
        preservation=report['globalPreservation']
        report['renderPassed']=report['status']=='RENDERED_INSPECTION_PENDING' and report['allSixHistoriesAndFinishMasksGraphsExact'] and preservation['contextRestored'] and preservation['allSceneSettingsAndMembershipsPreserved'] and preservation['allCollectionMembershipsPreserved'] and preservation['noPersistentDatablocksCreatedOrRemoved']
    except Exception as error:
