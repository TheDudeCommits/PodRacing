...
The checked-in `tests/fixtures/blockrunnerPackage.json` fixture supplies actual package hashes, bytes and counts; ordinary `npm run verify` needs no external environment or private source GLB. The staged fixture now pins the actual final V5 packages from `../packaged-atlas-v5-v1/`; the entire prior V3 candidate is preserved in `../runtime-candidate-BEFORE-V5-FINAL/`. No placeholder hash is used. The tests independently inspect public bytes, canonical meshes/materials, absence of COLOR attributes and embedded image dimensions.
...
After final package preparation/copy and coordinated application, run ordinary focused tests and `npm run verify`; the fixture is part of the patch. Both the preserved V3 and final V5 isolated validations passed 42 tests/5 files with real private package bytes and synthetic renderer fixtures; the final result is `../focused-checks-v5-sola.json`. No browser, build, driving, performance or final art acceptance is claimed.
