...
import type { CourseSample } from '../../../../src/game/race/types';
import { LAUNCH_LANDSCAPE_BOUNDS, launchRidgeSurface } from '../../../../src/game/race/LaunchBasinPlan';
...
export function createCourseGulfField(course: PodraceCourse, options: { forkApproach?: boolean } = {}): CourseGulfField | null {
  ...
  // Preserve the original grid origin: moving its texels would resample every
  // unrelated ridge and protected normal in the launch field.
  const routeCenter = course.samplePlanAtProgress((launch.distance + 510) / course.totalLength);
