#!/usr/bin/env python3
"""Read actual latest receipts and compose an MCP payload. Never executes Blender."""
...
def main():
    ...
    if 'paintAttributeSignatures' in state: state_keys.append('paintAttributeSignatures')
    ...
    text='\n'.join((BASE/'mcp-safe'/f).read_text() for f in ['00-source-reference.py','01-source-guard.py','13-v4-attribute-signatures.py'])+'\n'
    text+='\n'.join(name+' = json.loads('+repr(json.dumps(value,allow_nan=False,separators=(',',':')))+')' for name,value in assignments.items())+'\n'
    text+=(HERE/'12-copy-normalize-export.py').read_text()
    ast.parse(text)
