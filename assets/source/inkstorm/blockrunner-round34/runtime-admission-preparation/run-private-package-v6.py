#!/usr/bin/env python3
"""Run a fresh V6 clamp-only private package/validator sequence against immutable V5.
...
try:
    ...
    run('05-sampler-only',[sys.executable,str(HERE/'verify-sampler-only-package-v6.py'),'--package-directory',str(out),'--output',str(out/'sampler-only-vs-v5-validation.json')])
    run('06-fixture',[sys.executable,str(HERE/'make-package-fixture.py'),'--hero',str(hero.with_suffix('.package-receipt.json')),'--rival',str(rival.with_suffix('.package-receipt.json')),'--output',str(out/'blockrunnerPackage.json'),'--scope','Actual private final-atlas package bytes. Runtime and fresh visual admission remain pending.'])
    assert hashlib.sha256(a.master.read_bytes()).hexdigest()==a.expected_master_sha256
    print(json.dumps({'status':'private V6 package/validator/sampler-only sequence PASS; no visual/runtime acceptance','output':str(out),'steps':steps},indent=2))
finally:
