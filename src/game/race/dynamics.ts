...
export function resolveLaunchOutcome(state: RacerLaunchState): Exclude<LaunchOutcome, 'pending'> {
  ...
  if (state.engineHeat >= 0.84 || state.committedThrottle > 0.96) outcome = 'overheat';
  else if (state.committedThrottle < 0.28) outcome = 'bog';
  else if (error <= 0.105 && state.committedThrottle >= 0.48) outcome = 'perfect';
  else outcome = 'good';
