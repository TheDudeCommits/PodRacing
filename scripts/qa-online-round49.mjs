...
try{
 ...
 await host.locator('.setup-destination>summary').click();await host.locator('[data-destination="frozen"]').click();await host.getByRole('button',{name:'Race',exact:true}).first().click();
 ...
 if(await guest.locator('.setup-types button:disabled').count()!==4)throw new Error('Guest mode controls are not locked');
