...
describe('shared off-road workshop grading', () => {
  it('clears the full rotated source slabs while freezing their original center anchors', () => {
    for (const { field, placements, before } of cases) {
      for (const pad of field.pads) {
        ...
        // The pit slab is offset; district foundations are centered.
        const district = pad.family === 'pit-district';
        const corners = district ? [[-60, -18], [-60, 18], [60, -18], [60, 18]]
          : [[-71, -32.5], [-71, 28], [79, -32.5], [79, 28]];
        for (const [lx, lz] of corners) {
          ...
          const corner = padLocalToWorld(pad, (lx! - (district ? 0 : 4)) * placement.sx,
            (lz! + (district ? 0 : 2.25)) * placement.sz);
        ...
        expect(maximum).toBeLessThan(pad.anchorHeight + pad.slabBottomOffset - .3);
      }
    }
  });
  ...
  it('records the expedition pad that conflicts with the protected shoulder without moving it', () => {
    ...
    expect(seed42.field.pads.filter(p => p.accepted && p.family === 'pit-complex')).toHaveLength(2);
    expect(cases[2]!.field.pads.filter(p => p.accepted && p.family === 'pit-complex')).toHaveLength(3);
    expect(cases.map(c => c.field.pads.filter(p => p.accepted && p.family === 'pit-district').length)).toEqual([4, 2, 3]);
