/** Optimize/validate the fractured-spire accent and its LOD; preserve broad buttresses.
...
const highBytes=await io.writeBinary(high),highRecord=await validate(highBytes,'fractured-spire.glb',8000);
...
const lowBytes=await io.writeBinary(low),lowRecord=await validate(lowBytes,'fractured-spire-lod.glb',3000);
assert(lowRecord.triangles<highRecord.triangles);
if(checkOnly){
 assert.equal(hash(await readFile(path.join(publish,'fractured-spire.glb'))),highRecord.sha256);
 assert.equal(hash(await readFile(path.join(publish,'fractured-spire-lod.glb'))),lowRecord.sha256);
 console.log('PASS: regenerated high/LOD bytes match published GLBs exactly; all geometry/validator gates pass.');
 process.exit(0);
}

// Record the preserved broad-cliff sources; this pipeline only publishes spires.
// The round-14 pipeline and receipt remain archived beside the original pair.
await mkdir(history,{recursive:true});
const archived=[];
for(const name of['canyon-buttress.glb','canyon-buttress-lod.glb']){
 const priorPath=path.join(history,name);
 await access(priorPath);
...
await writeFile(path.join(publish,'fractured-spire.glb'),highBytes);
await writeFile(path.join(publish,'fractured-spire-lod.glb'),lowBytes);
const receipt={source:'Original Blender geometry authored through Blender MCP; not a downloaded model',sourceScript:'build_geology_revision.py',sourceSha256:hash(await readFile(path.join(source,'build_geology_revision.py'))),postprocessScript:'build_geology_revision.mjs',rawBlenderExportSha256:hash(raw),
 ...
 preserved:{archived,priorSourceScriptsUnchanged:['build_fractured_rocks.py','build_rock_lods.mjs'],unrelatedPublicAssets:'Only fractured-spire.glb and fractured-spire-lod.glb are published by this script; restored canyon buttresses remain unchanged',originalActiveBlenderScene:'Korostyshiv Fractured Quarry Revision',originalSceneRestored:true,priorBlenderObjectsPreserved:true,noPurges:true},
...
await writeFile(path.join(source,'fractured-spire-receipt.json'),JSON.stringify(receipt,null,2)+'\n');
