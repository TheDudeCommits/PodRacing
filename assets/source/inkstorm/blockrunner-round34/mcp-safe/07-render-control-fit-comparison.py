...
Also inject BLOCKRUNNER_CONTROL_FIT_RECEIPT (successful stage-06 receipt) and BLOCKRUNNER_SOURCE_RENDER (matching actual original-view receipt). Repeats its exact camera. No source/fit edit or shared blend save.
"""
...
output_path = OUT + '/control-fit-v1-' + BLOCKRUNNER_VIEW + '-' + BLOCKRUNNER_RENDER_TOKEN + '.png'
stage_name = 'PodRacing temporary Blockrunner control fit comparison ' + BLOCKRUNNER_VIEW + ' ' + BLOCKRUNNER_RENDER_TOKEN
...
owned_cameras = []
owned_world = None
report = {'stage': 'control-fit-neutral-comparison', 'status': 'executing', 'sourceUid': REFERENCE['uid'],
          ...
          'scope': 'Source-only control fit under the exact prior neutral-view camera and lighting; mechanical appearance, style and gameplay acceptance pending.'}
try:
    source = source_guard()
    ...
    cleanup = bpy.data.scenes.get(BLOCKRUNNER_CONTROL_FIT_RECEIPT['targetScene'])
    assert cleanup is not None, 'Exact reviewed fit scene missing.'
    ...
    assert cleanup_before == BLOCKRUNNER_CONTROL_FIT_RECEIPT['fitSignature'], 'Fit geometry/materials changed since receipt.'
    assert cleanup_normals_before == BLOCKRUNNER_CONTROL_FIT_RECEIPT['fitCornerNormalSignature'], 'Fit normals changed since receipt.'
    matched = []
    for lineage in BLOCKRUNNER_CONTROL_FIT_RECEIPT['objectLineage']:
    ...
    assert len(matched) == 61 and sum(len(o.data.polygons) for o in cleanup.objects if o.type == 'MESH') == 44028
    ...
    if cleanup_before is not None:
        report['fitPreservation'] = source_signature(cleanup) == cleanup_before
        report['fitCornerNormalsPreserved'] = cleanup_normal_signature(cleanup) == cleanup_normals_before
    ...
    if cleanup_before is not None:
        assert report['fitPreservation'] and report['fitCornerNormalsPreserved'], 'Fit source changed during comparison.'
