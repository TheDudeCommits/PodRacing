...
OUTPUT = BASE / '15d-author-contact-v7-composed-payload.py'
INVENTORY = BASE.parent / 'v7-contact-preparation' / 'preparation-inventory-v4.json'
...
names = ['00-source-reference.py', '01-source-guard.py', '13-v4-attribute-signatures.py',
         '14-atlas-wrap-guards.py', '15d-ao-contact-guards.py']
...
payload += (BASE / '15d-copy-contact-color-v7-literal-properties.py').read_text()
...
for node in ast.walk(tree):
    ...
    if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in {'getattr', 'hasattr', 'setattr'}:
        assert len(node.args) >= 2 and isinstance(node.args[1], ast.Constant) and isinstance(node.args[1].value, str), 'Safe MCP attribute probe requires literal name.'
...
inventory = {
 'stage': 'V7 short contact color preparation', 'status': 'prepared only; no Blender execution', 'supersedesRejectedBeforeExecutionPayload': '15c-author-contact-v7-composed-payload.py', 'safeModeCorrection': 'Explicit node-type property reads replace dynamic getattr RNA loop; AO property probes use literal names. Module-as-value preflight remains removed. Exact semantic graph/AO/roughness/geometry/normal/image/context guards retained', 'transactionFix': 'Cleanup nested finally; success only after all source/context/image/graph guards pass',
 ...
 'fragments': [digest(BASE / name) for name in names + ['15d-copy-contact-color-v7-literal-properties.py']],
