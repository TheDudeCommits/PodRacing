    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      for (const point of points) {
        ...
        if (impactFrame) {
          ...
          // interval endpoints retain x=+/-.74 and y=[-.58,.70] exactly.
          leftAim = Math.max(leftAim, x + .74 * horizontal * z);
          rightAim = Math.min(rightAim, x - .74 * horizontal * z);
      ...
      if (impactFrame) {
        distance = Math.max(distance, (leftAim - rightAim) / (1.48 * horizontal),
