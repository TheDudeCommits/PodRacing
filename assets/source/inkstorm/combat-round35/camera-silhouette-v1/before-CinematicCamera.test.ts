...
describe('cinematic cockpit presentation', () => {
  it('keeps a separating vehicle envelope inside the live wreck shot across desktop and portrait aspects', () => {
    const rig = new CinematicCamera();
    const subject = { position: new Vector3(), forward: new Vector3(0, 0, 1), velocity: new Vector3(), speed: 0 };
    rig.setMode('side');
    rig.setCombatFraming(true);
    for (const aspect of [1440 / 900, 390 / 844]) {
      rig.camera.aspect = aspect;
      rig.snap(subject);
      rig.camera.updateMatrixWorld(true);
      // A conservative authored envelope, not an assertion about every imported asset.
      for (const x of [-13, 13]) for (const y of [-1, 12]) for (const z of [-14, 24]) {
