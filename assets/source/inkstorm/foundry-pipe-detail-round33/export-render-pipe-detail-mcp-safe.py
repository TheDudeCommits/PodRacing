...
SOURCE_STAGE = 'PodRacing — Foundry pipe detail round33 V1'
REVIEW_STAGE = 'PodRacing — Foundry pipe detail round33 technical V1'
FAMILY = 'pipe-bank-detail-v1'
OUT = '/Users/amir/Projects/PodRacing/assets/source/inkstorm/foundry-pipe-detail-round33/v1/'
EXPECTED_SOURCE_SHA256 = '4b3510d0f8b10e1a44158edb0b0bbdc25bac33019176368638937ab1834a3b81'
RENDER_RESOLUTION = (1400,1000)
CAMERAS = [
    ('approach-three-quarter',(-1.05,-1.60,.90)),
    ('rear-three-quarter',(1.10,1.60,.80)),
    ('side-depth',(1.00,0,.32)),
]
...
def execute_export_and_review():
    ...
    assert source.get('retainedTriangles')==7160 and source.get('connectedMouthsUnchanged') is True
    ...
    assert len(source.data.loop_triangles)==19960 and source.get('candidateTriangles')==19960
    ...
    report = {'sourceScene':SOURCE_STAGE,'scope':'Neutral source construction inspection; not gameplay, art acceptance or FPS evidence.',
              ...
              'sourceGeometryDraws':1,'sourceMaterialSlots':1,'sourceImages':0,'sourceTriangles':19960,
              ...
              'foundationView':'The exact original foundation footprint is fully exposed for technical review.'}
    try:
        ...
        center = (low+high)*.5
        assert abs(low.x+47)<2e-4 and abs(high.x-47)<2e-4
        assert abs(low.y+20.20081901550293)<2e-4 and abs(high.y-23)<2e-4
        assert abs(low.z)<2e-4 and abs(high.z-51)<2e-4
        ...
        floor_mesh.from_pydata([(-160,-140,-.04),(160,-140,-.04),(160,140,-.04),(-160,140,-.04)],[],[(0,1,2,3)])
        ...
        # Fit the full preserved base, tanks and free terminal constructions, with common scale across
