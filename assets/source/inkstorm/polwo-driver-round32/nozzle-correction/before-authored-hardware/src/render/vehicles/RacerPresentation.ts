...
export class RacerPresentation extends Group {
  ...
  update(pose: PodracerPose, time: number): void {
    ...
    if (this.importedActive) {
      for (let index = 0; index < this.exhaustMeshes.length; index += 1) {
        ...
        // The jet exits the recessed 0.44m throat. A full 0.82m cone spilled
        // cyan over the metal bowl, visually flattening its real depth.
        flame.scale.x *= .54;
        flame.scale.z *= .54;
  ...
  syncVisibility(): void {
    ...
    this.effects.visible = imported && this.lodMode !== 'silhouette';
  ...
  private refreshCoupling(): void {
    ...
    this.nozzleLips?.setAnchors(this.imported.getAttachment('exhaustLeft'), this.imported.getAttachment('exhaustRight'));
    if (this.coupling) {
