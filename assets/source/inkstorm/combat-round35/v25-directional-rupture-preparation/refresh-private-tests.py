...
(b/'vitest.config.ts').write_text("import {defineConfig} from 'vitest/config';import{dirname,resolve}from'node:path';const targets=new Map("+json.dumps(list(paths.items()))+");export default defineConfig({plugins:[{name:'private-v25',enforce:'pre',resolveId(source,importer){const p=source.startsWith('.')&&importer?resolve(dirname(importer),source):source;return targets.get(p)??targets.get(p+'.ts')??null;}}],test:{maxWorkers:1,fileParallelism:false,include:['tests/render/GalacticEffectsView.test.ts','tests/render/GalacticEffectsContact.test.ts','tests/render/WreckFootprintEffects.test.ts','tests/combat/AuthoredRupture.test.ts','assets/source/inkstorm/combat-round35/v25-directional-rupture-preparation/*.test.ts']}});\n")
