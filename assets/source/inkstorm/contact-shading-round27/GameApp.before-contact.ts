...
import { RaceMastery, INKSTORM_HERO_SEED, type CompetitionProfile, type MasteryStartOptions } from '../../game/mastery';
import { loadVehicleAppearance, saveVehicleAppearance, type VehicleAppearanceId } from '../../game/vehicleAppearance';
...
export class GameApp {
  ...
  private readonly inkstormWorld = new InkstormWorld((x, z) => this.terrainAdapter.heightAt(x, z), this.terrain.gulfTextures.uniforms);
  ...
  constructor(private readonly mount: HTMLElement) {
    ...
    for (const racerView of this.racerViews) {
      ...
      void racerView.setAppearance(racerView === this.playerView ? this.vehicleAppearance : 'teemto');
    }
  ...
  private stepSimulation(liveInput?: PlayerInputState): void {
    ...
    if (!this.captureMode && this.room.lobby.role === 'solo') this.mastery.step(result);
  ...
  private render(dt: number): void {
    ...
    for (let racerIndex = 0; racerIndex < this.racerViews.length; racerIndex += 1) {
      ...
      void view.setAppearance(entry.id === this.localRacerId() ? this.vehicleAppearance : 'teemto');
  ...
  private consumeGalacticEffects(events: readonly GalacticEvent[], time: number): void {
    for (const event of events) {
      ...
      this.galacticEffects.emitCrash({
        ...
        groundY: this.race.terrain.heightAt(entry.vehicle.position.x, entry.vehicle.position.z),
        severity,
        color,
        style,
      });
