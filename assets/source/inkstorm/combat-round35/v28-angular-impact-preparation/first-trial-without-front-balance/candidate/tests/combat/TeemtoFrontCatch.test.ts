...
describe('front edge catch and changing source silhouette', () => {
  ...
  it.each(variants)('%s changes the actual front longitudinal silhouette, then reaches the exact broad roof orientation', name => {
    ...
    try {
      ...
      const rows: unknown[] = [];
      let contactAxis: Vector3 | undefined, contactGrade = 0;
      for (const age of [.13, .3, .4, .5, .6, .65, .77, .9]) {
        const snap = sample(age), axis = zAxis.clone().applyQuaternion(snap.q); if (!contactAxis) { contactAxis = axis.clone(); contactGrade = snap.grade; }
        expect(Math.asin(axis.y)).toBeCloseTo(snap.grade - TEEMTO_FRONT_CATCH_PITCH * sampleTeemtoFrontCatch(age), 8);
        ...
        if (age === .9) expect(axis.angleTo(contactAxis)).toBeCloseTo(-TEEMTO_FRONT_CATCH_PITCH + contactGrade - snap.grade, 8);
      }
      ...
      expect(zAxis.clone().applyQuaternion(rollOnlyQ).angleTo(zAxis.clone().applyQuaternion(rest.q))).toBeCloseTo(Math.abs(contact.grade - rest.grade), 8);
