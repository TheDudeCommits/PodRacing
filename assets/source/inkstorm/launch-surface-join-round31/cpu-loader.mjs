...
const candidate=path.join(root,'assets/source/inkstorm/launch-surface-join-round31');
export async function resolve(specifier, context, next){
 ...
 if(specifier.startsWith('.')&&parent){
  ...
  for(const ext of ['', '.ts', '/index.ts']) if(existsSync(file+ext)&&statSync(file+ext).isFile()) { const resolved=file+ext; const selected=process.env.JOIN_CANDIDATE==='1' && resolved===path.join(root,'src/game/race/CourseGulfField.ts') ? path.join(candidate,'CourseGulfField.ts') : process.env.JOIN_CANDIDATE==='1' && resolved===path.join(root,'src/game/mastery/events.ts') ? path.join(candidate,'events.ts') : resolved; return {url:pathToFileURL(selected).href,shortCircuit:true}; }
 }
