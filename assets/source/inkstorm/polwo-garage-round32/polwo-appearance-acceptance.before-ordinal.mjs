...
function assetHeader(bytes) {
  ...
  assert(triangles === expected.triangles && bodyTriangles === expected.bodyTriangles,
...
try {
  ...
  const servedAsset = await fetch(`${origin}${assetPath}`, { signal: AbortSignal.timeout(15000) });
  assert(servedAsset.ok, `Served Polwo asset is missing: HTTP ${servedAsset.status}`);
  const servedBytes = Buffer.from(await servedAsset.arrayBuffer());
  receipt.asset.preflightServed = { bytes: servedBytes.length, sha256: sha256(servedBytes), status: servedAsset.status };
  assert(receipt.asset.preflightServed.sha256 === receipt.asset.sha256, 'Preview serves a stale/different Polwo GLB; rebuild before running');
  ...
  page.on('response', response => {
    if (response.url() !== `${origin}${assetPath}`) return;
    const responseStage = stage;
    const intentionallyFailed = failureInjectionActive && response.status() === 503;
    const task = (async () => {
      ...
      assert(row.sha256 === receipt.asset.sha256, `Browser Polwo payload differs from the public GLB in ${responseStage}`);
  ...
  stage = 'ordinary-drive'; await page.locator('[data-action="start-race"]').click();
