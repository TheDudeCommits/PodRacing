...
const RUPTURE_FLASH_SCALE = 1.35;
const RUPTURE_FLASH_CORE_RADIUS = 0.4;
...
function createAtmosphericMaterial(): MeshBasicMaterial {
  ...
  result.onBeforeCompile = (shader) => {
    ...
    `).replace('#include <color_fragment>', `
      ...
      float alpha;
      if (vEffectSurface.x > 1.5) {
        // Short ivory rupture point with broken radial sparks. Its radius is
        // bounded in world space; no screen-space wash or bloom is required.
        float angle = atan(p.y, p.x);
        float rays = pow(max(0.0, cos(angle * 9.0 + vEffectSurface.z)), 18.0);
        float core = 1.0 - smoothstep(0.12, ${RUPTURE_FLASH_CORE_RADIUS}, radius);
        alpha = max(core, rays * (1.0 - smoothstep(0.2, 0.92, radius)) * 0.72);
  ...
  result.customProgramCacheKey = () => 'painterly-atmosphere-contact-v2';
  return result;
}
...
export class GalacticEffectsView extends Group {
  ...
  emitCrash(event: CrashEffectEvent): void {
    ...
    const burstY = contactBurst && Number.isFinite(event.groundY)
    ...
    for (let fragment = 0; fragment < fragmentCount; fragment += 1) {
      ...
      const radial = contactBurst
        ? ((spark ? 20 : 6) + hash01(seed + 1) * (spark ? 16 : 8)) * Math.sqrt(severity)
    ...
    const plateCount = style === 'rock' ? 2 : contactBurst || style === 'reward' || style === 'energy' ? 4 : 3;
    for (let plate = 0; plate < plateCount; plate += 1) {
      ...
      if (contactBurst) {
        // One rupture point, one restrained amber flare, two soft smoke lobes.
        // Four existing slots replace four ever-growing red discs.
        slot.startTime = finite(event.time, 0) + (plate === 0 ? 0 : plate * 0.025);
        slot.duration = [0.14, 0.28, 0.68, 0.82][plate]!;
        slot.x = finite(event.position.x, 0);
        slot.y = burstY + (plate > 1 ? (plate - 1) * 0.35 : 0);
        slot.z = finite(event.position.z, 0);
        slot.scale = severity * [RUPTURE_FLASH_SCALE, 1.8, 1.45, 1.7][plate]!;
        slot.billboard = true;
        slot.style = style;
  ...
  private updateCrash(time: number): void {
    ...
    for (const slot of this.explosionSlots) {
      ...
      if (slot.contactLayer >= 0) {
        ...
        const y = slot.y + (smoke ? progress * 1.8 : 0);
        if (!this.isVisible(slot.x, y, slot.z, size, EFFECT_VISIBILITY_DISTANCE.crashes)) continue;
        const opacity = (smoke ? 0.38 : flash ? 1 : 0.58)
          * (1 - MathUtils.smoothstep(progress, smoke ? 0.28 : 0.08, 1));
        this.writeInstance(this.explosionPlates, plateCount, slot.x, y, slot.z,
          this.billboardQuaternion, size, size, size, slot.r, slot.g, slot.b);
        this.writeEffectSurface(plateCount, smoke ? 1 : flash ? 2 : 0,
