...
.race-event-atlas__orbits { stroke-width:1.2; opacity:.58; }
.race-event-atlas__orbits .atlas-orbit-main { stroke-width:3; opacity:.8; }
.race-event-atlas__orbits .atlas-orbit-secondary { stroke-width:1.2; opacity:.7; }
.race-event-atlas__orbits .atlas-orbit-dash { stroke-width:1.3; stroke-dasharray:6 11; opacity:.55; }
.race-event-atlas__footer { padding-bottom:28px; }
...
.race-event-atlas__details { border-left:5px solid var(--atlas-red); padding-left:17px; }
.race-event-atlas__description { font-size:16px; }
.race-event-atlas__name { font-size:13px; }
.race-event-atlas__help { font-size:11px; }
.race-event-atlas__surface { position:absolute; inset:0; width:100%; height:100%; border-radius:50%; opacity:.8; mix-blend-mode:soft-light; }
.race-event-atlas__disc { background:radial-gradient(ellipse at 28% 20%,#fff2d688,transparent 55%),radial-gradient(ellipse at 35% 32%,var(--atlas-tone),var(--atlas-petrol) 84%,var(--atlas-ink)); box-shadow:inset -12px -12px 16px #101e2888,0 5px 8px #171e2317; }
.race-event-atlas__number { top:72%; font-size:13px; line-height:1; padding:3px 4px; color:#fff0c9; background:#182f36c9; border-radius:2px; }
.race-event-atlas__event:not(.is-selected):nth-child(3n+1) .race-event-atlas__disc { transform:scale(.9); }
.race-event-atlas__event:not(.is-selected):nth-child(3n+2) .race-event-atlas__disc { transform:scale(1.08); }
@media (min-width:721px) and (min-height:541px) {
  .race-event-atlas__map { height:clamp(340px,calc(100dvh - 340px),640px); }
}

/* One selection ring and a quiet satellite system. */
.race-event-atlas__disc::before,.race-event-atlas__disc::after { content:none; }
...
.race-event-atlas__event:not(.is-selected) .race-event-atlas__name { color:#415a5f; font-size:12px; letter-spacing:.01em; background:#e8e7dcf2; }
.race-event-atlas__event.is-selected .race-event-atlas__name { font-size:15px; font-weight:700; }
.race-event-atlas__marker { font-size:11px; letter-spacing:.1em; }
.race-event-atlas__orbits { opacity:.65; }
.race-event-atlas__orbits .atlas-orbit-main { stroke-width:2; opacity:.95; }
.race-event-atlas__orbits .atlas-orbit-dash { opacity:.24; }
.race-event-atlas__help { font-size:12px; letter-spacing:.035em; }

/* A broad printed sweep establishes a common orbital reading direction. */
.race-event-atlas__header { padding-bottom:16px; }
.race-event-atlas__band { height:32px; min-height:32px; background:linear-gradient(90deg,#244249 0 70%,#41666c 70%); }
.race-event-atlas__band::after { top:39px; background:#a7b6aa; }
.race-event-atlas__orbits { opacity:.8; stroke-width:1.2; }
.race-event-atlas__orbits .atlas-orbit-main { stroke-width:2.6; opacity:1; }
.race-event-atlas__orbits .atlas-orbit-secondary { stroke-width:1.4; opacity:.78; }
.race-event-atlas__orbits .atlas-orbit-accent { stroke-width:4; opacity:.75; }
.race-event-atlas__event:not(.is-selected) .race-event-atlas__name { font-size:14px; color:#203a42; line-height:1.2; font-weight:700; }
.race-event-atlas__event.is-selected .race-event-atlas__name { font-size:17px; }
.race-event-atlas__event.is-selected .race-event-atlas__marker { border-top:2px solid #b54a36; width:82px; margin:7px auto 0; padding-top:5px; }
.race-event-atlas__surface { opacity:.9; }
.race-event-atlas__description { font-size:17px; }
.race-event-atlas__title { font-weight:700; letter-spacing:-.025em; }
@media (max-width:720px), (max-height:540px) {
 .race-event-atlas__event:not(.is-selected) .race-event-atlas__name { font-size:12px; }
 .race-event-atlas__description { font-size:14px; }
}
...
@media (max-width:720px) { .race-event-atlas__heading::after { width:48px; height:20px; margin-left:12px; } }

/* Broad destination lettering and three orbital weights establish the map hierarchy. */
.race-event-atlas__heading { font:400 clamp(34px,3.7vw,54px)/1 var(--ui-heading); letter-spacing:-.015em; }
.race-event-atlas__edition { font-size:12px; letter-spacing:.1em; }
.race-event-atlas .race-event-atlas__event { width:128px; transition:left 420ms var(--ui-settle),top 420ms var(--ui-settle); }
.race-event-atlas__event:not(.is-selected) .race-event-atlas__name { font:400 15px/1.25 var(--ui-heading); letter-spacing:0; color:#203a42; margin-left:-12px; margin-right:-12px; padding:3px 0; }
.race-event-atlas__event.is-selected .race-event-atlas__name { font:400 20px/1.18 var(--ui-heading); letter-spacing:0; padding:5px 7px; background:#e8e7dcf5; }
.race-event-atlas__event.is-selected .race-event-atlas__marker { font-size:12px; letter-spacing:.06em; width:96px; }
.race-event-atlas__disc { transition:transform 380ms var(--ui-settle),box-shadow 380ms var(--ui-settle); }
.race-event-atlas__name,.race-event-atlas__marker { transition:transform 380ms var(--ui-settle),opacity 220ms ease; }
.race-event-atlas__orbits { opacity:.85; }
.race-event-atlas__orbits .atlas-orbit-main { stroke-width:3.4; }
.race-event-atlas__orbits .atlas-orbit-secondary { stroke-width:1.8; opacity:.78; }
.race-event-atlas__orbits .atlas-orbit-accent { stroke-width:3.5; opacity:.85; }
.race-event-atlas__eyebrow { font-size:12px; letter-spacing:.08em; }
.race-event-atlas__title { font:400 clamp(26px,2.3vw,34px)/1.15 var(--ui-heading); letter-spacing:0; }
.race-event-atlas__description { max-width:57ch; margin-top:10px; font-size:17px; }
.race-event-atlas__details { border-left-width:3px; padding-left:18px; }
.race-event-atlas .race-event-atlas__start { font:400 16px/1.2 var(--ui-heading); letter-spacing:0; border-radius:4px; transition:background-color 220ms var(--ui-settle); }
.race-event-atlas .race-event-atlas__start:hover:not(:disabled) { background:#355b63; }
.race-event-atlas__close { font-size:13px!important; }
...
@media (max-width:720px), (max-height:540px) {
 .race-event-atlas__event:not(.is-selected) .race-event-atlas__name { font-size:13px; }
...
/* A short screen scrolls the complete map and footer together. The 360px
   drawing needs an equal intrinsic viewport; a 240px basis let labels reach
   into the footer even though the map itself retained its 340px minimum. */
@media (max-height:540px) and (min-width:721px) {
