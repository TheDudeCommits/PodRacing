...
let refinementLockCount=0;
if(args['surface-locks']){const measured=JSON.parse(await fs.readFile(args['surface-locks'],'utf8'));if(measured.heroSha256!==hash(raw)||!Array.isArray(measured.additionalVertexLocks))throw Error('Exact sampled hero locks required');for(const i of measured.additionalVertexLocks){if(!Number.isInteger(i)||i<0||i>=locks.length)throw Error('Bad refinement lock');if(!locks[i])refinementLockCount++;locks[i]=1;}}
