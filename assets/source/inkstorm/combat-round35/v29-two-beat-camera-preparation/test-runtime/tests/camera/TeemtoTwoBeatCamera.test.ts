...
import { CinematicCamera, type CameraSubject } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v29-two-beat-camera-preparation/test-runtime/src/camera/CinematicCamera.ts';
import { WRECK_PRESENTATION_DURATION, type WreckVisualPose } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v29-two-beat-camera-preparation/test-runtime/src/render/combat/WreckVisualPose.ts';
import { fixture, matrices, race, step, variants } from '/Users/amir/Projects/PodRacing/tests/fixtures/teemtoActualCamera';
...
describe('event-relative two-beat authored camera', () => {
  it('arcs once by 14 degrees, independently of frame history, and keeps the terminal direction through intact recovery', () => {
    ...
    const ages = [0, .015, .06, .15, .3, .5, .8, 1.3];
    ...
    try {
      ...
      subject.combatImpactFraming = { ...subject.combatImpactFraming!, ageSeconds: .15 };
  ...
  it('bounds explicit horizontal composition margins and restores the legacy limit for absent or invalid input', () => {
    const subject = framingSubject(), a = shot(), b = shot();
    try {
      const frame = subject.combatImpactFraming!;
      for (const [input, expected] of [[undefined, .74], [NaN, .74], [Infinity, .74], [-2, .74], [.88, .88], [4, .90]] as const) {
        a.snap({ ...subject, combatImpactFraming: { ...frame, horizontalHalfSpan: input } });
        b.snap({ ...subject, combatImpactFraming: { ...frame, horizontalHalfSpan: expected } });
        expect(state(a)).toEqual(state(b));
        // A wide composition never grants permission for full source bounds
        // to cross the lens, even when some source parts leave the picture.
        a.camera.updateMatrixWorld(true);
        for (const point of subject.combatBounds!) expect(point.clone().applyMatrix4(a.camera.matrixWorldInverse).z).toBeLessThan(-a.camera.near - .49);
      }
    } finally { a.dispose(); b.dispose(); }
  });
  ...
  it.each(variants)('%s reuses its composition storage, admits the intact engine only during the second beat, and resets exactly', variant => {
    ...
    try {
      ...
      for (const age of [.09, .13, .3, .65, 1.3, .1]) {
        ...
        if (age <= .13) for (let i = 18; i < 26; i++) expect(frame.bounds[i]!.distanceTo(earlyCenter)).toBeLessThan(1e-10);
        if (age >= .65) for (let i = 0; i < 8; i++) expect(frame.bounds[18 + i]!.distanceTo(pose.bounds[8 + i]!)).toBeLessThan(1e-10);
