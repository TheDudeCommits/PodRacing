...
records=[]
for asset in ['rock_face_02','rock_face_01']:
    ...
    records.append({'asset':asset,'page':'https://polyhaven.com/a/'+asset,'author':'Dario Barresi','processing':'Rico Cilliers' if asset=='rock_face_02' else None,'license':'CC0','licenseUrl':'https://polyhaven.com/license','manifestUrl':'https://api.polyhaven.com/files/'+asset,'manifestSha256':hashlib.sha256(manifest_path.read_bytes()).hexdigest(),'resolution':'1k','files':files,'archive':{'path':str(archive.relative_to(ROOT)),'sha256':hashlib.sha256(archive.read_bytes()).hexdigest(),'purpose':'Locally packaged unmodified downloaded files; not an official provider ZIP'}})
receipt={'date':'2026-09-07','userAgent':UA,'scope':'Two staged art experiments only; no runtime/public replacement','licenseVerification':'Official model pages explicitly label both assets CC0; Poly Haven license page permits use, modification and redistribution. Verified 2026-09-07.','assets':records}
(ROOT/'source-download-receipt.json').write_text(json.dumps(receipt,indent=2)+'\n')
