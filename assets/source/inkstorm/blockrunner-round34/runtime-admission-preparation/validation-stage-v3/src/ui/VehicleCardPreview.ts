...
export class VehicleCardPreviewRenderer {
  ...
  private collectHosts(root: ParentNode): PreviewHost[] {
    ...
    for (const element of findPreviewHosts(root)) {
      ...
      const size = calculateVehiclePreviewRenderSize(rect.width, rect.height, pixelRatio, this.limits);
  ...
  private renderVehicle(
    ...
    const parkedPolwo = vehicle.activeAppearanceId === 'polwo';
    vehicle.update({ x: 0, y: 0, z: 0, yaw: -.18, pitch: .025, roll: 0,
      steer: .12, throttle: parkedPolwo ? 0 : .7, speed: parkedPolwo ? 0 : 126,
      boost: parkedPolwo ? 0 : .12, damage: 0 }, 1.75 + vehicle.racerIndex * .31);
