...
interface PreviewResources {
  ...
  readonly vehicles: ReadonlyMap<GalacticVehicleClass, PodracerView>;
 ...
 */
function setVisibleGeometryBounds(target: Box3, vehicle: PodracerView): Box3 {
  target.makeEmpty();
  vehicle.traverseVisible((object) => {
    ...
    geometryBounds.copy(localBounds).applyMatrix4(object.matrixWorld);
...
export class VehicleCardPreviewRenderer {
  ...
  private readonly cache = new Map<GalacticVehicleClass, PreviewCacheEntry>();
  ...
  private async performRefresh(): Promise<void> {
    ...
    const missing = hosts.filter((host) => {
      const cached = this.cache.get(host.vehicleClass);
      return !cached
    ...
    if (missing.length > 0) {
      ...
      for (const host of missing) {
        ...
        this.cache.set(host.vehicleClass, {
    ...
    const commits = hosts.flatMap((host) => {
      const cached = this.cache.get(host.vehicleClass);
      return cached ? [{ host, source: cached.source }] : [];
    });
    ...
    for (const { host, source } of commits) {
      const image = ensurePreviewImage(host.element);
      this.managedImages.add(image);
      image.alt = `${GALACTIC_VEHICLES[host.vehicleClass].label} procedural 3D model`;
  ...
  private collectHosts(root: ParentNode): PreviewHost[] {
    ...
    const byClass = new Map<GalacticVehicleClass, PreviewHost>();

    for (const element of findPreviewHosts(root)) {
      const vehicleClass = resolveVehiclePreviewClass(element.dataset.vehicleId);
      if (!vehicleClass || byClass.has(vehicleClass)) continue;
      const rect = element.getBoundingClientRect();
      byClass.set(vehicleClass, {
        element,
        vehicleClass,
        size: calculateVehiclePreviewRenderSize(
          rect.width,
          rect.height,
          pixelRatio,
          this.limits,
    ...
    return GALACTIC_VEHICLE_ORDER.flatMap((vehicleClass) => {
      const host = byClass.get(vehicleClass);
      return host ? [host] : [];
    });
  }
  ...
  private ensureResources(): PreviewResources {
    ...
    const vehicles = new Map<GalacticVehicleClass, PodracerView>();
    ...
    GALACTIC_VEHICLE_ORDER.forEach((vehicleClass, index) => {
      const view = new PodracerView(createPreviewMaterials(index), index);
