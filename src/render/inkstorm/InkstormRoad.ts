...
export function createInkstormRoad(points:readonly CourseRenderPoint[], closed=true, elevated=false, gulfUniforms:CourseGulfUniforms=createCourseGulfUniforms(), biome:RacingBiome=RACING_BIOMES.desert):Mesh {
  ...
  const material=new ShaderMaterial({name:'Inkstorm compacted racing surface',glslVersion:GLSL3,transparent:true,depthWrite:false,side:DoubleSide,toneMapped:false,
    ...
    fragmentShader:`precision highp float;uniform vec3 uWorldSun;uniform float uLegacyHaze;in float vProgress;in float vRoadWidth;uniform vec3 uBiomeTint;uniform float uBiomeStrength;uniform float uFrozen;uniform float uJungle;uniform float uVolcanic;uniform float uElevated;in vec2 vUv;in vec3 vWorld;out vec4 fragColor;uniform sampler2D uGroundPaint;uniform float uGroundReady;
...
${INKSTORM_RACER_SHADOW_GLSL}
      ...
      void main(){
        ...
        float light=smoothstep(-.1,.8,dot(n,normalize(uWorldSun)));
        ...
        float haze=1.-exp(-max(0.,distanceToCamera-300.)*.00045);base=mix(base,vec3(.48,.34,.47),haze*.65*uLegacyHaze);
        ...
        float sheen=pow(max(0.,dot(reflect(normalize(vWorld-cameraPosition),n),normalize(uWorldSun))),18.);
