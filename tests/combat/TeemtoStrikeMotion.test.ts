...
import { sampleTeemtoStrikeMotion, teemtoStrikeTime, teemtoSlideTime } from '../../src/render/combat/TeemtoStrikeMotion';
...
describe('principal strike motion', () => {
  it.each([0, 1, 2])('mass %i has exact birth, one-way travel, an impact speed loss and a terminal rest', index => {
    ...
    const contact = teemtoStrikeTime(index), end = contact + teemtoSlideTime(index);
