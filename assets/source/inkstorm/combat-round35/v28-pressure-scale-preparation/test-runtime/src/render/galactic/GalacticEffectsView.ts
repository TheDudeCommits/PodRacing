...
import { RUPTURE_ATLAS_GLSL, RUPTURE_FLAME_ATLAS, RuptureFlameAtlas, ruptureArtworkScale, type RuptureAtlasOptions } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v28-pressure-scale-preparation/test-runtime/src/render/galactic/RuptureFlameAtlas.ts';
import type { WreckGroundFootprint } from '/Users/amir/Projects/PodRacing/src/render/combat/WreckGroundContact';
...
function createAtmosphericMaterial(atlas: RuptureFlameAtlas): MeshBasicMaterial {
  ...
  result.onBeforeCompile = (shader) => {
    ...
    `).replace('#include <color_fragment>', `
      ...
      float alpha;
      if (uRuptureAtlasReady > 0.5 && vEffectSurface.x > 5.5 && vEffectSurface.x < 7.5) {
  ...
  result.customProgramCacheKey = () => 'painterly-atmosphere-rooted-atlas-v9';
  return result;
}
...
export class GalacticEffectsView extends Group {
  ...
  private readonly debrisSlots = Array.from(
    ...
    (): DebrisSlot => ({ active: false, startTime: 0, duration: 0, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, rx: 0, ry: 0, rz: 0, ax: 0, ay: 0, az: 0, scale: 1, groundY: 0, metal: false, spark: false, ember: false, contactSpark: false, authoredEjection: false, emberPhase: 0, wreckOwner: -1, wreckSequence: -1, r: 1, g: 0.3, b: 0.1 }),
  );
  ...
  private updateCrash(time: number): void {
    ...
    for (const slot of this.debrisSlots) {
      ...
      if (slot.authoredEjection && slot.metal) {
