...
def cloth_shell(m,x,y,w,d,height,color,kind,seed=0):
    ...
    part(m,verts,faces,color)
    part(m,verts,[tuple(layer+i for i in reversed(face)) for face in faces],'shadow')
...
def export_overlay(scene,name,data):
    ...
    mesh=bpy.data.meshes.new(name+'-grid-construction-r29-overlay')
    ...
    material=bpy.data.materials.new(name+'-grid-r29-paint')
    ...
    obj=bpy.data.objects.new(name+'-grid-r29-overlay',mesh);scene.collection.objects.link(obj)
...
scene=bpy.data.scenes.new('Inkstorm Grid Construction Round29')
try:
