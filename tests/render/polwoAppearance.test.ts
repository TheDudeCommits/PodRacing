...
describe('Polwo appearance integration', () => {
  ...
  it('retains the authored cowl and places coupling, flames, wake and dust at measured model-root anchors', async () => {
    ...
    expect(nozzles.visible).toBe(false); expect(flames).toHaveLength(2);
    for (const [index, key] of ['exhaustLeft', 'exhaustRight'].entries()) {
      const anchor = racer.imported.getAttachment(key)!;
      // Original source rear fan slab; the earlier full-engine bound selected
      // the aft spike tip instead and visibly detached the adapter hardware.
      expect(anchor.position.z).toBeGreaterThan(8.10173);
      expect(anchor.position.z).toBeLessThan(8.16957);
