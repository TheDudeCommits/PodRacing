...
@font-face { font-family:Inkstorm Plate; src:url('/fonts/inkstorm/RussoOne-Regular.ttf') format('truetype'); font-style:normal; font-weight:400; font-display:swap; }
...
.pod-hud { --pod-paper:#f5d58e; --pod-sand:#f5d58e; --pod-orange:#ef9d67; --pod-sky:#a5e0d4; --pod-green:#bce5b9; --pod-panel:#1e393ee8; --ui-paper:#e8e7dc; --ui-ink:#1a292d; --ui-teal:#28444a; --ui-rust:#b54a36; --ui-heading:Inkstorm Plate,"Arial Black",sans-serif; --ui-settle:cubic-bezier(.22,.68,.21,1); font-family:Inkstorm UI,sans-serif; }
...
.pod-hud__workshop.is-visible { animation:none; }
...
.pod-hud.has-wreck-state>.pod-hud__driving-feedback,
.pod-hud.has-wreck-cue>.pod-hud__driving-feedback,
.pod-hud.is-paused>.pod-hud__driving-feedback,
...
.pod-hud.has-wreck-state>.pod-hud__galactic-alert { left:50%; right:auto; top:auto; bottom:max(calc(7% + 12px),env(safe-area-inset-bottom,0px)); width:min(280px,calc(100vw - 40px)); max-width:none; min-height:65px; height:auto; padding:10px 14px; transform:translateX(-50%); border:0; border-radius:3px; background:#152e36dc; text-align:center; }
...
.pod-hud__driving-instruments .pod-hud__speed-number { font-size:48px; text-shadow:none; }
...
.pod-hud__workshop-toggle:disabled + .pod-hud__start-button { grid-column:1/-1; }
.pod-hud__vehicle-select-footer>.pod-hud__selector-setup { min-height:0; }
.pod-hud.has-fixed-event .pod-hud__selector-setup { grid-template-rows:auto auto; }
.pod-hud__garage-name { bottom:74px; }
...
.pod-hud__launch[data-stage=result] { top:153px; }
.pod-hud__workshop-effective>strong>small { display:block; margin-top:5px; font:700 10px/1.4 Inkstorm UI,sans-serif; letter-spacing:.035em; color:#c7d5c7; }
.pod-hud__driving-instruments .pod-hud__telemetry>.pod-hud__meter { border:1px solid #f5d58e66; border-radius:13px 2px 13px 2px; }
.pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--boost { border:1px solid #f5d58e66; border-radius:2px 16px 16px 2px; }
@media (min-width:761px) and (min-height:541px) {
 .pod-hud__driving-instruments .pod-hud__meter-head .pod-hud__label { font-size:14px; }
 .pod-hud__driving-instruments .pod-hud__meter-value { font-size:15px; }
 .pod-hud__system-label { font-size:13px; }
 .pod-hud__vehicle-select-footer>.pod-hud__selector-setup { min-height:0; }
}
@media (max-width:760px) { .pod-hud__garage-name { bottom:74px; } }

/* Broad equipment lettering carries the menus; stencil numerals remain instruments. */
.pod-hud__vehicle-select-head h1 { font:400 clamp(32px,3.35vw,50px)/1 var(--ui-heading); letter-spacing:-.015em; }
.pod-hud__garage-bay,.pod-hud__mastery>.pod-hud__eyebrow,.pod-hud__vehicle-cards::before { font:400 14px/1.3 var(--ui-heading); letter-spacing:.015em; }
.pod-hud__garage-name h2 { font:400 clamp(30px,3.1vw,44px)/1.08 var(--ui-heading); letter-spacing:0; }
.pod-hud__garage-name p { font-size:15px; line-height:1.45; }
.pod-hud__mastery h2 { font:400 clamp(24px,2vw,29px)/1.15 var(--ui-heading); letter-spacing:0; overflow-wrap:normal; }
.pod-hud__garage-nav button,.pod-hud__start-button { font-family:var(--ui-heading); font-weight:400; letter-spacing:.015em; }
.pod-hud__start-button { font-size:19px; }
.pod-hud__garage-hero,.pod-hud__mastery { border-radius:12px; }
/* Handling choices precede the explicitly cosmetic body-style group. */
.pod-hud__vehicle-choice { grid-column:1; min-width:0; display:grid; gap:12px; align-content:start; }
.pod-hud__vehicle-choice>.pod-hud__vehicle-cards { order:0; }
.pod-hud__vehicle-choice>.pod-hud__appearance { position:static; inset:auto; margin:0; padding:10px 0 0; border-top:1px solid #24424940; opacity:1; color:var(--ui-ink); }
.pod-hud__vehicle-choice>.pod-hud__appearance[hidden] { display:none; }
.pod-hud__vehicle-choice .pod-hud__appearance span,.pod-hud__vehicle-choice .pod-hud__appearance small,.pod-hud__vehicle-choice .pod-hud__appearance p { color:#365159; }
.pod-hud__vehicle-choice .pod-hud__appearance button { color:var(--ui-ink); min-height:44px; background:transparent; }
.pod-hud__vehicle-choice .pod-hud__appearance button[aria-pressed=true] { color:var(--ui-paper); background:var(--ui-teal); }
.pod-hud__vehicle-choice .pod-hud__appearance button:not([aria-pressed=true]) { border-bottom-color:#24424955; }
.pod-hud__vehicle-choice .pod-hud__appearance p { margin-top:5px; font-size:12px; line-height:1.4; }
.pod-hud__vehicle-choice .pod-hud__vehicle-card { min-height:88px; height:auto; padding:12px 78px 12px 12px; overflow:visible; }
.pod-hud__vehicle-choice .pod-hud__vehicle-card:not(.is-selected) { background:transparent; border-color:#24424940; }
.pod-hud__vehicle-choice .pod-hud__vehicle-card h2 { margin-top:6px; max-width:none; font:400 15px/1.2 var(--ui-heading); overflow-wrap:anywhere; }
.pod-hud__vehicle-choice .pod-hud__vehicle-card .pod-hud__vehicle-preview { width:60px; height:60px; right:8px; bottom:10px; }
.pod-hud__vehicle-choice .pod-hud__vehicle-card-check { right:8px; top:6px; font-size:8px; }
.pod-hud__garage-hero .pod-hud__garage-model { top:46px; }
.pod-hud__garage-inspect button { min-width:44px; min-height:44px; }
.pod-hud__mastery .pod-hud__event-list { flex:0 0 auto; height:220px; min-height:0; max-height:none; scrollbar-gutter:stable; scroll-snap-type:y proximity; }
.pod-hud__mastery .pod-hud__event-list button { min-height:44px; scroll-snap-align:start; }
.pod-hud__mastery .pod-hud__event-list button strong { font:700 14px/1.3 Inkstorm UI,sans-serif; }
.pod-hud__mastery-controls button { min-height:44px; }
@media (max-width:760px) {
 .pod-hud__vehicle-choice { grid-row:3; }
 .pod-hud__mastery { grid-row:4; }
 .pod-hud__vehicle-select-footer { grid-row:5; }
 .pod-hud__vehicle-choice .pod-hud__vehicle-card { padding-right:70px; }
 .pod-hud__vehicle-choice .pod-hud__vehicle-card h2 { font-size:14px; }
 .pod-hud__vehicle-choice .pod-hud__vehicle-card .pod-hud__vehicle-preview { width:52px; height:52px; }
}
@media (max-width:650px) {
 .pod-hud__vehicle-choice { order:2; }
 .pod-hud__mastery { order:3; }
 .pod-hud__vehicle-select-footer { order:4; }
 .pod-hud__garage-hero .pod-hud__garage-model { top:auto; }
 .pod-hud__garage-hero .pod-hud__garage-name { inset:auto; max-width:none; }
}
.pod-hud__appearance button { border-color:transparent; min-height:36px; padding:6px 10px; }
.pod-hud__appearance button:not([aria-pressed=true]) { background:transparent; border-bottom:1px solid #c8d5c655; }
.pod-hud__appearance button,.pod-hud__vehicle-card,.pod-hud__event-list button { transition:background-color 220ms var(--ui-settle),color 220ms var(--ui-settle),box-shadow 220ms var(--ui-settle); }
.pod-hud__garage-stats { gap:16px; }
.pod-hud__garage-stats span { font-size:13px; }
.pod-hud__garage-stats b { font:400 18px/1 var(--ui-heading); }
.pod-hud__garage-stats i { background:repeating-linear-gradient(90deg,transparent 0 calc(20% - 4px),#19323a calc(20% - 4px) 20%),linear-gradient(90deg,#d4dbbc var(--rating),#e4ead921 var(--rating)); }
.pod-hud__event-heading { font-size:13px; }
.pod-hud__vehicle-card { height:auto; min-height:112px; }
.pod-hud__vehicle-card h2 { font:400 16px/1.2 var(--ui-heading); letter-spacing:0; overflow-wrap:normal; max-width:72%; }
.pod-hud__event-list button strong { font-size:17px; line-height:1.25; }
.pod-hud__personal-record>span { font-size:12px; }
.pod-hud__vehicle-select.is-visible .pod-hud__mastery { animation:instrument-enter 340ms 50ms var(--ui-settle) both; }
.pod-hud__vehicle-select.is-visible .pod-hud__vehicle-choice { animation:instrument-enter 340ms 90ms var(--ui-settle) both; }

/* Open instrument strokes carry speed and the three actual resource readings. */
.pod-hud__driving-instruments { width:244px; height:264px; background:transparent; border:0; border-radius:0; box-shadow:none; filter:none; }
.pod-hud__driving-instruments>.pod-hud__redline-heat { top:22px; left:62px; right:22px; width:auto; min-height:0; height:17px; padding:0; transform:none; display:flex; align-items:center; gap:8px; box-shadow:none; border:0; background:none; z-index:2; }
.pod-hud__driving-instruments>.pod-hud__redline-heat>span { flex:0 0 auto; font:700 11px/1.2 Inkstorm UI,sans-serif; letter-spacing:0; }
.pod-hud__driving-instruments>.pod-hud__redline-heat::after { content:none; }
.pod-hud__driving-instruments>.pod-hud__redline-heat .pod-hud__redline-track { flex:1; min-width:22px; height:4px; border:0; transform:none; }
.pod-hud__driving-instruments>.pod-hud__speed { inset:16px 12px auto; height:245px; z-index:1; }
.pod-hud__driving-instruments .pod-hud__speed-ring { width:216px; height:245px; }
/* The speed loop and return arc end above the independent resource rows. */
.pod-hud__driving-instruments .pod-hud__speed-dial { stroke-width:1.6; filter:none; }
.pod-hud__driving-instruments .pod-hud__speed-dial .dial-glass { fill:transparent; stroke:none; }
.pod-hud__driving-instruments .pod-hud__speed-dial .dial-outline { fill:#093b4c14; stroke:#ffe39b; stroke-width:1.6; }
.pod-hud__driving-instruments .pod-hud__speed-dial .dial-track { stroke:#ffe39bc9; stroke-width:1.6; }
.pod-hud__driving-instruments .pod-hud__speed-dial .dial-charge { stroke-width:3; }
.pod-hud__driving-instruments .pod-hud__speed-dial .dial-key-socket { fill:#093b4c; stroke:#ffe39b; }
.pod-hud__driving-instruments .pod-hud__speed-dial .dial-heat-edge { fill:none; stroke:transparent; stroke-width:4; }
.pod-hud__driving-instruments:has(>.pod-hud__telemetry.is-hot) .dial-heat-edge { stroke:#ffb794; }
.pod-hud__driving-instruments .pod-hud__speed-readout { top:49px; left:31px; width:147px; text-shadow:none; }
.pod-hud__driving-instruments .pod-hud__speed-number { font-size:48px; }
.pod-hud__driving-instruments .pod-hud__speed-unit { font:700 11px/1 Inkstorm UI,sans-serif; }
/* Tight glyph-edge protection leaves the measured text and bar layout open. */
.pod-hud__driving-instruments .pod-hud__speed-number,.pod-hud__driving-instruments .pod-hud__speed-unit,.pod-hud__driving-instruments .pod-hud__meter-head .pod-hud__label,.pod-hud__driving-instruments .pod-hud__meter-value { background:none; border-radius:0; box-shadow:none; text-shadow:-1px 0 0 var(--pod-readout-ink,#073b4b),1px 0 0 var(--pod-readout-ink,#073b4b),0 -1px 0 var(--pod-readout-ink,#073b4b),0 1px 0 var(--pod-readout-ink,#073b4b); }
.pod-hud.is-high-contrast .pod-hud__driving-instruments { --pod-readout-ink:#0b2026; }

.pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--boost { top:130px; bottom:auto; left:8px; right:26px; width:auto; padding:0; background:none; border:0; border-radius:0; }
.pod-hud__driving-instruments>.pod-hud__telemetry { left:20px; right:38px; bottom:32px; height:auto; display:grid; grid-template-columns:minmax(0,1fr); gap:8px; }
.pod-hud__driving-instruments .pod-hud__telemetry>.pod-hud__meter { width:auto; min-height:22px; padding:0; background:none; border:0; border-radius:0; box-shadow:none; }
.pod-hud__driving-instruments .pod-hud__meter--boost,.pod-hud__driving-instruments .pod-hud__telemetry>.pod-hud__meter { display:grid; grid-template-columns:50px minmax(0,1fr) 42px; align-items:center; column-gap:7px; }
.pod-hud__driving-instruments .pod-hud__meter-head { display:contents; text-shadow:none; }
.pod-hud__driving-instruments .pod-hud__meter-head .pod-hud__label { grid-column:1; grid-row:1; margin:0; font:700 13px/1.25 Inkstorm UI,sans-serif; letter-spacing:0; }
.pod-hud__driving-instruments .pod-hud__meter-value { grid-column:3; grid-row:1; font:400 16px/1.15 var(--ui-heading); letter-spacing:0; text-align:right; }
.pod-hud__driving-instruments .pod-hud__meter-value::after { font-size:.7em; }
.pod-hud__driving-instruments .pod-hud__meter-track,.pod-hud__driving-instruments .pod-hud__telemetry .pod-hud__meter-track { grid-column:2; grid-row:1; height:7px; margin:0; background:#073b4bdf; border:0; border-radius:999px; box-shadow:0 0 0 1px #ffe39b59; transform:none; }
.pod-hud__driving-instruments .pod-hud__telemetry .pod-hud__meter-fill { display:block; width:var(--pod-meter-value,0%)!important; height:100%; }
.pod-hud__driving-instruments .pod-hud__telemetry.is-hot .pod-hud__meter--heat { background:none; box-shadow:none; outline:0; }
.pod-hud__driving-instruments .pod-hud__telemetry.is-hot .pod-hud__meter--heat .pod-hud__label { color:#ffb794; }
.pod-hud__driving-instruments .pod-hud__telemetry.is-damaged .pod-hud__meter--damage { background:none; box-shadow:none; outline:0; }
.pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--drift { top:-58px; padding:7px 10px; border-radius:5px; background:#112e36ed; }
.pod-hud__driving-instruments .pod-hud__meter--drift .pod-hud__meter-head { display:flex; }
.pod-hud__flight { bottom:300px; width:244px; }
.pod-hud__race-stat .pod-hud__label,.pod-hud__race-time>.pod-hud__label { font:400 12px/1.25 var(--ui-heading); letter-spacing:.035em; }

/* Brief guidance keeps its backing over sky, dust, and sunlit rock. */
.pod-hud__detail-toggle { border:0; background:#112e36eb; border-radius:4px; padding:7px 10px; font:700 13px/1.2 Inkstorm UI,sans-serif; letter-spacing:.04em; text-shadow:none; }
.pod-hud__corner { background:#112e36e6; border:0; border-radius:4px; text-shadow:none; padding:8px 10px; }
.pod-hud__corner-distance { font:400 16px/1.15 var(--ui-heading); }
.pod-hud__corner-tag { font-size:13px; line-height:1.2; }
.pod-hud__launch[data-stage=result] { background:#112e36eb; border:0; border-left:3px solid #ef9d67; border-radius:4px; padding:10px 12px; }
.pod-hud__launch[data-stage=result] header>strong { font:400 19px/1.15 var(--ui-heading); }
.pod-hud__launch[data-stage=result] footer { color:#ffe8ac; text-shadow:none; }
.pod-hud__combat-slot { background:#112e36c9; border-color:#f5d58e99; border-radius:20px 5px 20px 5px; box-shadow:none; }
.pod-hud__combat-key { background:#112e36; border:0; font-size:13px; }
.pod-hud__system-label { font:400 11px/1.2 var(--ui-heading); text-shadow:none; }
.pod-hud__threat-cue { background:#112e36e8; box-shadow:none; border:0; border-left:2px solid #ef9d67; border-radius:3px; text-shadow:none; }
.pod-hud__threat-cue span { background:none; font:700 12px/1.3 Inkstorm UI,sans-serif; text-shadow:none; }
.pod-hud__combat-feedback>strong { font:400 17px/1.2 var(--ui-heading); letter-spacing:.015em; }
.pod-hud__combat-feedback[data-kind=takedown]>strong,.pod-hud__combat-feedback[data-kind=wreck]>strong,.pod-hud.has-wreck-state>.pod-hud__galactic-alert>strong { font:400 24px/1.15 var(--ui-heading); letter-spacing:.015em; }
/* The bounded result cue retains its footprint while backing follows the words. */
.pod-hud__combat-feedback[data-kind=takedown],.pod-hud__combat-feedback[data-kind=wreck],.pod-hud.has-wreck-state>.pod-hud__galactic-alert { background:none; grid-template-columns:minmax(0,1fr); gap:3px; }
.pod-hud.has-wreck-state>.pod-hud__galactic-alert { display:grid; }
.pod-hud__combat-feedback[data-kind=takedown]>strong,.pod-hud__combat-feedback[data-kind=wreck]>strong,.pod-hud.has-wreck-state>.pod-hud__galactic-alert>strong { display:block; width:fit-content; max-width:100%; margin:0 auto; padding:0 5px; border-radius:2px; background:#093b4cd9; }
.pod-hud__combat-feedback[data-kind=takedown]>span,.pod-hud__combat-feedback[data-kind=wreck]>span,.pod-hud.has-wreck-state>.pod-hud__galactic-alert>span { display:block; box-sizing:border-box; width:fit-content; max-width:100%; margin:0 auto; padding:2px 6px; border-radius:2px; background:#093b4ceb; color:#fff0b3; font:700 13px/1.4 Inkstorm UI,sans-serif; letter-spacing:.02em; }
.pod-hud.is-high-contrast .pod-hud__combat-feedback[data-kind=takedown]>strong,.pod-hud.is-high-contrast .pod-hud__combat-feedback[data-kind=takedown]>span,.pod-hud.is-high-contrast .pod-hud__combat-feedback[data-kind=wreck]>strong,.pod-hud.is-high-contrast .pod-hud__combat-feedback[data-kind=wreck]>span,.pod-hud.is-high-contrast.has-wreck-state>.pod-hud__galactic-alert>strong,.pod-hud.is-high-contrast.has-wreck-state>.pod-hud__galactic-alert>span { background:#0b2026; }

@media (min-width:1280px) {
 .pod-hud__vehicle-select-frame { grid-template-columns:minmax(0,1fr) 312px; }
}
@media (max-width:760px), (max-height:540px) {
 .pod-hud__garage-bay,.pod-hud__mastery>.pod-hud__eyebrow,.pod-hud__vehicle-cards::before { font-size:12px; }
 .pod-hud__garage-stats { gap:8px; }
 .pod-hud__garage-stats span { font-size:10px; }
 .pod-hud__garage-stats b { font-size:14px; }
 .pod-hud__vehicle-card h2 { font-size:14px; }
 .pod-hud__vehicle-card { height:auto; min-height:116px; }
 .pod-hud__detail-toggle { font-size:11px; padding:6px 8px; }
 .pod-hud__corner-distance { font-size:14px; }
 .pod-hud__corner-tag { font-size:11px; }
 .pod-hud__driving-instruments { width:180px; height:224px; }
 .pod-hud__driving-instruments>.pod-hud__redline-heat { top:22px; left:48px; right:13px; gap:5px; }
 .pod-hud__driving-instruments>.pod-hud__redline-heat>span { font-size:9px; }
 .pod-hud__driving-instruments>.pod-hud__redline-heat .pod-hud__redline-track { min-width:16px; }
 .pod-hud__driving-instruments>.pod-hud__speed { inset:18px 9px auto; height:182px; }
 .pod-hud__driving-instruments .pod-hud__speed-ring { width:160px; height:182px; }
 .pod-hud__driving-instruments .pod-hud__speed-readout { left:22px; top:37px; width:111px; }
 .pod-hud__driving-instruments .pod-hud__speed-number { font-size:35px; }
 .pod-hud__driving-instruments .pod-hud__speed-unit { font-size:9px; }
 .pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--boost { top:103px; left:5px; right:13px; }
 .pod-hud__driving-instruments>.pod-hud__telemetry { left:14px; right:22px; bottom:34px; gap:6px; }
 .pod-hud__driving-instruments .pod-hud__telemetry>.pod-hud__meter { min-height:18px; }
 .pod-hud__driving-instruments .pod-hud__meter--boost,.pod-hud__driving-instruments .pod-hud__telemetry>.pod-hud__meter { grid-template-columns:39px minmax(0,1fr) 32px; column-gap:5px; }
 .pod-hud__driving-instruments .pod-hud__meter-head .pod-hud__label { font-size:10px; }
 .pod-hud__driving-instruments .pod-hud__meter-value { font:700 13px/1.15 Inkstorm UI,sans-serif; }
 .pod-hud__driving-instruments .pod-hud__meter-track,.pod-hud__driving-instruments .pod-hud__telemetry .pod-hud__meter-track { height:6px; }
 .pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--drift { top:-58px; }
 .pod-hud__flight { width:180px; bottom:254px; }
 .pod-hud__race-stat .pod-hud__label,.pod-hud__race-time>.pod-hud__label { font-size:10px; }
 .pod-hud__system-label { font:700 10px/1.2 Inkstorm UI,sans-serif; }
}
@media (max-width:760px) {
 .pod-hud__vehicle-select-head h1 { font-size:34px; }
 .pod-hud__garage-name { bottom:74px; }
 .pod-hud__garage-name h2 { font-size:29px; }
 .pod-hud__garage-name p { font-size:13px; }
 .pod-hud__garage-stats { grid-template-columns:repeat(3,minmax(0,1fr)); row-gap:10px; }
 .pod-hud__mastery h2 { font-size:26px; }
 .pod-hud__vehicle-card h2 { max-width:100%; }
}
@media (max-height:540px) and (min-width:651px) {
 .pod-hud__driving-instruments { width:174px; height:189px; }
 .pod-hud__driving-instruments>.pod-hud__redline-heat { top:17px; left:46px; right:11px; }
 .pod-hud__driving-instruments>.pod-hud__speed { top:13px; height:175px; }
 .pod-hud__driving-instruments .pod-hud__speed-ring { width:154px; height:175px; }
 .pod-hud__driving-instruments .pod-hud__speed-readout { top:34px; }
 .pod-hud__driving-instruments .pod-hud__speed-number { font-size:30px; }
 .pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--boost { top:99px; }
 .pod-hud__driving-instruments>.pod-hud__telemetry { bottom:9px; gap:4px; }
 .pod-hud__driving-instruments .pod-hud__meter-value { font-size:12px; }
 .pod-hud__driving-instruments .pod-hud__speed>.pod-hud__meter--drift { top:-52px; }
 .pod-hud__flight { top:153px; bottom:auto; }
}
/* Filled timing numerals share one shallow transparent instrument glass. */
.pod-hud__race { top:18px; height:76px; }
.pod-hud__race::before { content:none; }
.pod-hud__race-rail { display:block; inset:0; height:100%; pointer-events:none; }
.pod-hud__race-rail svg { display:block; overflow:hidden; stroke:#ffe39bc9; stroke-width:1.3; filter:none; }
.pod-hud__race-rail .race-glass { fill:#063e57a6; stroke:none; }
.pod-hud.is-high-contrast .pod-hud__race-rail .race-glass { fill:#0b2026; }
.pod-hud__race-stat,.pod-hud__race-time { top:2px; background:none; border:0; padding-bottom:0; }
.pod-hud__race-time { width:220px; }
.pod-hud__race-value,.pod-hud__clock { color:#ffe39b; font:400 40px/1 Inkstorm Instrument,sans-serif; letter-spacing:-.015em; -webkit-text-stroke:0; text-shadow:none; }
.pod-hud__race-value small { color:inherit; font:inherit; }
.pod-hud__race-stat .pod-hud__label,.pod-hud__race-time>.pod-hud__label { display:block; align-self:center; padding:0; border-radius:0; background:none; color:#ffe39b; font:700 13px/1.25 Inkstorm UI,sans-serif; letter-spacing:.075em; text-shadow:none; }
.pod-hud__race>.pod-hud__detail-toggle { position:absolute; z-index:1; left:12px; right:auto; top:10px; min-width:76px; min-height:44px; padding:8px; background:none; border:0; border-radius:0; font-size:12px; }
.pod-hud__detail-toggle:focus-visible { outline:2px solid #ffe39b; outline-offset:2px; }
.pod-hud__corner { top:107px; padding:6px 9px; background:#173c438f; border-bottom:1px solid #f5d58e88; border-radius:0; }
.pod-hud__launch[data-stage=result] { top:107px; padding:7px 10px; border:0; border-bottom:1px solid #f5d58e88; border-radius:0; background:#173c439e; box-shadow:none; }
.pod-hud__launch[data-stage=result] header>strong { font-size:17px; }
.pod-hud__launch[data-stage=result] footer { margin-top:4px; font:700 12px/1.3 Inkstorm UI,sans-serif; }
.pod-hud__driving-instruments>.pod-hud__redline-heat>span { padding:2px 4px; border-radius:2px; background:#112e36e6; color:#ffe39b; text-shadow:none; }
.pod-hud__driving-instruments .pod-hud__meter-head { padding:0; border-radius:0; background:none; }
/* Open mechanical frames share the speed instrument's cream linework.
   The independent inner arc keeps the actual ability readiness visible. */
.pod-hud__combat-slot { background:none; border:0; border-radius:0; box-shadow:none; overflow:visible; }
.pod-hud__combat-key { top:0; left:0; width:22px; height:22px; padding:0; border:1px solid #ffe39bc9; border-radius:3px; background:#112e36b3; box-shadow:none; color:#ffe39b; font:700 13px/1 Inkstorm UI,sans-serif; text-shadow:0 1px 0 #112e36; transform:none; }
.pod-hud__system-gauge { z-index:1; left:50%; top:5px; width:48px; height:48px; background:#073b4b1a; box-shadow:inset 0 0 0 1.6px #ffe39b,0 1px 0 #112e36; transform:translateX(-50%) rotate(-35deg); }
.pod-hud__system-gauge::before { inset:3px; background:conic-gradient(var(--pod-system-accent) 0 var(--pod-readiness-angle),#ffe39b22 var(--pod-readiness-angle) 360deg); mask:radial-gradient(farthest-side,transparent calc(100% - 2px),#000 calc(100% - 1.9px)); box-shadow:none; }
.pod-hud__system-icon { inset:10px; filter:drop-shadow(0 1px 0 #112e36); }
.pod-hud__combat-slot--weapon .pod-hud__system-icon::before { left:0; top:30%; width:80%; height:12%; background:#ffe39b; box-shadow:0 6px 0 #ffe39b; }
.pod-hud__combat-slot--weapon .pod-hud__system-icon::after { left:73%; top:19%; width:24%; height:39%; background:#ffe39b; }
.pod-hud__combat-slot--shield .pod-hud__system-icon { border-color:#ffe39b; }
.pod-hud__combat-slot--mine .pod-hud__system-icon { background:#ffe39b; }
.pod-hud__combat-slot--mine .pod-hud__system-icon::after { background:#112e36; }
.pod-hud__combat-slot::after { content:""; position:absolute; left:12%; right:12%; top:43px; bottom:6px; z-index:0; background:linear-gradient(#ffe39b66,#ffe39b66) center/1px 100% no-repeat; border-bottom:1px solid #ffe39b99; border-radius:0; clip-path:none; pointer-events:none; }
.pod-hud__system-label { left:50%; right:auto; bottom:6px; width:max-content; max-width:100%; padding:2px 4px; border-radius:0; background:linear-gradient(90deg,transparent,#112e36b3 14% 86%,transparent); overflow:visible; text-overflow:clip; color:#ffe39b; font:700 12px/1.2 Inkstorm UI,sans-serif; text-shadow:0 1px 0 #112e36; transform:translateX(-50%); }
.pod-hud__system-ammo { z-index:2; right:0; bottom:30px; color:#ffe39b; font:700 13px/1 Inkstorm UI,sans-serif; text-shadow:0 1px 0 #112e36,-1px 0 0 #112e36,1px 0 0 #112e36; }
.pod-hud__galactic.is-shield-active .pod-hud__combat-slot--shield { background:none; }
.pod-hud__galactic.is-shield-active .pod-hud__combat-slot--shield .pod-hud__system-gauge { box-shadow:inset 0 0 0 1.6px #ffe39b,0 0 0 1px var(--pod-system-accent); }
.pod-hud__galactic.is-shield-active .pod-hud__combat-slot--shield .pod-hud__system-icon { border-color:var(--pod-system-accent); }
.pod-hud.is-high-contrast .pod-hud__race { background:none; }
.pod-hud.is-high-contrast .pod-hud__race::before { background:#0b2026; }
.pod-hud.is-high-contrast .pod-hud__race-stat,.pod-hud.is-high-contrast .pod-hud__race-time { background:none; }
.pod-hud.is-high-contrast .pod-hud__speed-dial .dial-glass,.pod-hud.is-high-contrast .pod-hud__speed-dial .dial-key-socket { fill:#0b2026; }
.pod-hud.is-high-contrast .pod-hud__combat-slot { background:none; }
.pod-hud.is-high-contrast .pod-hud__system-gauge,.pod-hud.is-high-contrast .pod-hud__combat-slot::after,.pod-hud.is-high-contrast .pod-hud__system-label,.pod-hud.is-high-contrast .pod-hud__combat-key { background:#0b2026; }
@media (max-width:760px) {
 .pod-hud__race { top:12px; height:66px; }
 .pod-hud__race-value,.pod-hud__clock { font-size:28px; }
 .pod-hud__race-time { width:138px; }
 .pod-hud__race-stat .pod-hud__label,.pod-hud__race-time>.pod-hud__label { font-size:10px; }
 .pod-hud__race>.pod-hud__detail-toggle { position:fixed; top:84px; left:12px; background:#173c438f; border-bottom:1px solid #f5d58e88; }
 .pod-hud__corner { top:84px; right:12px; min-height:44px; }
 .pod-hud__launch[data-stage=result] { top:140px; left:15px; }
 .pod-hud__system-gauge { width:40px; height:40px; top:5px; }
 .pod-hud__system-icon { inset:7px; }
 .pod-hud__combat-slot::after { top:36px; bottom:5px; }
 .pod-hud__combat-key { top:0; width:18px; height:20px; font-size:11px; }
 .pod-hud__system-label { bottom:5px; font-size:10px; }
 .pod-hud__system-ammo { bottom:26px; font-size:11px; }
}
@media (max-height:540px) and (min-width:651px) {
 .pod-hud__race { top:10px; height:61px; }
 .pod-hud__race-value,.pod-hud__clock { font-size:30px; }
 .pod-hud__race-stat .pod-hud__label,.pod-hud__race-time>.pod-hud__label { font-size:10px; }
 .pod-hud__race>.pod-hud__detail-toggle { position:absolute; top:2px; background:none; border-bottom:0; }
 .pod-hud__corner { top:80px; }
 .pod-hud__launch[data-stage=result] { top:80px; }
 .pod-hud__system-gauge { width:40px; height:40px; top:4px; }
 .pod-hud__system-icon { inset:7px; }
 .pod-hud__combat-slot::after { top:36px; bottom:5px; }
 .pod-hud__system-label { bottom:5px; font-size:10px; }
}
/* One occupied status column shares a backing and spine. Charging stays screen anchored. */
.pod-hud__driving-feedback { position:absolute; right:64px; bottom:294px; z-index:6; width:244px; display:flex; flex-direction:column; gap:3px; pointer-events:none; background:none; box-shadow:none; }
.pod-hud__driving-feedback>.pod-hud__launch:not([data-stage=result]) { position:fixed; }
.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] { position:static; inset:auto; order:3; width:100%; display:none; padding:5px 9px; transform:none; background:linear-gradient(90deg,#073b4bdf,#073b4b66 80%,transparent); border:0; border-left:2px solid #ef9d67; clip-path:none; text-align:left; }
.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result].is-visible { display:block; }
.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] header { margin:0; }
.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] header>strong { font:400 17px/1.2 var(--ui-heading); }
.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] footer { text-align:left; margin:4px 0 0; font:700 12px/1.3 Inkstorm UI,sans-serif; }
.pod-hud__driving-feedback>.pod-hud__flight { position:static; inset:auto; order:1; display:none; width:100%; min-width:0; height:auto; padding:5px 9px; transform:none; border:0; border-left:2px solid #a5e6df; background:linear-gradient(90deg,#073b4bdf,#073b4b66 80%,transparent); }
.pod-hud__driving-feedback>.pod-hud__flight.is-visible { display:grid; grid-template-columns:auto minmax(0,1fr); grid-template-areas:"state clear" "motion motion"; gap:2px 8px; transform:none; }
.pod-hud__driving-feedback>.pod-hud__flight>.pod-hud__flight-state,.pod-hud__driving-feedback>.pod-hud__flight>small { display:block; }
.pod-hud__driving-feedback>.pod-hud__flight>.pod-hud__flight-state { font:700 12px/1.3 Inkstorm UI,sans-serif; letter-spacing:.04em; }
.pod-hud__driving-feedback>.pod-hud__flight>strong { font:700 13px/1.3 Inkstorm UI,sans-serif; }
.pod-hud__driving-feedback>.pod-hud__flight>small { margin-top:0; text-align:left; font:700 11px/1.3 Inkstorm UI,sans-serif; letter-spacing:0; }
.pod-hud__driving-feedback>.pod-hud__meter--drift { position:static; order:2; display:none; width:100%; margin:0; padding:5px 9px; border:0; border-left:2px solid #ffe39b99; background:linear-gradient(90deg,#073b4bdf,#073b4b66 80%,transparent); }
.pod-hud.has-drift-charge .pod-hud__driving-feedback>.pod-hud__meter--drift { display:block; }
...
.pod-hud[data-phase=finished]>.pod-hud__driving-feedback { visibility:hidden; opacity:0; }
.pod-hud.is-high-contrast .pod-hud__driving-feedback>.pod-hud__launch,.pod-hud.is-high-contrast .pod-hud__driving-feedback>.pod-hud__flight,.pod-hud.is-high-contrast .pod-hud__driving-feedback>.pod-hud__meter { background:#0b2026; }
.pod-hud.is-high-contrast .pod-hud__driving-feedback { background:#0b2026; box-shadow:inset 2px 0 #fffbd8; }
@media (max-width:760px) {
 .pod-hud__driving-feedback { right:50px; bottom:248px; width:180px; }
 .pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] header>strong { font-size:15px; }
 .pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] footer,.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] footer>span { font-size:11px; }
 .pod-hud__driving-feedback>.pod-hud__flight.is-visible { grid-template-columns:1fr; grid-template-areas:'state' 'clear' 'motion'; gap:0; }
 .pod-hud__driving-feedback>.pod-hud__flight>strong { justify-self:start; }
}
@media (max-height:540px) and (min-width:651px) {
 .pod-hud__race::before { bottom:10px; }
 .pod-hud__driving-feedback { right:244px; bottom:18px; width:min(190px,calc(100vw - 501px)); }
 .pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] header>strong { font-size:15px; }
 .pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] footer,.pod-hud__driving-feedback>.pod-hud__launch[data-stage=result] footer>span { font-size:11px; }
 .pod-hud__driving-feedback>.pod-hud__flight.is-visible { grid-template-columns:1fr; grid-template-areas:'state' 'clear' 'motion'; gap:0; }
 .pod-hud__driving-feedback>.pod-hud__flight>strong { justify-self:start; }
...
.pod-hud__threat-cue>span { flex:0 1 auto; min-width:0; width:auto; padding:2px 5px; border-radius:2px; background:#112e36df; font:700 11px/12px Inkstorm UI,sans-serif; letter-spacing:.025em; white-space:normal; text-overflow:clip; }
.pod-hud__threat-cue>em { flex:0 0 auto; padding:2px 4px; border-radius:2px; background:#112e36df; }
.pod-hud__threat-cue.is-direction-only>span { display:none; }
.pod-hud__threat-cue.is-primary>b { border-width:2px; }
.pod-hud__threat-cue.is-critical { animation:none; box-shadow:none; }
...
@media (prefers-reduced-motion:reduce) { .pod-hud *,.pod-hud *::before,.pod-hud *::after { animation:none!important; transition:none!important; } }
/* Hangar hierarchy: machine portrait, event itinerary, then launch. */
.pod-hud__vehicle-select .pod-hud__garage-hero { border-radius:0; border-color:transparent; }
.pod-hud__vehicle-select .pod-hud__garage-nav button { border-color:transparent; border-bottom:1px solid #24424966; background:none; }
.pod-hud__vehicle-select .pod-hud__garage-nav button:hover,.pod-hud__vehicle-select .pod-hud__garage-nav button[aria-expanded=true] { color:var(--ui-ink); background:#2442490c; border-bottom-color:var(--ui-rust); }
.pod-hud__vehicle-select .pod-hud__garage-inspect button { border-color:transparent; background:transparent; }
.pod-hud__vehicle-select .pod-hud__garage-inspect button:hover { background:#d1ddd022; }
.pod-hud__vehicle-select .pod-hud__vehicle-card:not(.is-selected),.pod-hud__vehicle-select .pod-hud__vehicle-card { border:1px solid transparent; border-bottom-color:#24424940; border-radius:0; background:transparent; color:var(--ui-ink); box-shadow:none; }
.pod-hud__vehicle-select .pod-hud__vehicle-card:hover { background:#2442490b; }
.pod-hud__vehicle-select .pod-hud__vehicle-card.is-selected { border-color:transparent; border-bottom:3px solid var(--ui-rust); background:#24424912; color:var(--ui-ink); }
.pod-hud__vehicle-select .pod-hud__vehicle-card-index { color:#456064; }
.pod-hud__vehicle-select .pod-hud__vehicle-card-check { color:var(--ui-rust); }
.pod-hud__vehicle-select .pod-hud__appearance button { background:none; border:0; border-bottom:1px solid transparent; border-radius:0; }
.pod-hud__vehicle-select .pod-hud__appearance button[aria-pressed=true] { color:var(--ui-ink); background:none; border-bottom:2px solid var(--ui-rust); }
.pod-hud__vehicle-select .pod-hud__mastery { background:transparent; color:var(--ui-ink); border:0; border-left:1px solid #24424942; border-radius:0; padding-left:22px; }
.pod-hud__vehicle-select .pod-hud__mastery h2 { color:var(--ui-ink); }
.pod-hud__vehicle-select .pod-hud__mastery>.pod-hud__eyebrow { color:var(--ui-rust); }
.pod-hud__vehicle-select .pod-hud__mastery>p { color:#365159; }
.pod-hud__vehicle-select .pod-hud__event-heading,.pod-hud__vehicle-select .pod-hud__event-heading small { color:#365159; }
.pod-hud__vehicle-select .pod-hud__event-list button { color:var(--ui-ink); border-bottom-color:#24424933; }
.pod-hud__vehicle-select .pod-hud__event-list button.is-selected { background:#2442490c; border-left-color:var(--ui-rust); }
.pod-hud__vehicle-select .pod-hud__personal-record>span,.pod-hud__vehicle-select .pod-hud__personal-record>small { color:#365159; }
.pod-hud__vehicle-select .pod-hud__personal-record>strong { color:var(--ui-teal); }
.pod-hud__vehicle-select .pod-hud__mastery-controls button { color:var(--ui-teal); }
.pod-hud__vehicle-select .pod-hud__mastery-controls button[aria-pressed=true] { color:var(--ui-teal); background:transparent; border-bottom:1px solid var(--ui-rust); padding-left:0; padding-right:0; }
