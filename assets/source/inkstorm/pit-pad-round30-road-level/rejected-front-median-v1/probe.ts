...
for (const seed of [0x494e4b53, 42, 1234]) {
  ...
  const pads = field.pads.map((pad, padIndex) => {
    ...
    return { ...pad, samples, minHeight, maxHeight, beforeMax, beforeAboveSlabBottom: beforeMax - pad.anchorHeight - pad.slabBottomOffset,
      ...
      review: { physicalCoreClear: aboveSlabBottom === 0, indexedMeshClear: !visualMesh || [visualMesh, ...meshPhaseChecks].every(m => m.maxAboveSlabBottom < -.1),
        nearRoadEntry: entryProfiles.length > 0 && entryProfiles.every(p => p.entryDeckSlope <= .08 && p.bridgeTerrainIntrusion <= .1 && p.maxBridgeGroundGap <= 2) } };
  });
