...
def cloth_shell(m,x,y,w,d,height,color,kind,seed=0):
    ...
    for j in range(ny):
        for i in range(nx):
            a=j*(nx+1)+i
            # Matching diagonals on both layers preserve exact vertical thickness
            # even when this tensioned cloth cell is a nonplanar quad.
...
def export_overlay(scene,name,data):
    ...
    mesh=bpy.data.meshes.new(name+'-grid-construction-r29-v3-overlay')
    ...
    material=bpy.data.materials.new(name+'-grid-r29-v3-paint')
    ...
    obj=bpy.data.objects.new(name+'-grid-r29-v3-overlay',mesh);scene.collection.objects.link(obj)
...
scene=bpy.data.scenes.new('Inkstorm Grid Construction Round29 v3 lower task lights')
try:
