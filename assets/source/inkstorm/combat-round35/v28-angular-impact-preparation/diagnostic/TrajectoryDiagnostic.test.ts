...
import { sampleTeemtoContactRoll, teemtoSlideTime, teemtoStrikeTime } from '/Users/amir/Projects/PodRacing/src/render/combat/TeemtoStrikeMotion';
...
import { ImportedVehiclePresentation } from '/Users/amir/Projects/PodRacing/src/render/vehicles/ImportedVehiclePresentation';
import { VehicleArtLibrary } from '/Users/amir/Projects/PodRacing/src/render/vehicles/VehicleArtLibrary';
import { TEEMTO_ART_DEFINITIONS } from '/Users/amir/Projects/PodRacing/src/game/vehicleAppearance';
...
const criticalAges = [...new Set([0, .05, ...starts.flatMap(t => [t - 1e-6, t, t + 1e-6]),
  ...peaks, ...rebounds, ...ends, .25, .35, .45, .55, .65, .9, 1.8, 1.96, 2.15])].sort((a, b) => a - b);
const unit = new Vector3(1, 1, 1);
const measurements: unknown[] = [];
afterAll(async () => {
  const path = (globalThis as unknown as { process?: { env: Record<string, string | undefined> } }).process?.env.V27_SUPPORT_METRICS_PATH;
  if (path) {
...
describe('support-pivot impact and settling', () => {
  ...
  it.each(variants)('%s keeps complete actual geometry above terrain at contact and angular extrema within the existing query budget', name => {
    ...
    const sample = (age: number, checkSource = true) => {
      ...
      maxQueries = Math.max(maxQueries, queries); expect(queries).toBeLessThanOrEqual(name === 'hero' ? 348 : 300);
    ...
    try {
      ...
      measurements.push({ name, ages: criticalAges, positions, minimum, maxQueries, postContactAngularRadians: angularResponses });
  ...
  it.each(variants)('%s retains one-way capped world-center travel while rotating about support', name => {
    ...
    try {
      ...
      const travel = [3.2, 3.8, 5]; let worstStep = Infinity; const rows: unknown[] = [];
      for (let tick = 1; tick <= 120; tick++) {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          rows.push({age,index:i,advance,displacement:points[i]!.distanceTo(birth[i]!)});
          
          previous[i]!.copy(points[i]!);
        }
      }
      measurements.push({ name, worldCenterSamples: 120, rows, worstForwardStep: worstStep, terminalDisplacements: terminal.map((p, i) => p.distanceTo(birth[i]!)) });
    } finally { art.breakup.reset(); }
  });
