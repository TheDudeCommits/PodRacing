...
Use [Round 35 implementation and current QA](docs/inkstorm-overhaul/ROUND35_UI_COMBAT_HANDOVER.md) as the active checkpoint. **V27 passes 889 tests / 149 files, TypeScript/build, 17 native interface captures and four crash/control cases.** It adds original painted flame artwork, support-pivot engine settling and clearer Build comparisons. The actual served/loaded flame asset is verified at every captured crash phase. Completed visual reviews remain below target. The previous overview below is historical.
