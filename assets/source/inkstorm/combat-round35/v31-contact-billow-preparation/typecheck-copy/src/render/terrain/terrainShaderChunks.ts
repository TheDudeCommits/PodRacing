...
// The signed field is baked at 6 m. Its broad authored cliff faces need that
// local gradient even when distant procedural dunes use wider lighting probes.
export const TERRAIN_AUTHORED_NORMAL_EPSILON = 6;
...
void main() {
  ...
  vec3 unusedFields;
  // Keep the original wide procedural-dune gradient. A signed cliff must not
  // average its face together with a neighbouring shelf as that radius grows.
  ...
  const float authoredEpsilon = ${TERRAIN_AUTHORED_NORMAL_EPSILON.toFixed(1)};
  vec2 authoredGradient = (vec2(
    courseGulfOffset(logicalXZ + vec2(authoredEpsilon, 0.0)),
    courseGulfOffset(logicalXZ + vec2(0.0, authoredEpsilon))
  ) - authoredOffset) / authoredEpsilon;
  vec2 broadAuthoredGradient = (vec2(offsetX, offsetZ) - authoredOffset) / normalEpsilon;
  // Subtract only the wide signed-field component, then restore its local
  // gradient. Zero authored offsets preserve the former dune normal exactly.
  // The bilinear samples and interpolated vertex normals stay continuous;
  // there are no flat triangle normals or quantized slope bands here.
  vec2 normalDelta = vec2(terrainHeight - heightX, terrainHeight - heightZ)
    + (broadAuthoredGradient - authoredGradient) * normalEpsilon;
  ...
  // Keep the stone material across the physical cut/fill zero crossing.
  vTerrainAuthoredGrade = length(authoredGradient);
