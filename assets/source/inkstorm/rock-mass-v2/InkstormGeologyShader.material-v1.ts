...
vec3 inkstormRockAlbedo(vec3 world, vec3 normal, sampler2D paint, float ready) {
  ...
  if (ready>.5) {
    ...
    // Broad fracture plates occupy metres, rather than the previous paint's
    // dozens of thin repeating strata on a single face. One 96m swatch spans
    // about three uneven courses and retains quiet planes between fissures.
    vec3 texel=texture2D(paint,world.zy/96.).rgb*weights.x
      +texture2D(paint,world.xz/96.).rgb*weights.y
      +texture2D(paint,world.xy/96.).rgb*weights.z;
...
vec3 inkstormRockLight(vec3 albedo, vec3 normal, vec3 sun, float visibility) {
  ...
  vec3 recess=albedo*vec3(.32,.47,.82)+pow(vec3(.10,.085,.15),vec3(2.2));
