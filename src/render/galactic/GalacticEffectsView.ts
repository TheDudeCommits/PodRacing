...
export type GalacticBurstStyle = 'crash' | 'mine' | 'sand' | 'rock' | 'redline' | 'reward' | 'energy';
...
function createMineGeometry() {
  const body = new CylinderGeometry(0.72, 0.92, 0.42, 8);
  const halo = new TorusGeometry(0.84, 0.11, 4, 8).rotateX(Math.PI / 2);
  const crossA = new BoxGeometry(2.1, 0.16, 0.22);
  const crossB = new BoxGeometry(0.22, 0.16, 2.1);
  const geometry = mergeGeometries([body, halo, crossA, crossB], false);
  body.dispose();
  halo.dispose();
  crossA.dispose();
  crossB.dispose();
  if (!geometry) throw new Error('Unable to build procedural scrap-mine geometry.');
  return geometry;
}
...
function createHazardBodyGeometry() {
  const rocks: BufferGeometry[] = [];
  for (const [x, y, z, scale, pitch, yaw] of [
    [-0.38, -0.32, 0.08, 0.34, 0.2, 0.1],
    [0.34, -0.08, -0.2, 0.27, -0.3, 0.6],
    [0.04, 0.22, 0.3, 0.24, 0.5, -0.4],
    [-0.2, 0.46, -0.18, 0.19, -0.2, 0.8],
    [0.28, 0.4, 0.14, 0.16, 0.7, 0.2],
  ] as const) {
    const indexedRock = new IcosahedronGeometry(scale, 0);
    const rock = indexedRock.index ? indexedRock.toNonIndexed() : indexedRock.clone();
    rock.rotateX(pitch).rotateY(yaw).translate(x, y, z);
    indexedRock.dispose();
    rocks.push(rock);
  }
  const geometry = mergeGeometries(rocks, false);
  for (const rock of rocks) rock.dispose();
  if (!geometry) throw new Error('Unable to build procedural hazard-body geometry.');
  return geometry;
}

function createExplosionPlateGeometry(): BufferGeometry {
  // A hard-edged sixteen-point anime impact plate reads in a single frame at
  // racing speed. Concentric smooth rings looked like flat decals when viewed
  // across the sand and made mine detonations almost disappear.
  const points = 16;
...
export class GalacticEffectsView extends Group {
  ...
  readonly explosionPlates = prepare(new InstancedMesh(
    ...
    // Solid anime impact plates hold their authored hue against bright sand;
    // additive blending made orange mine cores disappear into the terrain.
    material(0.86),
    GALACTIC_EFFECT_CAPACITY.explosionPlates,
  ...
  private readonly hazardSlots = Array.from(
    { length: GALACTIC_EFFECT_CAPACITY.hazardBodies },
  ...
  private readonly debrisSlots = Array.from(
    ...
    (): DebrisSlot => ({ active: false, startTime: 0, duration: 0, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, rx: 0, ry: 0, rz: 0, ax: 0, ay: 0, az: 0, scale: 1, groundY: 0, r: 1, g: 0.3, b: 0.1 }),
  );
  ...
  setScrapMines(states: readonly ScrapMineEffectState[]): void {
    ...
    for (let index = 0; index < this.mineCount; index += 1) {
      ...
      slot.phase = finite(state.phase, 0);
      slot.variant = state.variant === 'pickup' ? 'pickup' : 'mine';
    }
  }
  ...
  emitCrash(event: CrashEffectEvent): void {
    ...
    const fragmentCount = Math.min(18, Math.max(7, Math.round((style === 'rock' ? 9 : 11) * severity)));
    const fallbackColor = style === 'reward' || style === 'energy'
      ? '#55ffc1'
      : style === 'sand'
        ? '#ffd071'
        : style === 'rock'
    ...
    for (let fragment = 0; fragment < fragmentCount; fragment += 1) {
      ...
      const radial = (5.5 + hash01(seed + 1) * 11) * severity * radialScale;
      slot.active = true;
      slot.startTime = finite(event.time, 0);
      slot.duration = 0.85 + hash01(seed + 2) * 1.15;
      slot.x = finite(event.position.x, 0);
      slot.y = finite(event.position.y, 0);
      slot.z = finite(event.position.z, 0);
      slot.vx = finite(event.velocity?.x, 0) * inheritedVelocity + Math.cos(angle) * radial;
      slot.vy = finite(event.velocity?.y, 0) * 0.12
        + (5.5 + hash01(seed + 3) * 11) * severity * liftScale;
      slot.vz = finite(event.velocity?.z, 0) * inheritedVelocity + Math.sin(angle) * radial;
      ...
      slot.scale = (0.12 + hash01(seed + 10) * 0.38) * severity * fragmentScale;
      slot.groundY = finite(event.groundY, event.position.y - 1);
      const spark = fragment % 4 === 0;
    ...
    const plateCount = style === 'redline' || style === 'reward' || style === 'energy' ? 4 : 3;
    for (let plate = 0; plate < plateCount; plate += 1) {
      ...
      slot.z = finite(event.position.z, 0);
      slot.scale = style === 'mine'
        ? severity * (plate === 0 ? 4.8 : 3.4 + plate * 1.15)
        : style === 'reward'
          ? severity * (2.2 + plate * 0.55)
          : severity * (2.4 + plate * 1.35) * (style === 'redline' ? 1.2 : 1);
      slot.style = style;
      slot.r = plate === 0 ? 1 : this.color.r;
      slot.g = plate === 0 ? (style === 'reward' ? 0.86 : 1) : this.color.g;
      slot.b = plate === 0 ? (style === 'reward' ? 0.24 : style === 'energy' ? 1 : 0.46) : this.color.b;
    }
  }

  update(time: number, camera?: Camera): void {
    this.prepareVisibility(camera);
    if (camera) camera.getWorldQuaternion(this.billboardQuaternion);
  ...
  private updateMines(time: number): void {
    ...
    for (let index = 0; index < this.mineCount; index += 1) {
      const slot = this.mineSlots[index];
      if (!slot) continue;
      const pickup = slot.variant === 'pickup';
      const spin = slot.yaw
        + time * (pickup ? 1.8 : 0)
        + Math.sin(time * 2 + slot.phase) * 0.08;
      if (pickup) {
        // Salvage floats as a tilted rotating glyph, distinct from the flat
        // armed mine that shares this instanced draw pool.
        this.euler.set(0.68, spin, 0.24);
        this.tempQuaternion.setFromEuler(this.euler);
      } else {
      ...
      this.writeInstance(this.scrapMines, visibleCount, slot.x, y, slot.z, this.tempQuaternion,
        pulse, pulse, pulse,
        pickup ? 0.2 : slot.armed ? (hot ? 1 : 0.78) : 0.22,
        pickup ? 1 : slot.armed ? (hot ? 0.22 : 0.1) : 0.68,
        pickup ? 0.66 : slot.armed ? 0.05 : 0.72);
      visibleCount += 1;
    }
  ...
  private updateHazards(time: number): void {
    ...
    for (let index = 0; index < this.hazardCount; index += 1) {
      ...
      const warningRadius = slot.radius * (0.88 + slot.warning * 0.24) * pulse;
      const bodyRadius = slot.radius * (0.32 + slot.intensity * 0.38) * pulse;
      const bodyHeight = slot.height * (0.28 + slot.intensity * 0.72);
      const boundsRadius = Math.max(warningRadius, bodyRadius, bodyHeight * 0.5);
      if (!this.isVisible(
        slot.x,
      ...
      if (slot.kind === 'sand-geyser') {
        ...
        // Magenta is reserved for lethal terrain warnings; it cannot be
        // mistaken for the mint course ribbon or checkpoint gates.
        r = 0.96;
        g = 0.16;
        b = 0.42;
      }
      this.writeInstance(this.hazardTelegraphs, visibleCount, slot.x, slot.y + 0.04, slot.z, IDENTITY,
      ...
      const bodyY = fallingRock
        ? slot.y + 0.9 + (1 - fallCycle) * Math.max(7, bodyHeight + 5)
        : slot.y + bodyHeight * 0.5;
      this.euler.set(
      ...
      this.tempQuaternion.setFromEuler(this.euler);
      const rockScale = Math.max(1.8, bodyRadius * 0.62);
      this.writeInstance(this.hazardBodies, visibleCount, slot.x, bodyY, slot.z, this.tempQuaternion,
        fallingRock ? rockScale : bodyRadius,
        fallingRock ? rockScale : bodyHeight,
        fallingRock ? rockScale : bodyRadius,
        r, g, b);
      visibleCount += 1;
    }
    this.dirty(this.hazardTelegraphs, visibleCount);
    this.dirty(this.hazardBodies, visibleCount);
  }
  ...
  private updateCrash(time: number): void {
    ...
    for (const slot of this.debrisSlots) {
      ...
      this.tempQuaternion.setFromEuler(this.euler);
      const fade = MathUtils.clamp(1 - age / slot.duration, 0.12, 1);
      this.writeInstance(this.crashDebris, debrisCount, x, y, z, this.tempQuaternion,
        slot.scale, slot.scale, slot.scale, slot.r * fade, slot.g * fade, slot.b * fade);
      debrisCount += 1;
    }
    ...
    for (const slot of this.explosionSlots) {
      ...
      const quaternion = (groundedMine && !slot.billboard) || installation
        ? GROUND_RING
