...
export function sampleTerrainSurface(
  ...
  out.height = terrainCore(x, z, scratchFields);
...
export function sampleTerrain(
  ...
  const height = terrainCore(x, z, scratchFields);
  const epsilon = TERRAIN_NORMAL_EPSILON;
  const slopeX = (
    terrainCore(x + epsilon, z) - terrainCore(x - epsilon, z)
  ) / (epsilon * 2);
  const slopeZ = (
    terrainCore(x, z + epsilon) - terrainCore(x, z - epsilon)
  ) / (epsilon * 2);
