...
export class InkstormWorld extends Group {
  ...
  private async load():Promise<void>{
    ...
    for(let i=0;i<models.length;i++){
      ...
      const batch=new InstancedMesh(geometry,new InkstormSurfaceMaterial(stone),512);
  ...
  setCourse(course:PodraceCourse):void{
    ...
    this.roads.push(createInkstormFoundations(getInkstormLayout(course), this.heightAt));
    ...
    for(const p of placements){
      ...
      const grounded=p.family==='canyon-buttress'?groundInkstormButtress(p,this.heightAt,fixedRockCrowns.get(p.id)):null;
      position.set(p.x,grounded?.baseY??this.heightAt(p.x,p.z)+(p.family==='pit-district'?1.55:-1.5),p.z);
