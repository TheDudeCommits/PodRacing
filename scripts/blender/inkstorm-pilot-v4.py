...
STAGE = 'PodRacing — Teemto pilot construction v4'
...
def conform_sample(kind,t,a,offset=0):
    ...
    p=p00.lerp(p01,at).lerp(p10.lerp(p11,at),rt)
    normal=(cloth_sample(kind,t,a,.001)-cloth_sample(kind,t,a)).normalized()
...
def surface_seam(name,kind,points,width=.0015,color=(.014,.020,.026,1),stitches=True):
    ...
    for t,a in points:
        ...
        eps=width/max(.018,(conform_sample(kind,t,a)-conform_sample(kind,t,a+math.pi)).length*.5)
        vertices.extend([conform_sample(kind,t,a-eps*.5,.00075),conform_sample(kind,t,a+eps*.5,.00075)])
    ...
    if stitches:
        ...
        for i in range(0,len(points)-1,2):
            ...
            delta=.036
            vv=[conform_sample(kind,t,a+delta,.00095),conform_sample(kind,t,a+delta+.011,.00095),
                conform_sample(kind,t2,a2+delta+.011,.00095),conform_sample(kind,t2,a2+delta,.00095)]
...
faces=[(i*28+j,(i+1)*28+j,(i+1)*28+(j+1)%28,i*28+(j+1)%28) for i in range(23) for j in range(28)]
...
# Balaclava tapers into the jacket; the sculpted collar conceals the former
# long cylindrical connector while leaving the fixed helmet anchor untouched.
loft('rubber','overlapped balaclava neck-seal',[(0,-.160,.027,.031,.027),
    (0,-.160,.044,.034,.028),(0,-.160,.057,.037,.030),
    (0,-.158,.071,.038,.033),(0,-.155,.083,.040,.037)],20,.88)
collar_vertices=[]
for row in range(5):
    for j in range(33):
        a=.13+j*(math.tau-.26)/32
        front=max(0,math.cos(a))
        top=.076-.024*front**3+.004*math.sin(a-.35)
        # The stand is taller at back/side, with a lowered overlapping front V.
        rx,ry,z=[(.058,.046,.026),(.057,.044,.042),(.051,.040,top),
            (.047,.036,top-.002),(.043,.033,.027)][row]
        z+=.002*math.sin(3*a+.5) if row in (1,2) else 0
        collar_vertices.append((rx*math.sin(a),-.163+ry*math.cos(a),z))
collar_faces=[(i*33+j,(i+1)*33+j,(i+1)*33+j+1,i*33+j+1) for i in range(4) for j in range(32)]
collar_faces.extend([(0,33,66,99,132),(32,164,131,98,65)])
geometry('suit','split stand racing collar',collar_vertices,collar_faces,True,(.025,.032,.037,1))
# Flat fabric binding follows the shaped top edge, with a matte interior reveal.
for j in range(32):
    a0=Vector(collar_vertices[66+j]);a1=Vector(collar_vertices[66+j+1])
    b0=Vector(collar_vertices[99+j]);b1=Vector(collar_vertices[99+j+1])
    geometry('suit','collar top binding',[a0,a1,b1,b0],[(0,1,2,3)],False,(.065,.059,.046,1))
# The front overlap is a cut fabric fastening tab, not an exposed neck ring.
ribbon('suit','collar overlapping throat tab',[(-.020,-.121,.038),
    (-.008,-.119,.050),(.008,-.119,.047),(.020,-.123,.036)],.009,.0015,axis=(0,0,1))
for side in (-1,1):
...
for side in (-1,1):
    ...
    # Patch shares the cloth lattice, so every corner follows the folded skin.
    vv=[]
    for i in range(9,16):
        for j in range(-1,3):
            a=j*math.tau/24
            vv.append(cloth_sample('arm',i/51,a if side>0 else math.pi-a,.0006))
    ff=[(i*4+j,i*4+j+1,(i+1)*4+j+1,(i+1)*4+j) for i in range(6) for j in range(3)]
    geometry('accent','upper sleeve cloth patch '+str(side),vv,ff,True)
    ...
    ribbon('rubber','glove cuff closure',[(wrist.x+side*.021,wrist.y-.010,wrist.z-.003),
        (wrist.x+side*.024,wrist.y+.003,wrist.z+.007),
        (wrist.x+side*.013,wrist.y+.014,wrist.z+.013)],.009,.0015,axis=(0,0,1))
...
try:
    ...
    for original,matrix in originals:
        ...
        mesh.name='Pilot v4 unchanged body '+original.data.name
        mesh.transform(matrix)
        obj=bpy.data.objects.new('pilot-v4-body-'+original.name,mesh)
        stage.collection.objects.link(obj)
        copied.append(obj)
    root=bpy.data.objects.new('teemto-pilot-construction-v4',None)
    ...
    for key,batch in batches.items():
        material=bpy.data.materials.new('Inkstorm pilot v4 '+key)
        ...
        mesh=bpy.data.meshes.new('Original Inkstorm pilot v4 '+key)
        ...
        obj=bpy.data.objects.new('teemto-pilot-v4-'+key,mesh)
        stage.collection.objects.link(obj)
        obj.parent=root
    ...
    for original in source.objects:
        ...
        obj=bpy.data.objects.new('Pilot v4 review '+original.name,data)
        stage.collection.objects.link(obj)
    ...
    result={'scene':STAGE,'sourceScene':SOURCE,'sourceModified':False,
        ...
        'bodyCopies':len(copied),'featureCount':len(features),'geometrySeed':'authored-static-v4-angular-cloth-collar',
