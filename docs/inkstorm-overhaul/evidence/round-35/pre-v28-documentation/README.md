> **Current local development: Round 35 / V27 (9 September 2026).** Painted flame animation, engine settling and clearer Build comparisons pass 889 tests / 149 files, TypeScript/build, 17 native UI captures and four crash/control cases. Completed UI and crash visual reviews remain FAIL; V28 is private crash-only work, not admitted. [Exact checkpoint, actual images and remaining work](docs/inkstorm-overhaul/ROUND35_UI_COMBAT_HANDOVER.md). The full visual/fleet overhaul is unfinished and has not been deployed; older descriptions below are historical.
...
**V28 is a focused crash-only private pass in progress, with no live admission yet. V27 UI remains frozen. No V27 FPS measurement has been run.** V24b's measured **59.47/58.98 Hz** M4 races remain historical and do not transfer to V27 or V28. The fleet remains four registered source families, with **22 remaining**.
