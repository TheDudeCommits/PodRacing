...
export function createPitPadField(placements: readonly PitPlacement[], route: readonly RoutePoint[],
  ...
  const pads = placements.filter(p => p.family === 'pit-complex' || p.family === 'pit-district').map(p => {
    ...
    const pad: PitPad = { id: p.id, family: p.family, slabBottomOffset: district ? 1.55 : 1.45, x: p.x, z: p.z, yaw: p.yaw,
      anchorHeight: originalAnchorHeight, originalAnchorHeight, floorHeight: originalAnchorHeight + floorOffset, floorOffset, frontage: [],
    ...
    const heights = frontage.map(f => f.height).sort((a, b) => a - b), floorHeight = heights[2]! + .15;
    const anchorHeight = floorHeight - floorOffset;
    return { ...pad, minRoadClearance: minimum, accepted, frontage, floorHeight, anchorHeight, targetHeight: anchorHeight + .9 };
  });
  ...
  for (let row = 1; row < rows - 1; row++) for (let column = 1; column < columns - 1; column++) {
    ...
    let selectedDistance = Infinity, weight = 0, targetSum = 0, targetWeight = 0;
