...
import { DEFAULT_PODRACER_CONFIG } from '../simulation/config';
...
function activeBranchFor(
  ...
  if (active) {
    ...
    const insideDecisionWindow = progress >= active.entryProgress - 0.035
      && progress <= active.exitProgress + 0.025;
...
function branchTargetSample(
  ...
  const routeSpan = Math.max(1, (branch.exitProgress - branch.entryProgress) * context.course.totalLength);
  const currentRouteProgress = clamp(
    (context.self.courseProgress - branch.entryProgress)
      / Math.max(1e-6, branch.exitProgress - branch.entryProgress),
    0,
    1,
  );
  const targetRouteProgress = clamp(
    currentRouteProgress + lookaheadDistance / routeSpan,
    0,
    1,
  );
  const targetIndex = Math.min(
    branch.points.length - 2,
    Math.max(0, Math.floor(targetRouteProgress * (branch.points.length - 1))),
  );
  const point = branch.points[targetIndex];
  const next = branch.points[targetIndex + 1];
  if (!point || !next) return null;
  const segmentLength = Math.max(1e-6, Math.hypot(next.x - point.x, next.z - point.z));
  const tangentX = (next.x - point.x) / segmentLength;
  const tangentZ = (next.z - point.z) / segmentLength;
  const canonical = context.course.sampleAtProgress(point.canonicalProgress);
  return {
    ...canonical,
    x: point.x,
    y: point.y,
...
export function stepAIController(
  ...
  const lookaheadDistance = clamp(
    (tune.lookaheadBase + speed * tune.lookaheadSpeed - corner.severity * 34)
      * (0.88 + difficulty.reactionScale * 0.12),
    48,
    170,
  );
  ...
  let targetProgress = checkpointApproach && context.requiredCheckpoint
    ? context.requiredCheckpoint.progress
    : wrapCourseProgress(
  ...
  desiredLane = clamp(desiredLane, -target.width * 0.98, target.width * 0.98);
  state.laneOffset += (desiredLane - state.laneOffset) * Math.min(
    1,
    delta * (checkpointApproach ? 5.8 : 1.45),
  );

  const targetX = target.x + target.rightX * state.laneOffset;
  const targetZ = target.z + target.rightZ * state.laneOffset;
  ...
  const curveFeedForward = clamp(target.curvature * speed * 1.6, -0.32, 0.32);
  const deterministicWobble = Math.sin(
    context.raceTime * (1.8 + (state.seed & 7) * 0.13) + (state.seed % 1024),
  ) * tune.noise * difficulty.noiseScale;
  let requestedSteer = headingError * tune.steeringGain * difficulty.steeringScale
    + curveFeedForward + avoidance.steer + deterministicWobble;
  if (state.mistake.kind === 'awkward-landing' && (!context.self.grounded || here.tag === 'launch-crest')) {
    requestedSteer += state.mistake.side * (0.3 + 0.36 * state.mistake.strength);
  }
  requestedSteer = clamp(requestedSteer, -1, 1);
  state.steerMemory += (requestedSteer - state.steerMemory) * Math.min(
    1,
    tune.steeringResponse * difficulty.reactionScale * delta,
  );
  ...
  const rubberBand = rubberSignal * tune.rubberBandStrength
    * (rubberSignal < 0 ? 1.45 : 1);
  const cornerSlowdown = 1 - corner.severity * (state.personality === 'aggressive' ? 0.39 : 0.5);
  let targetSpeed = DEFAULT_PODRACER_CONFIG.maxSpeed * tune.pace * difficulty.paceScale
    * cornerSlowdown * (1 + rubberBand) * objective;
  ...
  targetSpeed = clamp(targetSpeed, 48, DEFAULT_PODRACER_CONFIG.maxSpeed * 1.17);

  const speedError = targetSpeed - speed;
  const desiredThrottle =
    clamp(0.48 + speedError / 38, 0, 1) * (1 - avoidance.threat * 0.7);
  state.throttleMemory += (desiredThrottle - state.throttleMemory) * Math.min(1, delta * 4.8);
  const brake = Math.max(
    clamp((-speedError - 7) / 34, 0, 1),
