...
const dir='output/round49';await mkdir(dir,{recursive:true});
...
try {
 ...
 await page.locator('.setup-destination>summary').click();await page.locator('[data-destination="frozen"]').click();
 ...
 await page.locator('.setup-destination>summary').focus();await padButton(15);
 ...
 if(!controllerFocus?.includes('setup-more'))throw new Error(`Controller did not navigate to race rules: ${controllerFocus}`);
 await padButton(0);await page.waitForFunction(()=>document.querySelector('.setup-more').open);
 ...
 for(const [width,height] of [[1024,768],[390,844],[360,740],[844,390]]){
  ...
  const layout=await page.evaluate(()=>{const mode=document.querySelector('.setup-header').getBoundingClientRect(),roster=document.querySelector('.setup-roster-heading').getBoundingClientRect();return {modeBottom:mode.bottom,rosterTop:roster.top};});
  if(layout.rosterTop<layout.modeBottom)throw new Error('Mode and roster rows overlap');
  results.push({viewport:[width,height],sizing,layout});
 }
 await page.setViewportSize({width:390,height:844});await page.locator('.setup-destination>summary').click();await page.screenshot({path:`${dir}/home-mobile-destinations.png`});await page.locator('[data-destination="jungle"]').click();
