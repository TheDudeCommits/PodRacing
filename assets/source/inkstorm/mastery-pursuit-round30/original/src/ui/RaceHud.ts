...
export class RaceHud {
  ...
  private updateMastery(mastery: HudMasteryViewModel | undefined, phase: RaceHudViewModel['phase'], preRace: boolean, tutorialVisible: boolean): void {
    ...
    if (mastery.latestSector && phase === 'racing') {
      ...
      write(this.split, `S${sector.index} ${sector.delta === null ? formatRaceTimeHundredths(sector.time) : formatSplitDelta(sector.delta)}`);
      this.split.classList.toggle('is-slow', (sector.delta ?? 0) > 0);
    }
    if (mastery.tutorial) {
