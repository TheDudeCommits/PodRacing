...
export class RaceSimulation {
  ...
  state: RaceSimulationState<AIControllerState>;
  private readonly markerColliders: readonly { x: number; z: number; id: string }[];
  ...
  constructor(options: RaceSimulationOptions) {
    ...
    this.course = options.course ?? createProceduralPodraceCourse(options.terrain, raceSeed);
    ...
    this.options = Object.freeze({
      ...
      fieldSize: clamp(Math.floor(options.fieldSize ?? 8), 4, 8),
  ...
  step(
    ...
    if (this.state.phase === 'racing' && playerEntry) {
  ...
  private createMarkerColliders(): readonly { x: number; z: number; id: string }[] {
    const renderData = this.course.getRenderData(1024);
    const stride = Math.max(5, Math.floor(renderData.points.length / 90));
    const colliders: { x: number; z: number; id: string }[] = [];
    for (let index = 0; index < renderData.points.length; index += stride) {
      ...
      for (const side of [-1, 1] as const) {
        colliders.push({
          x: point.x + rightX * point.width * side,
          z: point.z + rightZ * point.width * side,
  ...
  /**
   * Once the winner has had a short cinematic window, classify racers still
   * out on the desert by their live standing. Their projected finish times
   * are deterministic and ordered, so the timing board remains complete
   * without forcing a player to wait for an off-camera AI recovery.
   */
  private classifyRemainingRacers(events: RaceEvent[]): void {
    this.updateStandings();
    const ordered = [...this.state.entries]
      .sort((a, b) => (
        (a.progress.placement ?? this.state.entries.length)
        - (b.progress.placement ?? this.state.entries.length)
      ));
    let projectedTime = Math.max(
      this.state.raceTime,
      ...ordered.map((entry) => entry.progress.finishTime ?? 0),
    );
    for (const entry of ordered) {
      if (entry.status === 'finished') continue;
      projectedTime += 0.42;
      entry.status = 'finished';
      entry.progress.finishTime = projectedTime;
      entry.progress.completedLaps = this.state.totalLaps;
      entry.progress.currentLap = this.state.totalLaps;
      if (entry.competition?.finishReason === 'running') {
        entry.competition.finishReason = 'classified';
      }

      const missingLaps = Math.max(0, this.state.totalLaps - entry.progress.lapTimes.length);
      if (missingLaps > 0) {
        const recorded = entry.progress.lapTimes.reduce((sum, lap) => sum + lap, 0);
        const projectedLap = Math.max(1, (projectedTime - recorded) / missingLaps);
        for (let lap = 0; lap < missingLaps; lap += 1) {
          entry.progress.lapTimes.push(projectedLap);
        }
      }
      events.push({
        type: 'finish',
