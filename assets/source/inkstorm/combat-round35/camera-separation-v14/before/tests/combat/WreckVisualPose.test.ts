...
describe('renderer-only rigid wreck pose', () => {
  ...
  it('remeasures an existing procedural proxy when its geometry changes without a revision change', () => {
    const root = new Group();
    const mesh = new Mesh(new BufferGeometry().setAttribute('position',
      new Float32BufferAttribute([-10, 0, -8, 10, 5, 22], 3)));
    root.add(mesh);
    const cache = new WreckVisualPoseCache(); cache.refresh(root, [mesh], 0);
    const old = mesh.geometry;
    mesh.geometry = new BufferGeometry().setAttribute('position',
      new Float32BufferAttribute([-2, 0, -3, 2, 2, 5], 3));
    cache.refresh(root, [mesh], 0);
    expect(cache.localBounds.min.toArray()).toEqual([-2, 0, -3]);
    expect(cache.localBounds.max.toArray()).toEqual([2, 2, 5]);
    expect(cache.localCenter.toArray()).toEqual([0, 1, 1]);
    old.dispose(); mesh.geometry.dispose();
  });
});
