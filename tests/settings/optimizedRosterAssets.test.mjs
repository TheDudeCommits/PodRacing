...
import { getLegacyVehicleArtDefinition as getVehicleArtDefinition, legacyPodFile } from '../fixtures/legacyVehicleArt';
...
// The retired replica packages, kept as fixtures for the damage and wreck contracts.
describe('lossless runtime pod packages',()=>{
 it('uses admitted, smaller packages for all eight pods with byte-identical geometry, normals, UVs and pilots',()=>{
  ...
  for(const id of SELECTABLE_POD_APPEARANCES)for(const lod of ['hero','rival']){
   ...
   const original=read(legacyPodFile(r.source)),runtime=read(legacyPodFile(r.runtime));expect(hash(original)).toBe(r.sourceSha256);expect(hash(runtime)).toBe(r.sha256);
