  ...
  private updateStrikingEngine(index: number, part: Part, pose: WreckVisualPose, age: number,
    ...
    for (let pass = 0; pass < (index === 0 || frontBalance > 0 ? 2 : 1); pass++) {
      ...
      for (const support of part.samples.supportPoints) {
        ...
        if (candidate < gap) { gap = candidate; if (index === 1) this.supportPivot.copy(support); if (index === 2) this.contact.position.set(this.point.x, ground, this.point.z); }
      ...
      this.groundOffset.copy(index === 1 ? this.supportPivot : part.samples.localCenter).applyQuaternion(this.rotation).add(this.translation);
      ...
      this.translation.copy(index === 1 ? this.supportPivot : part.samples.localCenter).applyQuaternion(this.rotation).negate().add(this.groundOffset);
      this.world.compose(this.translation, this.rotation, UNIT);
    }
    if (Number.isFinite(gap)) {
