...
const source=await readFile(path.join(stage,'canyon-arch-v2b-source.glb'));
...
primitive.setMaterial(doc.createMaterial('canyon-arch-v2b-stone').setBaseColorFactor([1,1,1,1]).setMetallicFactor(0).setRoughnessFactor(.93));
previous.dispose();
await doc.transform(dedup(),prune());
const bytes=await io.writeBinary(doc),report=await validator.validateBytes(bytes,{uri:'canyon-arch-v2b.glb',maxIssues:1000});
...
await writeFile(path.join(stage,'canyon-arch-v2b.glb'),bytes);
assert.equal(hash(await readFile(oldPath)),oldHash);
const receipt={asset:'canyon-arch-v2b.glb',status:'isolated candidate; no public/runtime integration',source:'original deterministic Blender MCP geometry',
