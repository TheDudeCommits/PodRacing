...
function createAtmosphericMaterial(): MeshBasicMaterial {
  ...
  result.customProgramCacheKey = () => 'painterly-atmosphere-contact-v5';
  return result;
}
...
export class GalacticEffectsView extends Group {
  ...
  emitWreckGroundContact(time: number, position: GalacticPoint, direction: GalacticPoint): void {
    ...
    for (let index = 0; index < 10; index++) {
      ...
      slot.groundY = position.y; slot.metal = false; slot.spark = true; slot.contactSpark = true;
    ...
    for (let layer = 0; layer < 2; layer += 1) {
      for (const slot of this.explosionSlots) {
        ...
        if (slot.contactLayer >= 4) {
          ...
          const height = width * (scrape ? .18 : .54);
          ...
          this.writeEffectSurface(plateCount, 4, .67 * (1 - MathUtils.smoothstep(progress, .25, 1)), slot.startTime * 1.37);
          plateCount++;
          continue;
        }
