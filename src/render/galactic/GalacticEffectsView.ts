...
import { isWreckSupportFootprint, type WreckGroundFootprint } from '../combat/WreckGroundContact';
...
export interface WreckGroundSurfaceContext {
  /** Only a producer-validated presentation support uses the per-part budget. */
  readonly partId?: 0 | 1 | 2;
...
export class GalacticEffectsView extends Group {
   ...
   * Explicit per-part context selects two porous billow parents; legacy
   * terrain/owner calls retain five parents.
  ...
  emitWreckGroundContact(time: number, position: GalacticPoint, direction: GalacticPoint, footprint?: WreckGroundFootprint, surface?: WreckGroundSurfaceContext): void {
    ...
    const partId = surface?.partId;
    if (partId !== undefined && partId !== 0 && partId !== 1 && partId !== 2) return;
    const perPart = partId !== undefined;
    // Match the producer gate before allocating or consuming an event seed.
    if (perPart && (!isWreckSupportFootprint(footprint) || typeof surface?.terrain?.heightAt !== 'function'
      || !Number.isInteger(surface.owner) || surface.owner < 0 || surface.owner >= 8
      || !Number.isSafeInteger(surface.sequence) || surface.sequence < 0)) return;
    ...
    const span = footprint && Number.isFinite(footprint.center.x + footprint.center.y + footprint.center.z
      ...
      && footprint.halfLength >= (perPart ? .000001 : 1) && footprint.halfLength <= 12 && footprint.halfWidth >= 0 && footprint.halfWidth <= 6
      && Math.abs(Math.hypot(footprint.axis.x, footprint.axis.z) - 1) < .001 ? footprint : undefined;
    // Legacy malformed optional edges retain their centre-fan fallback;
    // per-part edges already passed the shared gate above. Copy scalar data
    // into pooled slots because the producer reuses these vectors each pose.
    ...
    if (edges) for (let index = 0; index < 2; index++) {
      ...
      if (!edge || !Number.isFinite(edge.x + edge.y + edge.z) || Math.abs(along) > (perPart ? 12 : span!.halfLength + 4)
    ...
    this.eventSequence++;
    // Three cues share the existing ten-grit budget: one casing per piece,
    // two clods on left/front and three on rear. Legacy calls stay unchanged.
    const debrisCount = perPart ? (partId === 2 ? 4 : 3) : 10;
    for (let index = 0; index < debrisCount; index++) {
      ...
      const along = span ? (index / (debrisCount - 1) - .5) * span.halfLength * 1.8 : 0;
      ...
      if (span) {
        // Across the three per-part cues (or one legacy cue): three raking
        // casing pieces and seven clods. The birth band stays unchanged;
        // these are departing
        // particles, not additional contact witnesses or gameplay impacts.
        const casing = index < (perPart ? 1 : 3), rakeSpeed = casing ? 10 + hash01(seed + 1) * 5 : 5 + hash01(seed + 1) * 5;
    ...
    if (span && edges && surface && typeof surface.terrain?.heightAt === 'function'
      ...
      for (let index = 0; index < (perPart ? 2 : 5); index++) {
        ...
        const edgeIndex = perPart ? index : index < 3 ? 1 : 0, edge = edges[edgeIndex]!;
        ...
        slot.along = perPart ? 0 : span.halfLength * (index < 3 ? (index - 1) * .68 : (index - 3.5) * .8);
