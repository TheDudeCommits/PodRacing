...
try{
 ...
 for(const p of [host,guest]) {await p.locator('[data-hud="setup-more"] > summary').click();await p.locator('[data-hud="room-panel"] > summary').click();}
 ...
 await host.locator('[data-destination="frozen"]').click();await host.getByRole('button',{name:'Race',exact:true}).first().click();
