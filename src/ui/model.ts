...
/** Pause navigation is local-only; teaching starts before the launch signal. */
export function createSessionHudViewModel(model: Pick<RaceHudViewModel, 'phase' | 'preRace' | 'mastery'>) {
  const solo = !model.preRace || model.preRace.lobby.role === 'solo';
  const activeAttempt = !model.preRace?.active && (model.phase === 'countdown' || model.phase === 'racing');
  return {
    solo,
    canRestart: solo && activeAttempt,
    eventTitle: model.mastery?.eventTitle ?? 'Race in progress',
    tutorialVisible: Boolean(model.mastery?.tutorial) && activeAttempt,
    pauseHint: solo
      ? 'Retry repeats this course. Back to Hangar ends this attempt.'
      : 'Online session · race navigation is managed by the room.',
  };
}
...
export function createWorkshopHudViewModel(
  ...
  const parts: HudWorkshopPartViewModel[] = WORKSHOP_SLOTS.flatMap((slot) =>
    WORKSHOP_PARTS_BY_SLOT[slot].map((partId) => {
      ...
      const candidate = slot === activeSlot ? deriveWorkshopSummary({ ...safeLoadout, slots: { ...safeLoadout.slots, [slot]: partId } }, vehicleClass) : null;
      const comparison = candidate ? (Object.keys(summary.totals) as WorkshopStat[])
        .filter((stat) => candidate.totals[stat] !== summary.totals[stat])
        .sort((a, b) => Math.abs(candidate.totals[b] - summary.totals[b]) - Math.abs(candidate.totals[a] - summary.totals[a]))
        .slice(0, 3).map((stat) => ({ label: WORKSHOP_STAT_LABELS[stat], current: summary.totals[stat], candidate: candidate.totals[stat] })) : [];
      return {
        id: part.id,
        slot,
        name: part.name,
  ...
  return {
    ...
    parts,
    summary: {
      totals: (Object.keys(summary.totals) as WorkshopStat[]).map((stat) => ({ label: WORKSHOP_STAT_LABELS[stat], value: summary.totals[stat] })),
...
export function deriveRaceHudViewModel(
  ...
  const profile = snapshot.settings?.competitionProfile;
  if (raceModeStatus && (profile === 'time-trial' || profile === 'training')) {
    raceModeStatus.name = profile === 'training' ? 'Flight School' : 'Time Attack';
    raceModeStatus.objective = profile === 'training' ? 'Learn the machine. Complete one lap.' : 'One clean lap. Chase your personal best.';
    raceModeStatus.scoreLabel = 'Personal best';
    raceModeStatus.scoreValue = options.mastery?.bestTime ? formatRaceTime(options.mastery.bestTime) : 'Set a time';
  }
