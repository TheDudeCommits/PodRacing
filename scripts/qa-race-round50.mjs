...
try {
 ...
 for(const [biome,seed] of Object.entries({desert:0x494e4b53,...RACING_BIOME_SEEDS})){
  const race=new RaceSimulation({terrain:{heightAt:sampleTerrainHeight},seed,countdownSeconds:3,totalLaps:3,competitionProfile:'clean-race',resultsGraceSeconds:20});
  ...
  const counts={},samples=[];let rankDrift=0;let ticks=0;const begin=performance.now();
  while(ticks<120*650 && race.state.phase!=='finished'){
   const sn=snapshots();
   ...
   for(const e of entries){if(e.status!=='finished'&&e.progress.offCourseDistance<3&&e.progress.nextCheckpointIndex!==(e.progress.lastCheckpointIndex+1)%race.course.checkpoints.length)throw new Error('Checkpoint cursor mismatch');}
   ticks++;
  }
  ...
  await mkdir('output/round50',{recursive:true});await writeFile('output/round50/full-race.json',JSON.stringify(results,null,2));
  if(player.progress.completedLaps!==3||player.progress.finishTime===null)throw new Error(`Player did not finish three laps: ${biome}`);
 }
