...
import type { WreckGroundFootprint } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v23-contact-edge-preparation/test-runtime/WreckGroundContact.ts';
import { mergeGeometries } from 'three/addons/utils/BufferGeometryUtils.js';
import { CEL_POST_EXCLUDE_USER_DATA_KEY } from '/Users/amir/Projects/PodRacing/src/render/materials/InvertedHullOutline';
...
import { createSolidEffectMaterial } from '/Users/amir/Projects/PodRacing/src/render/galactic/SolidEffectMaterial';
import {
  createSolidHardwareGeometry, createSolidRubbleGeometry,
  SOLID_HARDWARE_MINE_DEPTH_SCALE, SOLID_RUBBLE_BASE_Y,
} from '/Users/amir/Projects/PodRacing/src/render/galactic/SolidEffectGeometry';
...
function createAtmosphericMaterial(): MeshBasicMaterial {
  ...
  result.customProgramCacheKey = () => 'painterly-atmosphere-contact-v6';
  return result;
}
...
export class GalacticEffectsView extends Group {
  ...
  private readonly debrisSlots = Array.from(
    ...
    (): DebrisSlot => ({ active: false, startTime: 0, duration: 0, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, rx: 0, ry: 0, rz: 0, ax: 0, ay: 0, az: 0, scale: 1, groundY: 0, metal: false, spark: false, ember: false, contactSpark: false, emberPhase: 0, wreckOwner: -1, wreckSequence: -1, r: 1, g: 0.3, b: 0.1 }),
  );
  ...
  emitCrash(event: CrashEffectEvent): void {
    ...
    for (let plate = 0; plate < plateCount; plate += 1) {
      ...
      if (contactBurst) {
        ...
        slot.scale = severity * [RUPTURE_FLASH_SCALE, tracked ? 1.25 : 3.5, 2.7, 3.2][plate]!;
        slot.billboard = true;
        slot.style = style;
   ...
   */
  emitWreckGroundContact(time: number, position: GalacticPoint, direction: GalacticPoint, footprint?: WreckGroundFootprint): void {
    ...
    for (let index = 0; index < 10; index++) {
      ...
      slot.groundY = origin.y; slot.metal = false; slot.spark = true; slot.contactSpark = true;
    ...
    for (let index = 0; index < 2; index++) {
      ...
      slot.x = origin.x + dx * index * .7; slot.z = origin.z + dz * index * .7;
  ...
  private updateCrash(time: number): void {
    ...
    for (const slot of this.debrisSlots) {
      ...
      const x = slot.x + slot.vx * motionAge;
      ...
      const y = slot.ember ? slot.y + Math.max(0, slot.vy) * motionAge - 1.5 * motionAge * motionAge
      ...
      const fade = slot.ember ? (1 - motionAge / .42) * (1 - MathUtils.smoothstep(progress, .8, 1))
        : slot.metal ? 1 : MathUtils.clamp(1 - progress, 0.12, 1);
