/** Shared off-road pit and district grading, with frozen rigid-building anchors. */
...
export function createPitPadField(placements: readonly PitPlacement[], route: readonly RoutePoint[],
  ...
  // Preserve the accepted pit-only terrain where an existing six-metre mesh
  // triangle can reach outside its slab. This is bake-time composition only;
  // the renderer still receives one atlas and the same four texture fetches.
  const previousPits = placements.some(p => p.family === 'pit-district')
    ? createPitPadField(placements.filter(p => p.family === 'pit-complex'), route, branches, ungradedHeightAt) : null;
  const segments = segmentsFor(route, branches);
  const nearby = protectionBuckets(segments);
  const pads = placements.filter(p => p.family === 'pit-complex' || p.family === 'pit-district').map(p => {
    const c = Math.cos(p.yaw), s = Math.sin(p.yaw), anchorHeight = ungradedHeightAt(p.x, p.z);
    const district = p.family === 'pit-district';
    const pad: PitPad = { id: p.id, family: p.family, slabBottomOffset: district ? 1.55 : 1.45, x: p.x, z: p.z, yaw: p.yaw, anchorHeight,
  ...
  if (previousPits?.grid) {
    const oldPads = previousPits.pads.filter(p => p.accepted);
    for (let row = 1; row < rows - 1; row++) for (let column = 1; column < columns - 1; column++) {
      const x = minX + column, z = minZ + row;
      let distance = Infinity;
      for (const pad of oldPads) distance = Math.min(distance, padDistance(pad, x, z));
      if (distance >= 24) continue;
      const index = row * columns + column, current = grid.values[index]!, previous = previousPits.sampleOffset(x, z);
      if (current > previous) grid.values[index] = current + (previous - current) * (1 - smooth((distance - 8) / 16));
    }
  }
