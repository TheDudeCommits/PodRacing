...
OUTPUT = BASE / '15c-author-contact-v7-composed-payload.py'
INVENTORY = BASE.parent / 'v7-contact-preparation' / 'preparation-inventory-v3.json'
...
payload += (BASE / '15c-copy-contact-color-v7-safe-types.py').read_text()
...
inventory = {
 'stage': 'V7 short contact color preparation', 'status': 'prepared only; no Blender execution', 'supersedesRejectedBeforeExecutionPayload': '15b-author-contact-v7-composed-payload.py', 'safeModeCorrection': 'Removed module-as-value preflight; actual nodes.new plus all AO property/socket assertions remain transactional', 'transactionFix': 'Cleanup nested finally; success only after all source/context/image/graph guards pass',
 ...
 'fragments': [digest(BASE / name) for name in names + ['15c-copy-contact-color-v7-safe-types.py']],
