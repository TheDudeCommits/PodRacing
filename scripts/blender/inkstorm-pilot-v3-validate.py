...
BASE=Path('/Users/amir/Projects/PodRacing/assets/source/inkstorm/vehicles/4eff45899ada40bb920c5c744663db90/processed/pilot-v3')
...
def inspect(path):
    ...
    pilot=[m for m in meshes if m['name'].lower().startswith('original inkstorm pilot v3')]
...
if __name__=='__main__':
    ...
    for path in [BASE/'pilot-color-atlas-v3.png',*sorted((BASE/'review').glob('*.png')),
            Path('/Users/amir/Projects/PodRacing/scripts/blender/inkstorm-pilot-v3.py'),
            Path('/Users/amir/Projects/PodRacing/scripts/blender/inkstorm-pilot-v3-material.py')]:
        if path.exists():
            data=path.read_bytes()
