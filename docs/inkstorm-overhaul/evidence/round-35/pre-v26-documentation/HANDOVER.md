...
Use [Round 35 implementation and current QA](docs/inkstorm-overhaul/ROUND35_UI_COMBAT_HANDOVER.md) as the active checkpoint. It distinguishes build verification, ordinary browser flows, actual combat footage, independent criticism and performance evidence. The previous overview below is historical. The completed V24b checkpoint passes862 V24 tests plus final TypeScript/build,64 responsive fixtures,17 native UI captures, four live crash cases, a219m/s crash and two full races at59.47/58.98Hz average on the tested M4. Visual critics remain below target: HUD6.5/7 and crash5/10. A private V25 HUD/effects revision and generated rupture target are being prepared; no V25 runtime result is claimed.
