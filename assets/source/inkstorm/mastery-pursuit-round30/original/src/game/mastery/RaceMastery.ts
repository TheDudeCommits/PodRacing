...
export class RaceMastery {
  ...
  step(result: RaceStepResult): void {
    ...
    if (recovered || wrecked) this.invalidReason = 'Recovery used — finish for practice, then retry for a clean record.';
    if (player.progress.splits.length > this.lastSplitCount) {
      const index = player.progress.splits.length - 1;
      const split = player.progress.splits[index]!;
      const prior = this.previousBest?.sectors[index];
      this.latestSectorValue = { index: index + 1, time: split.segmentTime, delta: prior === undefined ? null : split.segmentTime - prior };
    ...
    if (player.progress.finishTime !== null && !this.resultValue) {
      ...
      this.resultValue = {
        ...
        sectors: player.progress.splits.map((split, index) => ({ index: index + 1, time: split.segmentTime,
          delta: this.previousBest?.sectors[index] === undefined ? null : split.segmentTime - this.previousBest.sectors[index]! })),
