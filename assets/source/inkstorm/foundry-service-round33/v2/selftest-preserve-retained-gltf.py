...
def fixture(source, *, drift=False, rgb8=False, mutation=None):
    ...
    for index in range(16016):
        x, z = -32.0 + (index % 128) * .5, -3.0 + (index // 128) * .045
        start = len(rows)
        positions = ((x, 61.4, z), (x, 61.4, z + .024), (x + .28, 61.4, z))
        if index == 0:
            positions = ((44.5, 3.93, 0.), (44.5, 4.93, .1), (45., 3.93, .3))
    ...
    if mutation == 'reversed':
        ...
        rows[-1][-1] = .5
    elif mutation == 'low-inside':
        for index in triangles[3940]:
            rows[index][0] -= 45.0
    elif mutation == 'below-riser-floor':
        rows[triangles[3940][0]][1] = 3.92
    ...
    indices = struct.pack('<' + 'H' * 59868, *(index for triangle in triangles for index in triangle))
    ...
    document['accessors'].append({'bufferView': 1, 'componentType': 5123, 'type': 'SCALAR', 'count': 59868})
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='gantry-preservation-selftest-') as temporary:
        ...
        assert check['lowAddedRisersAllowedOnlyOutsideOpenCorridor']
        assert abs(check['addedBoundsGltf'][1][0] - 3.93) < 2e-4
        tests.append({'test': 'low-added-riser-outside-open-corridor-allowed', 'pass': True})
        ...
        expected = {'reversed': 'Reversed', 'duplicate': 'Duplicate', 'signal': 'Removed signal', 'moved': 'Missing/moved',
                    'missing': 'Missing/moved', 'false-match': 'Duplicate', 'alpha': 'Nonopaque', 'low-inside': 'clearance', 'below-riser-floor': 'addition envelope'}
    ...
    report = {'status': 'PASS', 'scope': 'Synthetic fixtures only; this selftest does not check the actual V2 native export or Khronos validation.',
