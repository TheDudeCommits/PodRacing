...
vec3 inkstormRockAlbedo(vec3 world, vec3 normal, sampler2D paint, float ready) {
  ...
  if (ready>.5) {
    ...
    vec3 texel=texture2D(paint,world.zy*.027).rgb*weights.x
      +texture2D(paint,world.xz*.027).rgb*weights.y
      +texture2D(paint,world.xy*.027).rgb*weights.z;
    // Existing mipmapped paint supplies chipped edges and pits; no new sharp
    // procedural grain or animated noise at racing speed.
    float wear=clamp(dot(texel,vec3(.30,.59,.11))/.19,.35,1.6);
    albedo*=mix(.60,1.34,smoothstep(.38,1.4,wear));
...
vec3 inkstormRockNormal(vec3 world,vec3 normal,vec3 albedo) {
  float relief=dot(albedo,vec3(.30,.59,.11))*3.2;
  ...
  return normalize(mix(normal,bumped,.48));
}
...
vec3 inkstormRockLight(vec3 albedo, vec3 normal, vec3 sun, float visibility) {
  ...
  float stoneValue=clamp(dot(albedo,vec3(.30,.59,.11))/.24,.48,1.55);
  vec3 recess=pow(vec3(.31,.27,.42),vec3(2.2))*(.40+stoneValue*.60);
  recess=mix(recess,albedo*vec3(.16,.30,.58),.16);
  float skyPlane=.84+.16*clamp(dot(normal,vec3(.58,.35,.73))*.5+.5,0.,1.);
  recess*=skyPlane;
