...
STAGE = 'PodRacing — Teemto pilot construction v3c'
...
def cloth_sample(kind,t,a,offset=0):
    ...
    for tc,aw,ac,amp,phase,slant,extent in config['creases']:
        ...
        d=t-direction
        displacement+=amp*mask*(gauss(d-aw*.70,aw)-.86*gauss(d+aw*.35,aw*.64))
...
def conform_sample(kind,t,a,offset=0):
    rings=28 if kind=='jacket' else surface_configs[kind]['rings']
    sides=32 if kind=='jacket' else surface_configs[kind]['sides']
...
for side in (-1,1):
    ...
    arm_creases=[
        (.115,.039,.2,.0044,.2,.027,1.35),(.23,.037,-.5,.0034,1.2,-.040,1.40),
        (.365,.035,-.5,.0058,2,.028,1.65),(.445,.033,-.85,.0070,1,.033,1.75),
        (.52,.032,.15,.0058,.3,-.021,1.5),(.60,.037,-.3,.0048,1.6,.029,1.45),
        (.78,.036,.4,.0034,2.5,-.027,1.35),(.875,.031,-.2,.0041,1.7,.025,1.50),
        (.96,.026,-.75,.0040,.4,-.022,1.50)]
    ...
    offset=len(batches['suit']['v'])-48*20
    for i in range(48):
        for j in range(20):
            t=i/47;a=j*math.tau/20
            oriented=a if side>0 else math.pi-a
            if .60<t<.93 and -.55<angle_delta(oriented,.2)<.7:
                batches['suit']['colors'][offset+i*20+j]=(.016,.023,.031,1)
    ...
    surface_seam('forearm reinforcement '+str(side),'arm',
        [(.60+i*.006,.86 if side>0 else math.pi-.86) for i in range(59)],.0014)
    ...
    for i in range(8,15):
        for j in range(-1,3):
            a=j*math.tau/20
            vv.append(cloth_sample('arm',i/47,a if side>0 else math.pi-a,.0006))
