...
const candidate=path.join(root,'assets/source/inkstorm/round30-next-steps/v3');
export async function resolve(specifier, context, next){
 let parent=context.parentURL;
 if((parent?.startsWith(pathToFileURL(candidate).href)||parent?.includes('/round30-next-steps/v2/'))&&parent.endsWith('/InkstormFoundry.ts')&&specifier.startsWith('.')) parent=pathToFileURL(path.join(root,'src/render/inkstorm',path.basename(fileURLToPath(parent)))).href;
