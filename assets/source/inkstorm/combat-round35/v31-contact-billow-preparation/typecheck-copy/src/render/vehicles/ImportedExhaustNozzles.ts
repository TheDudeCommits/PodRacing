...
export function createImportedNozzleGeometry(): LatheGeometry {
  const profile = [
    [.90, .52], [.95, -.11], [.88, -.27], [.70, -.27],
    [.64, -.16], [.44, -.02], [.44, .08], [.72, .52], [.90, .52],
  ...
  for (let i = 0; i < position.count; i++) {
    ...
    const value = radius < .72 ? .10 : radius < .80 ? .28 : 1;
...
export class ImportedExhaustNozzles extends InstancedMesh<LatheGeometry, CelMaterial> {
  ...
  constructor() {
    super(createImportedNozzleGeometry(), new CelMaterial({
      ...
      vertexColors: true, specularStrength: .025, rimStrength: 0, reflectionStrength: 0, wear: .18,
    }), 2);
