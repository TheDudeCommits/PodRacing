...
import { GalacticEffectsView, GALACTIC_EFFECT_DRAW_CALL_BUDGET, type CrashEffectEvent, type GalacticPoint } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v25-directional-rupture-preparation/test-runtime/GalacticEffectsView.ts';
...
describe('directional authored rupture lifecycle', () => {
  it('orients both hot surfaces in the camera plane while keeping exact birth and moving-cut roots', () => {
    ...
    for (const direction of [new Vector3(1, 0, 0), new Vector3(.4, .3, -.8660254037844386), cameraNormal.clone()]) {
      ...
      try {
        ...
        view.syncWreckRupture(0, 3, moved); view.update(4.1, camera);
        const records = plates(view), hot = records.filter(p => p.kind === 6 || p.kind === 7);
        expect(hot).toHaveLength(2);
        expect(records.map(p => p.kind)).toEqual([8, 8, 6, 7]);
  ...
  it('keeps ownership and original expiry through sync, ring wrap and unrelated effects', () => {
