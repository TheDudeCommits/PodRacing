...
export function createInkstormFoundations(placements: readonly InkstormPlacement[], heightAt: (x: number, z: number) => number,
  pitAnchorHeight?: (id: string) => number | undefined): InstancedMesh {
  const geometry = new BoxGeometry(1, 1, 1);
  const mesh = new InstancedMesh(geometry, new InkstormSurfaceMaterial(false), placements.length * 2);
  ...
  for (const p of placements) {
    ...
    const anchor = p.family === 'pit-complex' ? pitAnchorHeight?.(p.id) : undefined;
