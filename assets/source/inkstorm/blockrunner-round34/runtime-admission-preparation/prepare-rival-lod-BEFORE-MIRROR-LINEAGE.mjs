 ...
 * No geometry update/reprojection, permissive mode, threshold relaxation or public writes. */
...
for(const r of protectedRows){
 ...
 for(let f=0;f<r.polygonCount;f++){
  ...
  const src=[0,1,2].map(k=>transform(m,point(sp,si[f*3+k]))),actual=idx.map(i=>point(positions,i));
...
// Absolute geometric error cap 2 cm, normal weight .15 m, UV weight 3 m.
const [simplified,error]=MeshoptSimplifier.simplifyWithAttributes(indices,positions,3,attributes,5,[.15,.15,.15,3,3],locks,24000*3,.02,['LockBorder','ErrorAbsolute']);
