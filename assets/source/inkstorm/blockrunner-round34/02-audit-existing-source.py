...
def components(mesh, matrix):
    ...
    result = []
    for key, vertices in sorted(groups.items(), key=lambda pair: min(pair[1])):
...
report = {'stage': 'audit', 'status': 'executing', 'sourceUid': REFERENCE['uid'],
          ...
          'scope': 'Exact named-source guard, evaluated scene occurrences and topology; no authoring or anchor acceptance.'}
try:
    source = source_guard()
    ...
    object_rows = []
    unique_meshes = {}
    for ob in sorted(source.objects, key=lambda item: item.name):
