...
const cases = [
  ...
  { id: 'solo-high-speed', camera: 'chase', reducedMotion: false, pauseCancel: false, accelerating: true },
];
...
const selected = requested ? cases.filter((test) => requested.includes(test.id)) : cases.filter(test => !test.accelerating);
...
const receipt = {
  ...
  scope: 'Native Chrome input and read-only clock/camera/HUD observations. Default four cases use actual Shift+S overheat/wreck; optional solo-high-speed releases S after countdown and holds W+Shift, requiring measured pre-impact speed above30m/s. No capture mode, presets, stepped simulation, health/heat writes, event injection, or race-state staging. Recorded/sampled run is not a performance benchmark.',
  cpuOnlyCoverage: [
...
const expectedFlameAtlasSha = process.env.INKSTORM_EXPECTED_FLAME_ATLAS_SHA;
if (expectedFlameAtlasSha && !/^[a-f0-9]{64}$/.test(expectedFlameAtlasSha)) {
  throw new Error('INKSTORM_EXPECTED_FLAME_ATLAS_SHA must be an exact lowercase SHA-256.');
}
async function flameAtlasReceipt(page) {
  if (!expectedFlameAtlasSha) return null;
  await page.waitForFunction(() => window.__PODRACING__.snapshot().game.flameAtlas?.status === 'ready', undefined, { timeout: 15000 });
  const atlas = (await snapshot(page)).game.flameAtlas;
  assert(atlas.width === 1254 && atlas.height === 1254 && atlas.frameCount === 4, 'Loaded flame atlas dimensions/frames do not match admitted artwork.');
  const source = new URL(atlas.url, page.url());
  assert(source.origin === new URL(page.url()).origin, 'Flame atlas must be served by this frozen local build.');
  const response = await fetch(source);
  assert(response.ok && /image\/png/i.test(response.headers.get('content-type') ?? ''), 'Flame atlas bytes are not a successful PNG response.');
  const bytes = new Uint8Array(await response.arrayBuffer());
  const sha256 = createHash('sha256').update(bytes).digest('hex');
  assert(sha256 === expectedFlameAtlasSha, `Flame atlas checksum mismatch: ${sha256}`);
  return { ...atlas, url: source.href, bytes: bytes.length, sha256, statusCode: response.status };
}
...
async function installObserver(page) {
  await page.evaluate(({ nativeCadence }) => {
    ...
    const observation = { samples: [], events: [], rafFrames: [], stopped: false, startedAt: performance.now(),
      racerIds: window.__PODRACING__.snapshot().galactic.racers.map(racer => racer.id) };
    ...
    const frame = (wallMs) => {
      ...
      if (wallMs - previous >= 25) {
        ...
        observation.samples.push({ wallMs, frame: state.simulationFrame, raceTime: state.raceTime,
          speed: state.game.speed, position: [...state.game.position],
          ...
          hudWreck: root?.classList.contains('has-wreck-state'),
          drivingFeedbackVisible: nativeCadence ? null : Boolean(root?.querySelector('.pod-hud__driving-feedback')?.getClientRects().length),
...
function analyze(observation, test) {
  ...
  const roster = new Set(observation.racerIds ?? []);
  const invalidCredits = events.filter(event => event.type === 'takedown'
    ? !roster.has(event.attackerId) || event.attackerId === event.victimId
    : event.type === 'wreck' && event.takedownBy !== null
      && (!roster.has(event.takedownBy) || event.takedownBy === event.racerId));
  require(roster.has('player') && invalidCredits.length === 0,
    'A wreck/takedown was credited to scenery, an unknown roster ID or its own victim.');
  ...
  require(cue.cueTitle === (wreck?.takedownBy ? 'TAKEN DOWN' : 'WRECKED'),
    'The visible victim cue disagrees with the actual racer takedown attribution.');
  ...
  let highSpeed = null;
  if (test.accelerating) {
    const prior = samples.filter(sample => sample.frame < wreck?.frame && sample.wreck === 'running').at(-1);
    highSpeed = prior ? { frame: prior.frame, speedMps: prior.speed, position: prior.position,
      gapToCueMs: cue.wallMs - prior.wallMs } : null;
    require(highSpeed && Number.isFinite(highSpeed.speedMps) && highSpeed.speedMps > 30
      && highSpeed.gapToCueMs >= 0 && highSpeed.gapToCueMs < 120,
      'Accelerating scenario did not establish actual speed above 30 m/s immediately before the wreck.');
  }
  ...
  let breakup = null;
  if (test.id === 'solo-chase' || test.accelerating) {
  ...
  return { outcome: issues.length ? 'FAIL' : 'PASS', issues, explosion, wreck, estimatedStartMs, before, slow, after,
    restoredCamera: test.camera, samples: samples.length, cadence, breakup, highSpeed };
}
...
async function runCase(test) {
  ...
  const capture = async (name) => {
    ...
    const hud = nativeCadence ? null : await page.evaluate(() => {
      const root = document.querySelector('.pod-hud');
      const panels = ['.pod-hud__race', '.pod-hud__driving-instruments', '[data-hud="redline-instrument"]', '.pod-hud__course-progress', '.pod-hud__driving-feedback'];
    ...
    const capturedSnapshot = await snapshot(page);
    if (expectedFlameAtlasSha) {
      const atlas = capturedSnapshot.game.flameAtlas;
      assert(atlas?.status === 'ready' && atlas.frameCount === 4 && atlas.width === 1254 && atlas.height === 1254,
        `Authored flame atlas not ready at ${test.id}/${name}; refusing fallback visual acceptance.`);
    }
    result.captures.push({ name, path, snapshot: capturedSnapshot, hud });
    if (hud?.cinematic) assert(hud.panels.every(panel => !panel.hasLayoutBox), 'Racing instrument remains under cinematic matte');
    if (hud?.wreck) assert(hud.panels.every(panel => !panel.hasLayoutBox), 'Driving instructions remain during wreck or recovery');
  };
  try {
    ...
    result.flameAtlas = await flameAtlasReceipt(page);
    ...
    if (test.accelerating) {
      ...
      result.commands.push({ action: 'After actual countdown, release S and hold native W throttle; no course, pose or speed writes.' });
    }
    ...
    await page.keyboard.down('Shift'); result.commands.push({ action: test.accelerating ? 'native Shift held with W to accelerate and overheat' : 'native Shift held with S to overheat', beforeFrame: result.before.simulationFrame });
    ...
    if (!nativeCadence) {
      const wreckHudSamples = observation.samples.filter(sample => sample.hudWreck);
      result.drivingFeedbackGuard = { samples: wreckHudSamples.length,
        visibleSamples: wreckHudSamples.filter(sample => sample.drivingFeedbackVisible).length };
      assert(wreckHudSamples.length >= 3 && result.drivingFeedbackGuard.visibleSamples === 0,
        'Driving feedback remained visible during observed wreck/recovery HUD states');
      const actualWreckSamples = observation.samples.filter(sample => sample.wreck === 'wrecked' || sample.wreck === 'recovering');
      result.simulationWreckFeedbackGuard = { samples: actualWreckSamples.length,
        visibleSamples: actualWreckSamples.filter(sample => sample.drivingFeedbackVisible).length };
      assert(actualWreckSamples.length >= 3 && result.simulationWreckFeedbackGuard.visibleSamples === 0,
        'Driving feedback remained visible during actual simulation wreck/recovery, including its birth frame');
    }
