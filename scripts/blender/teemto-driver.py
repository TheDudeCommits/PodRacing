"""Original helmeted seated driver fitted to the inspected Teemto source cockpit.
...
stage = bpy.data.scenes['PodRacing — Teemto material study v1']
assert 'teemto-pilot' not in stage.objects
saved_scene = bpy.context.window.scene
saved_layer = bpy.context.window.view_layer
bpy.context.window.scene = stage
pilot = bpy.data.objects.new('teemto-pilot', None)
...
for name, color, rough, metal in (
    ...
    m = bpy.data.materials.new('Teemto original pilot ' + name)
    m.use_nodes = True
...
print(json.dumps({'driver':'Original Inkstorm helmeted racer','scene':stage.name,
                  'pilotMeshes':4,'pose':'seated, both hands on the source controls',
                  'pelvis':[0,-.13,-.205], 'helmetCenter':[0,-.16,.143],
                  'handContacts':[[-.122,.103,-.071],[.122,.103,-.071]],
