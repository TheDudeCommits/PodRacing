...
const LAUNCH_ESCARPMENT_CUTS: Readonly<Record<string, readonly LaunchEscarpmentCut[]>> = {
  'west-basin-shelves': [
    { id: 'outer-low-shelf', height: 42, feather: 24,
      points: [[668, -625], [750, -510], [844, -545], [820, -660]] },
    { id: 'inner-middle-shelf', height: 58, feather: 24,
      points: [[690, -450], [820, -305], [955, -315], [856, -436]] },
    { id: 'outer-upper-shelf', height: 100, feather: 24,
      points: [[790, -607], [928, -555], [1035, -585], [948, -670]] },
    { id: 'outer-through-slot', height: -20, feather: 40,
      points: [[625, -568], [810, -518], [958, -541], [1034, -520],
        [1063, -584], [952, -610], [790, -588], [655, -648]] },
    { id: 'inner-through-slot', height: -55, feather: 40,
      points: [[735, -326], [852, -296], [952, -303], [1050, -337],
        [1044, -370], [926, -374], [814, -369], [717, -401]] },
  ],
  'industrial-east-ridge': [
    { id: 'leading-service-shelf', height: 48, feather: 24,
      points: [[352, 328], [500, 314], [558, 367], [537, 472], [447, 501], [335, 465]] },
    { id: 'outer-middle-shelf', height: 88, feather: 24,
      points: [[418, 521], [594, 505], [712, 562], [718, 630], [549, 635], [427, 601]] },
    { id: 'outer-upper-shelf', height: 128, feather: 24,
      points: [[700, 490], [871, 493], [903, 560], [794, 611], [696, 574]] },
    { id: 'inner-through-slot', height: -65, feather: 40,
      points: [[342, 257], [512, 247], [596, 281], [592, 315], [501, 325], [354, 334]] },
    { id: 'outer-through-slot', height: -25, feather: 40,
      points: [[411, 548], [512, 513], [628, 509], [736, 559], [810, 548],
        [824, 584], [734, 616], [620, 566], [512, 572], [415, 609]] },
  ],
};
...
function escarpmentCutWeight(cut: LaunchEscarpmentCut, forward: number, right: number): number {
  ...
  return inside ? 1 : 1 - smoother(Math.sqrt(distanceSquared) / cut.feather);
}
...
function escarpmentHeight(id: string, forward: number, right: number, top: number, window: number): number {
  ...
  for (const cut of LAUNCH_ESCARPMENT_CUTS[id] ?? []) {
    ...
    result = Math.min(result, top + (Math.min(top, cut.height) - top) * weight);
  }
  return result;
}
...
export function launchRidgeSurface(forward: number, right: number): { height: number; weight: number } {
  ...
  for (const [ridgeIndex, ridge] of LAUNCH_RIDGES.entries()) {
    ...
    for (let i = 1; i < ridge.stations.length; i++) {
      ...
      // Keep the old wall midpoint, but expose a 48m steep face in these two
      // bounded fronts. The cut polygons below break it into unequal pieces.
      const cliffCap = 1 - smoother((distance - width * (capEdge + .18) + 24) / 48);
