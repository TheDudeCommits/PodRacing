...
export interface CombatPresentationDiagnostics {
  /** Absolute performance-clock boundary, including after a cancelled shot. */
  readonly cooldownUntilWallMs: number | null;
  /** Retained accepted shot; it may have expired. Reading does not reconcile it. */
  readonly cinematic: {
...
export class CombatPresentationController {
  ...
  /** Scalar copies only: diagnostic callers cannot change timing or eligibility. */
  get diagnostics(): CombatPresentationDiagnostics {
    return {
      cooldownUntilWallMs: Number.isFinite(this.nextCinematicAt) ? this.nextCinematicAt : null,
      cinematic: this.cinematic ? {
        startedAtWallMs: this.cinematic.startedAt,
        focusRacerId: this.cinematic.focusRacerId,
        victim: this.cinematic.victim,
      } : null,
    };
  }
