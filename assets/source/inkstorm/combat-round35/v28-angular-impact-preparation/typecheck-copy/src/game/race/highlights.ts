...
export interface RaceHighlightMoment {
  ...
  label: string;
  frameIndex: number;
  /** Race-clock display time, separate from the pose-history timeline. */
  raceTime?: number;
}
...
export class RaceHighlightRecorder {
  ...
  consumeEvents(time: number, events: readonly HighlightEventLike[], raceTime?: number): void {
    for (const event of events) {
      ...
      this.moments.push({
        ...
        ...classified,
        time,
        raceTime: event.type === 'finish' ? eventNumber(event, 'totalTime', raceTime ?? time) : raceTime,
