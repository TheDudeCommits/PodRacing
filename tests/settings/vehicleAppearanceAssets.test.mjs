...
import { TEEMTO_ART_DEFINITIONS, TEEMTO_BODY_GROUP_NODES, TEEMTO_BODY_NODES, SEBULBA_ART_DEFINITIONS, SEBULBA_BODY_NODES, legacyPodFile } from '../fixtures/legacyVehicleArt';
...
describe('published vehicle appearance assets', () => {
  it.each(['hero', 'rival'])('matches the %s body and embedded driver contract using only its JSON header', (lod) => {
    ...
    const bytes = readFileSync(new URL(`../../${legacyPodFile(admittedUrl(definition.url))}`, import.meta.url));
    ...
    expect(triangles).toBe(lod === 'hero' ? 57_618 : 26_180);
    if (lod === 'hero') {
      expect(createHash('sha256').update(bytes).digest('hex')).toBe('af5d69514a51f7e9c089a1710e1587f46e053412b488268dc433f029576c0a7e');
