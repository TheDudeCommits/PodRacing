...
export class CinematicCamera {
  ...
  setComfortSettings(settings: Partial<CameraComfortSettings>): void {
    ...
    if (this.comfort.reducedMotion || this.comfort.shakeIntensity === 0) this.shake = 0;
  }
  ...
  snap(subject: CameraSubject): void {
    ...
    this.camera.fov = this.highlightFraming
      ...
      : this.mode === 'cockpit'
        ? 84
        : 62 + speedT * 15 * this.comfort.fovKickIntensity;
  ...
  private getDesired(subject: CameraSubject, time: number): { position: Vector3; lookAt: Vector3 } {
    ...
    switch (this.mode) {
      ...
      default: {
        ...
        position.copy(subject.position)
          .addScaledVector(forward, -25 - speedT * 9)
          .addScaledVector(UP, 8.5 + speedT * 2)
          .addScaledVector(
            right,
            -3 + (this.comfort.reducedMotion ? 0 : Math.sin(time * 0.7) * 0.25),
          );
        lookAt.copy(subject.position)
          .addScaledVector(forward, 11 + speedT * 13)
    ...
    tempQuaternion.setFromUnitVectors(new Vector3(0, 0, -1), forward);
