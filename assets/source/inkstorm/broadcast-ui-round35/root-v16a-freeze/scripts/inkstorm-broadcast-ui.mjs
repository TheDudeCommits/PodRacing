...
async function finishMotion(page, expectMotion = true) {
  ...
  assert((expectMotion ? activeSamples > 0 : activeSamples === 0) && finalAnimations.length === 0,
    `${trace.label}: ${expectMotion ? 'missing native motion or transition did not settle' : 'reduced motion still animates'}`);
}
