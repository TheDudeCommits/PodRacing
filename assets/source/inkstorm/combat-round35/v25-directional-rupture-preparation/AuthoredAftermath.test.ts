...
import { GalacticEffectsView, GALACTIC_EFFECT_CAPACITY } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v25-directional-rupture-preparation/test-runtime/GalacticEffectsView.ts';
...
describe('fixed authored aftermath slots', () => {
  it('keeps directional hot tongues, smoke and two embers on the moving cut after the birth flash has expired', () => {
    ...
    try {
      ...
      const hot = Array.from({ length: view.explosionPlates.count }, (_, i) => i).filter(i => surface.getX(i) === 7);
