...
export interface TerrainRingGeometryOptions {
  ...
  /** Sample each cell centre for curved authored cliffs without changing shared boundaries. */
  cellCenters?: boolean;
}
...
export function createTerrainRingGeometry(
  ...
  for (let zIndex = 0; zIndex < segments; zIndex += 1) {
    ...
    for (let xIndex = 0; xIndex < segments; xIndex += 1) {
      ...
      const d = a + row;
      const c = d + 1;
      // Counter-clockwise when viewed from +Y. A centre sample keeps a sharp
      // cliff inside a cell from being replaced by its two diagonal planes.
      if (options.cellCenters) {
        const center = positions.length / 3;
        positions.push(xCenter, 0, zCenter);
        indices.push(a, center, b, b, center, c, c, center, d, d, center, a);
  ...
  // Match the Float32 position buffer, including its rounded skirt depth.
  const minY = -72 - (options.outerSkirt ? Math.fround(skirtDepth) : 0);
  const middleY = (minY + 72) * .5;
  const radius = Math.hypot(outer, outer, 72 - middleY);
  geometry.boundingSphere = new Sphere(new Vector3(0, middleY, 0), radius);
  geometry.boundingBox = new Box3(
    new Vector3(-outer, minY, -outer),
    new Vector3(outer, 72, outer),
  );
  geometry.userData.terrainBaseMinY = minY;
