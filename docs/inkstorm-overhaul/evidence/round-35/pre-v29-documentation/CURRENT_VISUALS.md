# Inkstorm current visual review — Round 35 / V28, 9 September 2026

These are actual captures from the verified V28 build, not generated target scenes. [Full current implementation and evidence](ROUND35_UI_COMBAT_HANDOVER.md). Final art acceptance and the full overhaul remain open.
...
## Actual V28 interface and crash

[Build](../../output/gauntlet/round35-broadcast-native-v28/03-desktop-build.png), [map](../../output/gauntlet/round35-broadcast-native-v28/02-desktop-map.png) and [garage](../../output/gauntlet/round35-broadcast-native-v28/01-desktop-garage.png) retain byte-identical V27 images. The seven-image critic uses those exact native shots and the unchanged original references; this supplemental on-course gallery does not replace its inputs.

![Actual V28 native HUD](/Users/amir/Projects/PodRacing/output/gauntlet/round35-broadcast-native-v28/06-desktop-live-race.png)

![Actual V28 native crash](/Users/amir/Projects/PodRacing/output/playwright/round35-combat-v28/solo-chase-wreck.png)

## Actual V28 on-course gallery — 10, 20 and 30 seconds

These three scheduled native frames use actual Open Expedition driving, the loaded Teemto and the unchanged controller through shipped GamepadInput. Before/after snapshots remain inside the course corridor. At 10 and 20 seconds no Director is visible. At 30 seconds, Sandstorm inbound is clear above a naturally behind SAND GEYSER warning. Two surrounding layout samples are separated. This scheduled series uses a behind warning. The additional native frame below verifies general forward IMPACT VECTOR placement; the original HEAT VENT type is demonstrated in paired component images. No every-frame collision-free, physical-controller, art-eight or FPS claim comes from this recorded supplement. [Receipt](../../output/playwright/round35-on-course-hud-v28/receipt.json) · [Independent 48-check audit and limits](../../output/critics/blind-hud-v28/ON_COURSE_RAW_AUDIT.json).

![Actual V28 on-course HUD at 10 seconds](/Users/amir/Projects/PodRacing/output/playwright/round35-on-course-hud-v28/hud-on-course-10s.png)

![Actual V28 on-course HUD at 20 seconds](/Users/amir/Projects/PodRacing/output/playwright/round35-on-course-hud-v28/hud-on-course-20s.png)

![Actual V28 on-course HUD at 30 seconds: visible Director and behind warning](/Users/amir/Projects/PodRacing/output/playwright/round35-on-course-hud-v28/hud-on-course-30s.png)

## Additional actual V28 forward-warning coexistence

An ordinary native opportunity at 42.82–43.05 seconds shows SANDSTORM ACTIVE and IMPACT VECTOR ahead (0°→7°), both visibly separated. The warning starts at x968.54 while the Director ends at x915: 53.54 px horizontal clearance. Both recorded snapshots show loaded running Teemto within the course corridor. This closes the general native forward-placement gap, while exact HEAT VENT type remains component evidence. [Supplement audit](../../output/critics/blind-hud-v28/FORWARD_DIRECTOR_SUPPLEMENT.json).

![Actual V28 forward IMPACT warning clear of the active Director](/Users/amir/Projects/PodRacing/output/playwright/round35-director-forward-native-v28/native-forward-director.png)

The complete V27 gallery follows as historical evidence. Its then-private V28 and then-latest V24b statements are superseded by the current V28 section above; [the pre-V28 document is also preserved](evidence/round-35/pre-v28-documentation/CURRENT_VISUALS.md).

## Historical V27 interface and crash
