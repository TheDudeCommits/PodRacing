...
const rockPaintUniforms = { uRockPaint: { value: null as Texture | null }, uRockPaintReady: { value: 0 } };
...
export function loadInkstormPaint():Promise<Texture|null>{
  ...
  ]).then(([rock,ground,machinery])=>{
    for(const texture of [rock,ground]){texture.colorSpace=SRGBColorSpace;texture.wrapS=RepeatWrapping;texture.wrapT=RepeatWrapping;texture.anisotropy=8;}
...
export class InkstormSurfaceMaterial extends ShaderMaterial {
  constructor(stone = true) {
    super({
      ...
      uniforms: { ...createInkstormShadowUniforms(), ...createInkstormRacerShadowUniforms(), ...machineryPaintUniforms, uPaint: rockPaintUniforms.uRockPaint, uPaintReady: rockPaintUniforms.uRockPaintReady, uStone: { value: stone ? 1 : 0 }, uSun: { value: new Vector3(-.42, .76, -.5).normalize() }, uHaze: { value: new Color('#b79cb8') } },
      vertexShader: `
        #include <common>
...
${INKSTORM_GEOLOGY_GLSL}
        varying vec3 vWorld;varying vec3 vNormal;varying vec3 vLocal;varying vec3 vPaintNormal;
        uniform float uStone;uniform vec3 uSun;uniform vec3 uHaze;uniform sampler2D uPaint;uniform float uPaintReady;
        ...
        void main(){
          ...
          if(uStone>.5){
            ...
            #endif
            base=inkstormRockAlbedo(vWorld,n,uPaint,uPaintReady)*mix(.68,1.25,
