...
export class TeemtoAuthoredDamage {
  ...
  update(pose: WreckVisualPose, remaining: number, terrain: { heightAt(x: number, z: number): number }, extrapolation = 0): void {
    ...
    for (let index = 0; index < this.parts.length; index++) {
      ...
      // The rear machinery makes its supported contact during the held beat;
      // other masses retain their slower follow-through and exact birth/reset.
      const settle = smooth(age / (index === 0 ? .34 : index === 1 ? .22 : index === 2 ? .075 : .18)) * release;
      if (index < 3 && settle > 0) {
        // Terrain clearance alone can leave a long shell balancing on its
        // nose. Settle its long axis along the local ground grade instead of
        // inheriting the parent wreck's pitch. The retained source surface
        // then rests on its broad side/belly, with a different heading per mass.
        const heading = pose.rotation.y + yaw, dx = Math.sin(heading), dz = Math.cos(heading);
        const reach = Math.max(2, (part.samples.localBounds.max.z - part.samples.localBounds.min.z) * .4);
        this.point.copy(part.samples.localCenter).applyMatrix4(this.body);
        const a = terrain.heightAt(this.point.x + dx * reach, this.point.z + dz * reach);
        const b = terrain.heightAt(this.point.x - dx * reach, this.point.z - dz * reach);
        ...
        this.axis.setFromAxisAngle(Z, index === 0 ? -Math.PI / 2 : index === 1 ? 1.3 : Math.PI / 2);
        this.settledRotation.multiply(this.axis).premultiply(this.inverseBody);
        this.rotation.slerp(this.settledRotation, settle);
      }
