...
import { sampleTeemtoStrikeMotion, teemtoStrikeTime } from './TeemtoStrikeMotion';
...
export class TeemtoAuthoredDamage {
  ...
  update(pose: WreckVisualPose, remaining: number, terrain: { heightAt(x: number, z: number): number }, extrapolation = 0): void {
    ...
    this.impactFraming.bounds[25]!.copy(pose.groundContact?.position ?? this.rupture.position);
