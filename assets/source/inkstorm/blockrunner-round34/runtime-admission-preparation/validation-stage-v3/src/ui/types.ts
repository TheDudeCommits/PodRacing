...
import type { VehicleAppearanceId } from '../game/vehicleAppearance';
...
export interface HudWorkshopPartViewModel {
  ...
  tradeoff: string;
  equipped: boolean;
  comparison?: readonly { label: string; current: number; candidate: number }[];
}

export interface HudWorkshopSummaryViewModel {
  totals?: readonly { label: string; value: number }[];
...
export type RaceHudAction =
  | { type: 'select-appearance'; appearance: VehicleAppearanceId }
  | { type: 'retry-appearance' }
  | { type: 'start-race' }
  | { type: 'retry-race' }
...
export interface RaceHudSnapshotLike {
  ...
  raceTime: number;
  totalLaps: number;
  settings?: { mode: HudRaceMode; competitionProfile?: 'chaos' | 'clean-race' | 'time-trial' | 'training' };
  modeState?: {
    mode: HudRaceMode;
