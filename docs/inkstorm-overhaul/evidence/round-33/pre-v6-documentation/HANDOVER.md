...
> Round32 final functional checkpoint: **618 tests / 111 files**, Polwo lifecycle and all three existing regression diagnostics PASS on `index-1xbykiGm.js`. **All six full-race cadence gates and all three Continue flows PASS; round32 is the last fully verified frozen technical checkpoint.** Active round33 source changes are unaccepted and excluded from that result. Three runtime families, 26 imported source families, 23 still awaiting runtime preparation. Strict vehicle/world art parity still FAILS. No deployment.
...
The user approved recommendations 1–8 and art style **B / Inkstorm**, requested all downloadable catalogue vehicles with drivers, Blender MCP/imagegen, repeated blind criticism and 40–60fps. **The request remains incomplete.** Read [ROUND32](docs/inkstorm-overhaul/ROUND32.md) and [CURRENT_VISUALS](docs/inkstorm-overhaul/CURRENT_VISUALS.md) for the frozen round32 checkpoint and active round33 boundary, then [STATUS](docs/inkstorm-overhaul/STATUS.md) and [ROUND31](docs/inkstorm-overhaul/ROUND31.md) for preserved prior gates.

Last fully verified **frozen technical** checkpoint: **round32**, `index-1xbykiGm.js`, SHA256 `f09516c19dc7c4b04372e7ee4416537444a9b989c404a05206088f9fb27855e0`, 1,604,509 bytes. **618 tests / 111 files**, typecheck/build, the 12-stage Polwo lifecycle and the 14-stage Teemto/Sebulba lifecycle pass. Real public/served Polwo hero/rival hashes match; Cup selection admits actual Sola index 4. Context suspension/framebuffer fallback/recovery pass without browser errors. Three staged resource cycles plateau at 212 geometries / 115 textures / 50 programs, with all final-cycle deltas 0. Only deliberate HTTP503 errors occur. These functional diagnostics are separate from the six completed-race cadence gates. All six ordinary Time Attack/Canyon Cup cadence gates and all three actual Continue flows PASS on the frozen round32 artifacts: **59.536–59.736 Hz mean, 16.7–16.8 ms p95**, adaptive DPR 1–2 / 1.75–2. All 37,179 raw rows remain, including 216 racing intervals above 25 ms, none above 50 ms and the normal eight-second Cup classification wait after player finish. [Exact full-race report](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_ROUND32.md) · [Functional receipts and limitations](docs/inkstorm-overhaul/ROUND32.md). The [394-file / 83,433,926-byte frozen snapshot](output/gauntlet/round32-frozen-runtime/inventory.json) remains the result authority.

Historical fully verified performance checkpoint: **round31**, `index-DGjkpNeh.js`, SHA256 `913a7744af6b9237800e54f7dd0898ec0ca3355c2f9638f93545cbcc747a24cb`, 1,602,555 bytes. **600 tests / 108 files**, typecheck/build/diff and final capture/lifecycle/context/resource checks pass. Both Teemto and Sebulba finish ordinary Time Attack and Canyon Cup, and Continue to Foundry. **59.699–59.736 Hz mean racing cadence, 16.8 ms p95, adaptive DPR 1–2 for Time Attack and 1.75–2 for Cup**; one preserved 66.6 ms hitch, 114 racing intervals >25 ms. All 24,745 raw rows remain, including Cup classification wait after player finish. 137 source/34 public-art/41 dist/six harness/four config entries match across all frozen manifests. These numbers apply only to that saved round31 bundle. [Exact historical scope](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_ROUND31.md).

Retained round31 changes: shared physical 138 m launch descent and broader bank joins, **course9 / drive4 / rules2** with old-record archive migration; advance fork guidance; open-cockpit Teemto V2 with visible driver and ten imported opaque draws. Round32 does not alter physics/record editions. Polwo adds its 48,975-triangle hero and 28,830-triangle rival, each with 2 body/6 pilot meshes. Its installed-only camera/garage view and material overrides are retained. Corrected exhaust anchors use the original rear fan plane at Z 8.1356463162 instead of the spike-tip Z 5.8353648 estimate; the duplicated generic pale lip is omitted because source cowls already exist. All originals and failed source derivatives remain.

**Vehicle/world art still FAILS.** The completed fresh [blind final review](docs/inkstorm-overhaul/BLIND_POLWO_FINAL_ROUND32.md) gives frozen round32 **5.8/10 overall**, below the strict 8/10 gate, with approximate vehicle **6.5**, world construction **5.5** and gameplay driver readability **4.5**. These are holistic judgments, not arithmetic averages. All seven original world targets remain unaccepted. The critic inspected all nine outputs, original seven targets and concept10 without implementation/prior-review context; all [17 image hashes match](docs/inkstorm-overhaul/evidence/round-32/blind-review-inventory.json). Major gaps are foundry/fork construction, pilot/control readability, dark engine/material hierarchy and sparse layered geology. Earlier implementation-aware revision3 and fresh round31 4.79/10 reviews remain historical; the V2 cockpit’s 6/10 FAIL remains. Original seven concepts stay the targets; concepts09/10 are supplementary. Earlier worker-cap failures are historical and the fresh32 review is now complete. Still images do not establish motion, grip/contact or articulation, and frozen32 functional/performance PASS remains separate.

**All 26 candidate vehicle sources are officially imported and GLB-preserved**, 748,730,812 total bytes, with the same 29-result search and three excluded results. The remaining 24 downloads succeeded in this continuation; all eight older 429 failures remain preserved. **Three families are runtime-integrated: Teemto, Sebulba, Polwo; 23 still need runtime preparation.** Source visual inspection covers26; formal source driver inspection covers5, with other occupancy uncertainty explicit. One NoDerivs source remains restricted from public adaptations. [Catalogue/provenance](docs/inkstorm-overhaul/VEHICLE_CATALOG.md) and [source inspection inventory](assets/source/inkstorm/catalogue-inspection-round32/README.md) retain acquisition history. The manifest now records frozen round32 performance separately from art FAIL; its full prior file and all earlier checkpoint/model histories remain. Main counters were already three; stale acquisition-state counters were reconciled.

The initial Polwo lifecycle FAIL (ordinal 0→1 on ordinary Start) and revision1 FAIL (waiting for an absent Time Attack AI rival) remain preserved. Corrected checks assert the exact intended Start/reload/Cup transitions and retain appearance identity/storage guards. Final Polwo port 64857 and diagnostic ports 65008/65070/65098 are closed with owned-browser/process checks. Full-race ports 65162/65284/65424 are also closed and the quiet window has ended. All six timing manifests matched 137 source/36 public-art/43 dist/11 harness/four config files. Preserve the recorded one-ULP finalizer failure and exact sequential-sum reconciliation; no native gate, sample or tolerance changed. Never stop unrelated port5211.

Blender work preserves the original active Cruise scene/layer/15-object selection, uses isolated source scenes and clears temporary configuration. Ben export observed an unrelated Cruise-scene membership addition of `Icosphere.005`; origin unestablished and object preserved. Do not claim every global membership stayed exact for that one import; subsequent 22 imports did. Never save the full shared `.blend`. **Round33 is active and unaccepted.** Industrial presentation metadata, an extra gantry render batch/module and a focused test now differ from frozen32; the [09:31:52 UTC audit](output/gauntlet/round32-freeze/documentation-audit.json) still found matching dist. The first isolated source authoring attempt failed normal re-encoding tolerance, restored Cruise context and removed its failed stage. Replacement source export/installation was not complete at that handoff; further work cannot inherit round32 acceptance. Historical full handover is [preserved](docs/inkstorm-overhaul/HANDOVER_BEFORE_ROUND31.md); the immediately previous introduction is [also preserved](docs/inkstorm-overhaul/HANDOVER_BEFORE_ROUND32_FINAL_DIAGNOSTICS.md).

Next: retain the frozen round32 baseline while reviewing and validating round33 independently; prepare the 23 remaining source families; improve cockpit/control readability, geological depth and foundry construction; resume fresh blind critics; complete human/controller/audio, full championship balance and other-device acceptance. Existing synthesized audio and the 32.43-second capture lack a critical direct audition. The prior published product description below remains historical and does not describe this overhaul.
