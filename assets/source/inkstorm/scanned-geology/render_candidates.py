...
scene=bpy.data.scenes['Inkstorm Scanned Geology Lab']
...
try:
    ...
    camera_data=bpy.data.cameras.new('Inkstorm Scanned Candidate Preview Camera')
    camera=bpy.data.objects.new('Inkstorm Scanned Candidate Preview Camera',camera_data)
    scene.collection.objects.link(camera)
    created.append(camera)
    camera.location=(165,-220,147)
    camera.rotation_euler=(Vector((0,0,55))-camera.location).to_track_quat('-Z','Y').to_euler()
    camera_data.type='ORTHO'
    camera_data.ortho_scale=158
    camera_data.clip_end=1000
    scene.camera=camera
    for name,rotation,energy,color in [('Inkstorm Scanned Candidate Preview Sun',(.4,-.55,-.6),3.2,(1,.84,.65)),('Inkstorm Scanned Candidate Preview Fill',(.9,.3,2.1),1.25,(.64,.75,1))]:
    ...
    world=bpy.data.worlds.new('Inkstorm Scanned Candidate Preview World')
    ...
    targets=[('procedural-buttress-comparison','procedural-buttress'),(sorted(o.name for o in scene.objects if o.name.startswith('scanned-sandstone-2-candidate'))[-1],'candidate-2'),(sorted(o.name for o in scene.objects if o.name.startswith('scanned-sandstone-1-candidate'))[-1],'candidate-1')]
    targets.extend([('scanned-sandstone-1-lod-review','candidate-1-lod'),('scanned-sandstone-2-lod-review','candidate-2-lod')])
    for name,label in targets:
        obj=scene.objects[name]
        obj.hide_render=False
        scene.render.filepath='/Users/amir/Projects/PodRacing/assets/source/inkstorm/scanned-geology/'+label+'-preview.png'
