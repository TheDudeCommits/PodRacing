"""V2 UV-ISLAND REPAIR — STAGED, NOT EXECUTED. Run via Blender MCP only after root coordinates authoring.
...
STAGE = 'PodRacing — Polwo Inkstorm shoulder repair round32 V2'
BASE = Path('/Users/amir/Projects/PodRacing/assets/source/inkstorm/polwo-driver-round32')
PAYLOAD = json.loads((BASE/'shoulder-cap-v3-loop-payload.json').read_text())
...
RECEIPT = BASE/'shoulder-repair-v2-execution-receipt.json'
assert not RECEIPT.exists(), 'Preserve previous receipts; choose a new revision.'
...
def geometry_uv_bytes(mesh):
    """Evidence bytes: original coordinates, polygon indices/material flags, UVs."""
    out=bytearray()
    for v in mesh.vertices: out.extend(struct.pack('<3f',*v.co))
    for p in mesh.polygons:
        out.extend(struct.pack('<III',len(p.vertices),p.material_index,int(p.use_smooth)))
        out.extend(struct.pack('<'+'I'*len(p.vertices),*p.vertices))
    for layer in mesh.uv_layers:
        out.extend(layer.name.encode()+b'\0')
        for value in layer.data: out.extend(struct.pack('<2f',*value.uv))
    return bytes(out)
...
before={mesh:hashlib.sha256(geometry_uv_bytes(mesh)).hexdigest() for mesh in source_meshes}
...
report={'source':SOURCE,'stage':STAGE,'status':'executing','runtimeIntegrated':False,
        ...
        'uvPolicy':'Each cap sector uses only its adjacent source triangle UVs; no ring or cross-island averages.',
        'patches':[]}
try:
    ...
    for source in bpy.data.scenes[SOURCE].objects:
        ...
        ob=source.copy(); ob.data=source.data.copy(); ob.name=source.name+' shoulder-repair-v2'
    ...
    try:
        ...
        for specification in PAYLOAD['loops']:
            ...
            # Smart Project creates independent atlas islands along the rim.
            # Every three-face cap sector stays inside its own donor triangle:
            # convex UV combinations cannot cross another atlas island.
            source_loops={li:tuple(p.loop_indices) for p in mesh.polygons for li in p.loop_indices}
            donor_loops=[]
            for edge in rim_edges:
                donor=source_loops[edge[2]]
                assert len(donor)==3 and edge[3] in donor, 'Expected a triangle donor at the sleeve rim.'
                third=[li for li in donor if li not in (edge[2],edge[3])]
                assert len(third)==1
                donor_loops.append((edge[2],edge[3],third[0]))
            uv={}
            for name in initial_uv:
                sectors=[]
                for ia,ib,ic in donor_loops:
                    a,b,c=(Vector(initial_uv[name][li]) for li in (ia,ib,ic))
                    center_uv=(a+b+c)/3
                    inner_a=a*.65+center_uv*.35
                    inner_b=b*.65+center_uv*.35
                    # UV loops are independent even where geometric vertices
                    # are shared by adjacent sectors or at the cap apex.
                    sectors.append({'a':a,'b':b,'innerA':inner_a,'innerB':inner_b,'center':center_uv})
                uv[name]=sectors
            faces=[]
            ...
            for i in range(22):
                j=(i+1)%22
                face((rim[j],rim[i],inner[i]),{n:(v[i]['b'],v[i]['a'],v[i]['innerA']) for n,v in uv.items()})
                face((rim[j],inner[i],inner[j]),{n:(v[i]['b'],v[i]['innerA'],v[i]['innerB']) for n,v in uv.items()})
                face((inner[j],inner[i],apex),{n:(v[i]['innerB'],v[i]['innerA'],v[i]['center']) for n,v in uv.items()})
            assert len(faces)==66
    ...
    preserved={mesh.name:before[mesh]==hashlib.sha256(geometry_uv_bytes(mesh)).hexdigest() and tuple(mesh.materials)==source_materials[mesh] for mesh in source_meshes}
    ...
    if success: RECEIPT.write_text(json.dumps(report,indent=2)+'\n')
