/** Isolated round29 candidate. No runtime imports or mutable source references. */
...
export function createPitPadField(placements: readonly PitPlacement[], route: readonly RoutePoint[],
  ...
  const pads = placements.filter(p => p.family === 'pit-complex').map(p => {
    ...
    for (let i = 0; i <= Math.ceil(pad.halfX * 2); i++) for (const side of [-1, 1]) {
      ...
      minimum = Math.min(minimum, roadClearance(segments, x, z));
    }
    ...
    return { ...pad, minRoadClearance: minimum, accepted: minimum > PIT_PAD_LANE_MARGIN + 1.15 + .5 };
  });
  ...
  // Candidate has three distant sites. Query only local route segments at bake time.
  const reach = PIT_PAD_LANE_MARGIN + PIT_PAD_FILTER_MARGIN + PROTECTION_FADE;
  const localSegments = segments.filter(s => s.ax >= minX - reach - s.width - Math.abs(s.dx)
    && s.ax <= minX + columns + reach + s.width + Math.abs(s.dx)
    && s.az >= minZ - reach - s.width - Math.abs(s.dz)
    && s.az <= minZ + rows + reach + s.width + Math.abs(s.dz));
  for (let row = 1; row < rows - 1; row++) for (let column = 1; column < columns - 1; column++) {
    const x = minX + column, z = minZ + row;
    let selected: PitPad | undefined, weight = 0;
    for (const pad of active) {
      const w = 1 - smooth((padDistance(pad, x, z) - PIT_PAD_COLLAR) / PIT_PAD_APRON);
      if (w > weight) { selected = pad; weight = w; }
    }
    if (!selected || weight === 0) continue;
    weight *= smooth((roadClearance(localSegments, x, z) - radius) / PROTECTION_FADE);
