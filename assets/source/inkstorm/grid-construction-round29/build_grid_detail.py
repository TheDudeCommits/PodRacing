...
TASK_LIGHTS = {
    ...
    'pit-district': [(-36,4,8.08),(1,8,13.08),(41,1,9.88)],
}
...
def cloth_shell(m,x,y,w,d,height,color,kind,seed=0):
    # A 3 cm textile edge, enclosing the unchanged authored predecessor.
    # District predecessor opposite faces used BOTH diagonals of each warped cell.
    # Their geometric envelope is retained inside upper/lower diagonal skins.
    nx,ny=(8,5) if kind=='pit' else (12,7)
    verts=[]
    for thickness in [.015,-.015]:
        for j in range(ny+1):
            for i in range(nx+1):
                u=i/nx;v=j/ny
                if kind=='pit':
    ...
    for j in range(ny):
        for i in range(nx):
            a=j*(nx+1)+i
            diagonal_a=[(a,a+1,a+nx+2),(a,a+nx+2,a+nx+1)]
            diagonal_b=[(a,a+1,a+nx+1),(a+1,a+nx+2,a+nx+1)]
            top,bottom=diagonal_a,diagonal_a
            if kind!='pit':
                a_high=verts[a][2]+verts[a+nx+2][2]>=verts[a+1][2]+verts[a+nx+1][2]
                top=diagonal_a if a_high else diagonal_b
                bottom=diagonal_b if a_high else diagonal_a
            paint=color
            if kind=='pit':
                # Restore the original two-column cream end repair on every pit.
                if i>=6:paint='cream'
            else:
                # Original district repair layout and restrained worn front panels.
                if (i+seed)%7==0:paint='cream'
                f=math.sin((i*3+j*7+seed)*12.9898+78.233)*43758.5453
                if j>ny-3 and f-math.floor(f)>.77:paint='rust'
            base=PAINT[paint]
            part(m,verts,[tuple(reversed(face)) for face in top],base)
            part(m,verts,[tuple(layer+k for k in face) for face in bottom],tuple(c*.68 for c in base))
    ...
    for k,a in enumerate(perimeter):
        ...
        paint='cream' if kind=='pit' and min(a%(nx+1),b%(nx+1))>=6 else color
        part(m,verts,[(a,b,layer+b,layer+a)],tuple(c*.82 for c in PAINT[paint]))
    # Fine stitched seams sit in the cloth instead of a raised metal binding.
    for i in range(0,nx+1,3 if kind=='pit' else 6):
        path=[tuple(Vector(verts[j*(nx+1)+i])+Vector((0,0,.004))) for j in range(ny+1)]
        thread=tuple(c*.8 for c in PAINT['cream'])
        pipe(m,path,.008,thread,4)
    if kind=='pit':
        for i in range(nx):
            # A small textile roll; district already has its original fine front hem.
            a=ny*(nx+1)+i
            front=[tuple(Vector(verts[k])+Vector((0,0,-.003))) for k in [a,a+1]]
            paint='cream' if i>=6 else color
            pipe(m,front,.01,tuple(c*.88 for c in PAINT[paint]),4)
...
def export_overlay(scene,name,data):
    ...
    mesh=bpy.data.meshes.new(name+'-grid-construction-r29-v4-overlay')
    ...
    material=bpy.data.materials.new(name+'-grid-r29-v4-paint')
    ...
    obj=bpy.data.objects.new(name+'-grid-r29-v4-overlay',mesh);scene.collection.objects.link(obj)
...
scene=bpy.data.scenes.new('Inkstorm Grid Construction Round29 v4 thin repaired cloth')
try:
