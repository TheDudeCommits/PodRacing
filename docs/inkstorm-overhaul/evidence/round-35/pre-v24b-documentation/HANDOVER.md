...
Use [Round 35 implementation and current QA](docs/inkstorm-overhaul/ROUND35_UI_COMBAT_HANDOVER.md) as the active checkpoint. It distinguishes build verification, ordinary browser flows, actual combat footage, independent criticism and performance evidence. The previous overview below is historical. The final UI/performance round is still being checked as this checkpoint is written.
