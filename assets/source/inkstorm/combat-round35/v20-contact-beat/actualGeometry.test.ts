...
describe('actual authored Teemto damage packages', () => {
  ...
  it.each(variants)('%s contains every visible source vertex through actual wreck trajectory and side/chase camera aspects', name => {
    ...
    let firstContact: { frame: number; simulationAge: number; pacedWallMs: number } | null = null;
    ...
    try {
      for (let frame = 1; frame <= 1168; frame++) {
        ...
        if (pose.groundContact) { contactSamples++; firstContact ??= { frame, simulationAge: WRECK_PRESENTATION_DURATION - entry.galactic!.wreck.timer, pacedWallMs: (WRECK_PRESENTATION_DURATION - entry.galactic!.wreck.timer) / .18 * 1000 }; }
      ...
      console.info('V20 actual authored damage geometry support/projection:', JSON.stringify({ name, positionsTested,
        cameraPositions, contactSamples, firstContact, maxQueries, settledMaximumGap: Object.fromEntries(settledMaximumGap), minimumGapByMesh: Object.fromEntries(statistics) }));
