...
for (let j = 0; j <= cloth.ny; j++) for (let i = 0; i <= cloth.nx; i++) {
  ...
  topPoints.push(glb([cloth.x + (u - .5) * cloth.w, cloth.y - v * cloth.d, z + .09]));
  bottomPoints.push(glb([cloth.x + (u - .5) * cloth.w, cloth.y - v * cloth.d, z - .16]));
}
...
const expectedClothVolume = cloth.w * cloth.d * .25;
