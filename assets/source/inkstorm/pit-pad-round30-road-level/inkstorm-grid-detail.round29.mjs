...
try {
  ...
  for (let index = 0; index < count; index++) {
    const view = await page.evaluate(({ index, family, diagnosticPose }) => {
      ...
      const pose = () => { camera.position.copy(position); camera.fov = 50; camera.near = .35;
    ...
    }, { index, family, diagnosticPose: poses[family] });
  ...
  await writeFile(`${out}/receipts.json`, JSON.stringify({ scope: 'Actual GameApp rendering with diagnostic camera; Vite source, not frozen bundle or FPS evidence. Camera poses anchored to unchanged terrain, no material or geometry overrides.',
