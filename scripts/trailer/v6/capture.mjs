...
const root='output/trailer-v6';await mkdir(`${root}/takes`,{recursive:true});await mkdir(`${root}/stills`,{recursive:true});
...
let browser,encoder;let loadedPod='';const errors=[];
try{
 ...
 for(const shot of shots){
  ...
  const wantedPod=(shot.localPod??'teemto')+':'+shot.biome;
  if(wantedPod!==loadedPod){
   await page.evaluate(p=>localStorage.setItem('now-this-is-podracing.vehicle-appearance',JSON.stringify({version:1,appearance:p})),shot.localPod??'teemto');
   ...
   await page.waitForFunction(()=>!document.querySelector('[data-action="start-race"]').disabled);
   await page.locator('[data-action="start-race"]').click();
   await page.waitForFunction(()=>!document.querySelector('.pod-hud').classList.contains('has-vehicle-selection'));
  ...
  for(let f=0;f<frames;f++){
   const t=await page.evaluate(({s,f,frames})=>{
    ...
    return {...r,f,time:f/60,biome:snap.game.course?.biome,ability:me.galactic.ability,position:me.position,progress:me.courseProgress,
