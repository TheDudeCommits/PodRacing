...
/** Closed rounded sand core: flat base and scalloped shoulders, 256 triangles.
 * Built once; active dust never rescans or rewrites its source geometry. */
export function createContactDustGeometry(): BufferGeometry {
  const sides = 16, bands = 8, positions: number[] = [], indices: number[] = [];
  for (let band = 0; band < bands; band++) {
    const phi = band / bands * Math.PI * .5;
    for (let side = 0; side < sides; side++) {
      const theta = side / sides * Math.PI * 2;
      // Broad overlapping billows interrupt the shoulder and crown. The
      // envelope remains within +/-1.06 horizontally and [0,1] vertically.
      const shoulder = .84 + .12 * Math.sin(theta * 3 + phi * 5)
        + .075 * Math.sin(theta * 5 - phi * 7);
      const radius = Math.cos(phi) * shoulder;
      const crown = Math.sin(phi) * (1 - .11 * Math.sin(phi * 2)
        * (.5 + .5 * Math.sin(theta * 3 - phi * 4)));
      positions.push(Math.cos(theta) * radius, crown, Math.sin(theta) * radius);
    }
  }
  const top = positions.length / 3; positions.push(0, 1, 0);
  const base = positions.length / 3; positions.push(0, 0, 0);
  for (let side = 0; side < sides; side++) {
    const next = (side + 1) % sides;
    for (let band = 0; band < bands - 1; band++) {
      const a = band * sides + side, b = band * sides + next;
      indices.push(a, a + sides, b, b, a + sides, b + sides);
    }
    indices.push((bands - 1) * sides + side, top, (bands - 1) * sides + next);
    indices.push(base, side, next);
  }
