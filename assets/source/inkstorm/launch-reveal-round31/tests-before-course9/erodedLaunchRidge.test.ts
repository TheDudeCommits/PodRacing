...
describe('eroded launch cliff composition', () => {
  ...
  it('retains the previous central negative bowl and the entire finish grid byte for byte', async () => {
    // Frozen round-21 physical evidence, taken before erosion source changes.
    // The existing separate 532,296-sample historical SHA protects all racing
    ...
    expect(await sha256(new Uint8Array(new Float64Array(floor).buffer)))
      .toBe('f3f00afd1e3cb4d1c5d4726485edb16ee259b28555baee125d3c35820849c727');
