...
const receipt = { outcome: 'FAIL', startedAt: new Date().toISOString(), output: relative(repo, output),
  ...
  scope: 'Time Attack hero lifecycle: public UI selection, decoded garage images and inspection angles, late-load cancellation, reload persistence, class/record invariants, five seconds of ordinary input, body-shadow submission and injected HTTP 503 fallback/Retry. Then a separate public UI switch to Canyon Cup for actual Sola rival import/admission. No full-race, FPS, human driving or visual-art acceptance.',
...
async function polwoRivalReady() {
  await page.waitForFunction(() => {
    const snap = window.__PODRACING__?.snapshot();
    const rival = snap?.game.vehiclePresentation?.racers[4];
    return snap?.galactic.racers.length === 8 && snap.galactic.racers[4]?.id === 'ai-sola'
      && rival?.requested === 'polwo' && rival.active === 'polwo' && rival.status === 'ready';
...
try {
  ...
  receipt.rivalAdmissionScope = 'No AI entry or rival browser download is expected during Time Attack. After the hero lifecycle completes, an explicit Canyon Cup UI selection must create eight actual entries including ai-sola at index 4 and admit its pinned rival package. Visibility/LOD are recorded without claiming a complete race or individual GPU draw timing.';
  ...
  assert(receipt.errors.every(error => error.expected), `Unexpected hero lifecycle errors: ${JSON.stringify(receipt.errors.filter(error => !error.expected))}`);
  const completedTrial = await snapshot();
  assert(completedTrial.game.mastery.eventId === 'inkstorm-trial' && completedTrial.galactic.racers.length === 1,
    'Hero lifecycle did not remain in the one-entry Time Attack event');
  assert(!receipt.requests.some(request => request.url === `${origin}${rivalAssetPath}`), 'Time Attack downloaded an absent Polwo rival');
  receipt.heroLifecycle = { outcome: 'PASS', completedAt: new Date().toISOString(),
    identity: baselineIdentity, masteryBytesUnchanged: true, trialEntries: 1, rivalBrowserRequests: 0 };

  // Deliberate event transition, after every hero appearance invariant passed.
  // Canyon Cup shares the fixed seed/course but changes profile/laps and adds
  // actual AI entries; never preload an absent rival just to satisfy this test.
  stage = 'cup-rival-admission';
  const cupIdentity = { ...baselineIdentity, ordinal: baselineIdentity.ordinal + 1,
    selectedLaps: 2, eventId: 'cup-canyon', competitionProfile: 'clean-race' };
  await page.locator('[data-action="select-event"][data-event-id="cup-canyon"]').click();
  await page.waitForFunction(() => {
    const game = window.__PODRACING__.snapshot().game;
    return game.awaitingStart && game.mastery.eventId === 'cup-canyon';
  });
  await verifyIdentityTransition('public-canyon-cup-selection', cupIdentity);
  await polwoReady(); await polwoRivalReady();
  await verifyUnchanged(stage); await recordStage('cup-polwo-rival-admitted');
  await Promise.all([...responseTasks]);
  assert(receipt.servedAssets.some(row => row.status === 200 && row.url === `${origin}${rivalAssetPath}`
    && row.stage === stage && row.sha256 === receipt.rivalAsset.sha256), 'Canyon Cup did not load the pinned Polwo rival payload');
