...
for (const family of ['pit-complex', 'pit-district']) {
  ...
  const supportPath = family === 'pit-district'
    ? path.join(stage, 'versions/v1/candidates', family + '.glb')
    : path.join(stage, 'predecessors', family + '.glb');
  const original = await triangles(supportPath);
  for (const fixture of anchors.fixtures.filter(f => f.family === family)) {
    ...
    fixtures.push({ id: fixture.id, family, emitterGLB: fixture.emitterGLB, undersideTriangles: faces.length,
      ...
      supportSource: path.relative(stage, supportPath) });
  }
}
