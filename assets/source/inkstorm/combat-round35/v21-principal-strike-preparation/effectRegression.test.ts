...
import {
  ...
  GalacticEffectsView,
} from '/Users/amir/Projects/PodRacing/src/render/galactic/GalacticEffectsView';
...
describe('localized wreck rupture', () => {
  ...
  it('uses the sampled engine groundpoint for readable sparks, a low scrape, and a raised sand fan', () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Contact sparks must remain streaks while being wide enough to read
      // beside the imported engine; the former >20 ratio forced subpixel width.
      expect(size.z / size.x).toBeGreaterThan(10);
      expect(size.x).toBeGreaterThanOrEqual(.34 * .26 - 1e-7);
    ...
    for (let i = 0; i < 2; i++) {
      ...
      expect(size.y / size.x).toBeCloseTo(i === 0 ? .18 : .54, 5);
      expect(surface.getX(i)).toBe(4);
