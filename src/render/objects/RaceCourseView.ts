...
const pylonVertex = /* glsl */ `
  ...
  void main() {
    ...
    // Height grows from the local base, so extreme-distance markers retain a
    // coherent dark stem and attached green cap instead of becoming specks.
    // The 900 m activation leaves nearby pylons exactly unchanged.
    float heightScale = 1.0
      + smoothstep(900.0, 2850.0, vDistanceToCamera) * uFarHeightGain;
    shapedPosition.y = (shapedPosition.y + uHalfHeight) * heightScale - uHalfHeight;
...
export class RaceCourseView extends Group {
  ...
  setCourse(course: CourseRenderData): void {
    ...
    this.buildBranches(course.branches ?? []);
    this.buildPylons(course.points);
  ...
  private buildRibbon(
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const centerHeight = sampleTerrainHeight(point.x, point.z) + 0.09;
      for (let side = 0; side < 2; side += 1) {
        ...
        positions[vertex * 3 + 1] = sampleTerrainHeight(lateralX, lateralZ) + 0.09;
  ...
  private buildFarRouteGeometry(
    ...
    for (let index = 0; index < segmentCount; index += 1) {
      ...
      const startY = sampleTerrainHeight(start.x, start.z) + 0.16;
      const endY = sampleTerrainHeight(end.x, end.z) + 0.16;
   ...
   */
  private buildBranches(branches: readonly CourseRenderBranch[]): void {
    ...
    this.branchBeacons = new InstancedMesh(
      new CylinderGeometry(0.42, 1.25, 7.2, 4),
      beaconMaterial,
      visible.length * 2,
    );
    this.branchBeacons.name = 'alternate-route-entry-exit-beacons';
    this.branchBeacons.userData[CEL_POST_EXCLUDE_USER_DATA_KEY] = true;
    let instance = 0;
    for (const branch of visible) {
      const endpoints = [branch.points[0], branch.points.at(-1)];
      for (const endpoint of endpoints) {
        if (!endpoint) continue;
        position.set(
          endpoint.x,
          sampleTerrainHeight(endpoint.x, endpoint.z) + 3.6,
          endpoint.z,
        );
        rotation.identity();
        scale.set(1, 1, 1);
  ...
  private buildPylons(points: readonly CourseRenderPoint[]): void {
    ...
    const bodyMaterial = new ShaderMaterial({
      ...
      uniforms: {
        ...
        uHalfHeight: { value: 4.8 },
        uFarHeightGain: { value: 1.72 },
    ...
    const lightMaterial = new ShaderMaterial({
      ...
      fragmentShader: pylonFragment,
      uniforms: {
        uColor: { value: new Color('#58ff89') },
        uFarWidthGain: { value: 1.18 },
        uHalfHeight: { value: 1.75 },
        uFarHeightGain: { value: 1.18 },
        uDetailFadeStart: { value: 420 },
        uDetailFadeEnd: { value: 670 },
      },
      transparent: true,
    ...
    this.pylonBodies = new InstancedMesh(new ConeGeometry(0.92, 9.6, 5), bodyMaterial, count);
    this.pylonLights = new InstancedMesh(new CylinderGeometry(0.34, 0.34, 3.5, 5), lightMaterial, count);
    const farMarkerMaterial = new ShaderMaterial({
      ...
      uniforms: {
        ...
        uLightColor: { value: new Color('#58ff89') },
        uViewportSize: { value: this.viewportSize },
    ...
    for (let index = 0; index < points.length; index += stride) {
      ...
      for (const side of [-1, 1]) {
        ...
        position.y = sampleTerrainHeight(pylonX, pylonZ) + 0.16;
        matrix.compose(position, rotation, scale);
  ...
  private buildGates(course: CourseRenderData): void {
    ...
    const ink = new MeshBasicMaterial({ color: '#1b1524' });
    const glow = new MeshBasicMaterial({ color: '#7dff9f', depthWrite: false });
    this.gatePosts = new InstancedMesh(new BoxGeometry(1.45, 14.5, 1.45), ink, gateCount * 2);
    this.gateBars = new InstancedMesh(new BoxGeometry(1, 1, 1), glow, gateCount);
    this.gatePosts.userData[CEL_POST_EXCLUDE_USER_DATA_KEY] = true;
    this.gateBars.userData[CEL_POST_EXCLUDE_USER_DATA_KEY] = true;
    ...
    for (let gate = 0; gate < gateCount; gate += 1) {
      ...
      for (const side of [-1, 1]) {
        position.set(point.x + right.x * gateWidth * side, point.y + 7.15, point.z + right.z * gateWidth * side);
        scale.set(1, 1, 1);
        matrix.compose(position, rotation, scale);
      ...
      scale.set(gateWidth * 2 + 1.45, 0.9, 0.9);
      matrix.compose(position, rotation, scale);
