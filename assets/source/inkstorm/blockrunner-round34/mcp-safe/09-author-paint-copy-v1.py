...
and all triangle surfaces/order are retained. No across-object welding.
"""
from mathutils import Matrix

PAINT_SCENE = 'PodRacing — Blockrunner Inkstorm atlas paint round34 V1'
...
def assemble(name, rows, materials, pilot=False):
    ...
    for row in rows:
        ...
        for polygon in mesh.polygons:
            ...
            expected_corners.extend(tuple(world @ mesh.vertices[i].co) for i in polygon.vertices)
            normals.extend(corner_normals[i] for i in polygon.loop_indices)
            if pilot:
        ...
        ranges.append({'sourceObject': row['sourceObject'], 'fitObject': original.name,
                       ...
                       'polygonCount': len(mesh.polygons), 'sourcePolygonOrderRetained': True,
    ...
    report.setdefault('consolidation', []).append({'object': name, 'vertices': len(vertices),
        ...
        'triangleSurfaceAndOrderExact': True, 'maximumNormalEncodingDifference': normal_error,
...
try:
    ...
    body_materials = [
        paint_material('Blockrunner V1 procedural oxblood', linear_hex('#A74432'), .67, .10, 1),
        paint_material('Blockrunner V1 procedural ivory', linear_hex('#D8C9AC'), .73, .055, .75),
        paint_material('Blockrunner V1 procedural graphite', linear_hex('#303744'), .75, .07, .5)]
    pilot_materials = []
    ...
    body = assemble('blockrunner-body-paint-v1', [r for r in BLOCKRUNNER_FIT['objectLineage'] if r not in pilot_row], body_materials)
    pilot = assemble('blockrunner-pilot-paint-v1', pilot_row, pilot_materials, pilot=True)
