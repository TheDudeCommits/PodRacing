...
function analyze(observation, test) {
  ...
  return { outcome: issues.length ? 'FAIL' : 'PASS', issues, explosion, wreck, estimatedStartMs, before, slow, after,
    restoredCamera: test.camera, samples: samples.length, cadence };
}
...
async function runCase(test) {
  ...
  const capture = async (name) => {
    ...
    result.captures.push({ name, path, snapshot: await snapshot(page) });
  };
  try {
    ...
    await page.keyboard.up('s'); result.after = await snapshot(page);
    ...
    await page.keyboard.up('Shift').catch(() => {}); await page.keyboard.up('s').catch(() => {});
