...
export class RaceHud {
  ...
  constructor(mount: HTMLElement, options: RaceHudOptions = {}) {
    ...
    this.root.innerHTML = /* html */ `
      ...
      <section class="pod-hud__speed" aria-label="Speed">
        <div class="pod-hud__speed-ring" data-hud="speed-ring"><svg class="pod-hud__speed-dial" viewBox="0 0 185 210" aria-hidden="true"><path class="dial-glass" d="M38 2H115C150 2 178 30 178 65V144C178 177 151 202 118 202H18C8 202 2 196 2 186V119C2 109 8 101 18 97C8 89 2 82 2 75V64C2 45 18 30 38 30Z"/><path class="dial-outline" d="M38 2H115C150 2 178 30 178 65V144C178 177 151 202 118 202H18C8 202 2 196 2 186V119C2 109 8 101 18 97C8 89 2 82 2 75V64C2 45 18 30 38 30Z"/><path class="dial-divider" d="M16 105H115C138 105 151 93 151 74"/><path class="dial-track" pathLength="100" d="M18 202H118C151 202 178 177 178 144V65C178 30 150 2 115 2H38"/><path class="dial-charge" pathLength="100" d="M18 202H118C151 202 178 177 178 144V65C178 30 150 2 115 2H38"/><path class="dial-heat-edge" d="M178 130V146C178 158 174 169 168 178"/><circle class="dial-key-socket" cx="36" cy="12" r="10"/></svg></div>
  ...
  private updateThreatCues(
    ...
    if (key !== this.threatStructureKey) {
      ...
      this.threatCues.replaceChildren(...visibleThreats.map(({ threat, count }, index) => {
        const isImpactVector = threat.kind === 'impact' && threat.label === 'IMPACT VECTOR';
        const directionOnly = isImpactVector && impactCaptionUsed;
