...
GEOMETRY = HERE / 'teemto-open-cockpit-round31-geometry.glb'
TARGET = HERE / 'teemto-open-cockpit-round31-candidate.glb'
...
if __name__ == '__main__':
    ...
    target['materials'] += [
        {'name': 'Inkstorm Teemto open cockpit graphite coaming',
         'pbrMetallicRoughness': {'baseColorFactor': [.028, .041, .052, 1], 'metallicFactor': .36, 'roughnessFactor': .65}},
        {'name': 'Inkstorm Teemto open cockpit bone return',
         'pbrMetallicRoughness': {'baseColorFactor': [.70, .62, .49, 1], 'metallicFactor': .12, 'roughnessFactor': .73}},
    ]
    for mesh in geometry['meshes']:
        ...
        for primitive in item['primitives']:
            ...
            name = mesh.get('name', '')
            primitive['material'] = 0 if name == 'teemto-open-cockpit-hull' else frame_index + (1 if 'bone-return' in name else 0)
    ...
    for node in geometry['nodes']:
        ...
        name = node.get('name', '')
        if name == 'teemto-open-cockpit-hull':
