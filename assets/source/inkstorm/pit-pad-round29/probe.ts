...
const dir = 'assets/source/inkstorm/pit-pad-round29';
...
const cameras = JSON.parse(readFileSync('output/gauntlet/grid-grounding-round29-before/receipts.json', 'utf8')).views.map((v: any) => v.position);
...
function meshCeiling(terrain: TerrainSystem, camera: number[], pad: PitPad, heightAt: (x: number, z: number) => number) {
  ...
  return { triangles, maxLod, maxHeight, maxAboveSlabBottom: maxHeight - pad.anchorHeight - 1.45 };
}
const reports = [];
for (const seed of [0x494e4b53, 42, 1234]) {
  ...
  for (const branch of course.branches) for (let i = 0; i < branch.points.length - 1; i++) {
    ...
    for (const t of [0, .123, .5, .877, 1]) {
      ...
      for (const lateral of [-width - 1.15, -width, 0, width, width + 1.15]) {
        ...
        maxCorridorDelta = Math.max(maxCorridorDelta, Math.abs(field.sampleOffset(x, z))); corridorProbeCount++;
      }
    }
  }
  ...
  const pads = field.pads.map((pad, padIndex) => {
    let samples = 0, minHeight = Infinity, maxHeight = -Infinity, beforeMax = -Infinity, maxTargetError = 0, aboveSlabBottom = 0, modifiedSamples = 0;
    for (let ix = 0; ix <= 150; ix++) for (let iz = 0; iz <= 61; iz++) {
      ...
      const old = before(x, z), h = after(x, z); samples++;
      ...
      aboveSlabBottom += Number(h > pad.anchorHeight + 1.45); modifiedSamples += Number(Math.abs(h - old) > .001);
    }
    const apron = [];
    ...
    const visualMesh = terrain ? meshCeiling(terrain, cameras[padIndex]!, pad, after) : null;
    const meshPhaseChecks = terrain ? [[.75, 0], [0, .75], [3, 3], [-3, -3]].map(([dx, dz]) => {
      const camera = [...cameras[padIndex]!]; camera[0] += dx!; camera[2] += dz!;
      return { dx, dz, ...meshCeiling(terrain, camera, pad, after) };
    }) : [];
    return { ...pad, samples, minHeight, maxHeight, beforeMax, beforeAboveSlabBottom: beforeMax - pad.anchorHeight - 1.45,
      maxAboveSlabBottom: maxHeight - pad.anchorHeight - 1.45, maxTargetError, aboveSlabBottom, modifiedSamples,
      savedAnchorDelta: field.anchorHeight(pad.id)! - before(pad.x, pad.z),
  ...
  for (const pad of pads) {
    ...
    if (pad.accepted) assert.equal(pad.aboveSlabBottom, 0, `${seed}/${pad.id} has terrain inside slab`);
    ...
    for (const edge of pad.apron) assert.equal(edge.outsideDelta, 0);
  }
  ...
  reports.push({ seed, signature: course.signature, bakeMs, grid: { ...g, values: undefined, sha256: gridHash, bytes: bytes.length, minOffset, maxOffset, changedTexels },
