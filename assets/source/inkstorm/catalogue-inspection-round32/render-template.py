...
OUT='/Users/amir/Projects/PodRacing/assets/source/inkstorm/catalogue-inspection-round32/'+UID
...
try:
 ...
 center=(low+high)*.5;extent=max(high-low);assert 0<extent<10000
 ...
 camera.data.type='ORTHO';camera.data.ortho_scale=extent*1.5;camera.data.clip_start=.01;camera.data.clip_end=extent*10
 ...
 for name,direction in [('quarter-a',(1.3,-1.7,1.1)),('quarter-b',(-1.3,1.7,1.1))]:
 ...
 print(json.dumps({'uid':UID,'sourceScene':SOURCE,'scope':'Original imported source geometry/materials at source frame; isolated inspection lighting. Not a stylized asset, in-game capture, occupancy verdict or FPS evidence.','bounds':[list(low),list(high)],'sourceFrame':source.frame_current,'images':images}))
finally:
