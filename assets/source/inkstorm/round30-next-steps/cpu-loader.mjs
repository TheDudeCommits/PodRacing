...
const candidate=path.join(root,'assets/source/inkstorm/round30-next-steps/candidate');
export async function resolve(specifier, context, next){
 let parent=context.parentURL;
 if(parent?.startsWith(pathToFileURL(candidate).href)&&specifier.startsWith('.')) parent=pathToFileURL(path.join(root,'src/render/inkstorm',path.basename(fileURLToPath(parent)))).href;
