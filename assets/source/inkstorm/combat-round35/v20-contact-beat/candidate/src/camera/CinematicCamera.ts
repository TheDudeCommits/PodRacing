...
export class CinematicCamera {
  ...
  /** Live wrecks need space for the full vehicle and its separating engines. */
  setCombatFraming(enabled: boolean): void {
    if (enabled && !this.combatFraming) this.hasAuthoredCombatBack = false;
  ...
  snap(subject: CameraSubject): void {
    if (!this.combatFraming && !subject.wreckChase) this.hasAuthoredCombatBack = false;
  ...
  update(dt: number, time: number, subject: CameraSubject): void {
    ...
    if (boundedWreck && this.hasCombatBounds(subject)) {
      ...
      const distance = this.boundsOffset.copy(this.camera.position).sub(subject.combatFocus!).dot(this.boundsBack);
  ...
  private combatDirection(subject: CameraSubject, forward: Vector3, right: Vector3): void {
    ...
    if (authored && this.hasAuthoredCombatBack) {
  ...
  private fitCombatBounds(subject: CameraSubject, direction: Vector3, minimumDistance = 0): void {
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      for (const point of points) {
        ...
        // Exact perspective half-spaces: x in +/-.78 and y in [-.58,.70].
        // The .06 upward screen bias reserves caption space below the craft.
