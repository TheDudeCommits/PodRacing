...
export class RaceHud {
      ...
      <section class="pod-hud__vehicle-select" data-hud="vehicle-selection" aria-label="Select your vehicle" aria-hidden="true">
        <div class="pod-hud__vehicle-select-frame">
          <header class="pod-hud__vehicle-select-head">
            <div><span class="pod-hud__eyebrow">INKSTORM / RACING LEAGUE</span><h1><small>NOW THIS IS</small> PODRACING</h1></div>
          ...
          <section class="pod-hud__garage-hero" aria-label="Inspect selected vehicle">
            <span class="pod-hud__garage-bay">HANGAR 07 <i></i> RACE PREPARATION</span>
            <div class="pod-hud__appearance" data-hud="appearance" hidden>
              <div role="group" aria-label="Podracer appearance"><span>FRAME</span>${Object.values(ART_APPEARANCES).map(art => `<button type="button" data-action="select-appearance" data-appearance="${art.id}" aria-pressed="false">${art.label}</button>`).join('')}<small>Same handling</small></div>
          ...
          <aside class="pod-hud__mastery" data-hud="mastery" aria-label="Event and records">
            <span class="pod-hud__eyebrow">THE NEXT CHALLENGE</span>
          ...
          <section class="pod-hud__workshop" data-hud="workshop" aria-label="Podracer workshop" aria-hidden="true">
            <header class="pod-hud__workshop-head">
              <div><span>INKSTORM / WORKSHOP</span><strong>Build your advantage.</strong></div>
            ...
            <div class="pod-hud__workshop-body">
              ...
              <aside class="pod-hud__workshop-summary" aria-label="Build summary">
                <div class="pod-hud__workshop-effective"><strong>Effective build / from base</strong><ul data-hud="workshop-totals"></ul></div>
  ...
  private updateWorkshop(workshop: HudWorkshopViewModel | undefined): void {
    ...
    this.workshopParts.replaceChildren(...parts.map((part) => {
      ...
      button.innerHTML = '<strong></strong><p></p><span class="is-benefit"></span><span class="is-tradeoff"></span><small class="pod-hud__part-comparison"></small><b class="pod-hud__equip-action"></b>';
  ...
  private updateMastery(mastery: HudMasteryViewModel | undefined, phase: RaceHudViewModel['phase'], preRace: boolean, tutorialVisible: boolean): void {
    ...
    write(requireElement(this.root, '[data-hud="mastery-objective"]'), mastery.storageWarning
      ?? (mastery.retryTarget ? `Next pursuit: ${mastery.retryTarget.label} · S${mastery.retryTarget.sectorIndex}. Last run lost ${mastery.retryTarget.loss.toFixed(2)}s against your previous PB here.`
    ...
    for (const event of this.root.querySelectorAll<HTMLButtonElement>('[data-action="select-event"]')) {
    ...
    if (mastery.latestSector && phase === 'racing') {
      ...
      write(this.split, pace === null ? `S${sector.index} ${formatRaceTimeHundredths(sector.time)}` : `PB ${formatSplitDelta(pace)}`);
    ...
    write(requireElement(head, 'span'), `TOTAL ${formatRaceTimeHundredths(result.time)}`);
    ...
    if (result.retryTarget) {
      ...
      pursuit.textContent = `NEXT PURSUIT · ${result.retryTarget.label} · S${result.retryTarget.sectorIndex}: +${result.retryTarget.loss.toFixed(2)}s against your previous PB. Race again to recover this section.`;
