  ...
  result.onBeforeCompile = (shader) => {
    ...
    `).replace('#include <color_fragment>', `
      ...
      if (vEffectSurface.x > 1.5) {
        ...
        float core = 1.0 - smoothstep(0.22, ${RUPTURE_FLASH_CORE_RADIUS}, radius);
  ...
  emitCrash(event: CrashEffectEvent): void {
    ...
    for (let fragment = 0; fragment < fragmentCount; fragment += 1) {
      ...
      const radial = contactBurst
        ? ((spark ? 20 : 10) + hash01(seed + 1) * (spark ? 16 : 10)) * Math.sqrt(severity)
    ...
    for (let plate = 0; plate < plateCount; plate += 1) {
      ...
      if (contactBurst) {
        ...
        slot.duration = [0.22, 0.42, 0.9, 1.1][plate]!;
        slot.x = finite(event.position.x, 0);
  ...
  private updateCrash(time: number): void {
    ...
    for (const slot of this.explosionSlots) {
      ...
      if (slot.contactLayer >= 0) {
        ...
        if (!this.isVisible(slot.x, y, slot.z, size * (flash ? 1.7 : 1.42), EFFECT_VISIBILITY_DISTANCE.crashes)) continue;
        const opacity = (smoke ? 0.5 : flash ? 1 : 0.68)
        ...
        this.writeInstance(this.explosionPlates, plateCount,
          slot.x + this.billboardToCamera.x * front,
          y + Math.max(0, this.billboardToCamera.y) * front,
          slot.z + this.billboardToCamera.z * front,
