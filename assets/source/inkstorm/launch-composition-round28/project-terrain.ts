...
const dir='assets/source/inkstorm/launch-composition-round28',mode=process.argv[2];
assert(['baseline','candidate'].includes(mode));
const frozen=JSON.parse(readFileSync(`${dir}/baseline-field.json`,'utf8'));
...
const results=[],landformResults=[];
const landforms=mode==='candidate'?JSON.parse(readFileSync(`${dir}/landforms.json`,'utf8')).accepted:[];
for(const saved of frozen.cameras){
 ...
 for(const form of landforms){
  const probes=form.visibilityProbePoints.map((values:number[])=>{
   const p=new Vector3().fromArray(values),view=p.clone().applyMatrix4(camera.matrixWorldInverse);
   if(view.z>=-camera.near)return {world:values,visible:false,reason:'behind-or-near-clipped'};
   const ndc=p.clone().project(camera),u=(ndc.x+1)/2,v=(1-ndc.y)/2;
   if(u<0||u>1||v<0||v>1||ndc.z>1)return {world:values,visible:false,reason:'outside-frame'};
   const terrainHit=cast(u,v),distance=p.distanceTo(camera.position),clearance=terrainHit?terrainHit.distance-distance:null;
   return {world:values,u,v,visible:clearance===null||clearance>-.8,reason:clearance===null||clearance>-.8?'terrain-ray-clear':'terrain-occluded',terrainDistance:terrainHit?.distance??null,pointDistance:distance,terrainClearance:clearance};
  });
  landformResults.push({id:form.id,camera:saved.id,probes,visibleProbes:probes.filter((p:any)=>p.visible).length});
 }
