...
describe('Owned rolling rear-contact volume', () => {
  it.each(variants)('%s uses real current terrain under its complete closed dust geometry with fixed query cost', name => {
    ...
    try {
      expect(geometry.index!.count / 3).toBe(256);
      // Fixed nine-probe/cull envelope must cover every closed subcap.
      for (const local of source) {
        expect(Math.abs(local.x)).toBeLessThanOrEqual(1.06);
        expect(Math.abs(local.z)).toBeLessThanOrEqual(1.06);
      ...
      for (const age of [0, .058333333333, .16, .408333333333, 1.208333333333, 1.64]) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          expect(geometry.getAttribute('aContactAge').getX(i)).toBeCloseTo(age, 5);
          for (const local of source) {
