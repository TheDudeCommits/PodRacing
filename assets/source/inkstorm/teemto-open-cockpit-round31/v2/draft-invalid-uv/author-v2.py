...
STAGE='PodRacing — Teemto open cockpit round31 V2'
...
try:
    ...
    def make(name,verts,faces,group,patch=None):
        ...
        if patch:
            ...
            for loop in data.loops:
                ...
                u=(v.x-xmin)/max(.001,xmax-xmin);w=(v.y-ymin)/max(.001,ymax-ymin)
                uv.data[loop.index].uv=(patch[0]+u*(patch[1]-patch[0]),patch[2]+w*(patch[3]-patch[2]))
    ...
    loft('V2 tapered rear shoulder and spine fairing',tail)
    loft('V2 low tapered forward cowling',nose)
    ...
    def surface(rows,x,z):
        width,top,bottom=interpolate(rows,z);f=min(1,abs(x)/max(.001,width))
        if f<.6:t=1-.015*f/.6
        elif f<.92:t=.985-.165*(f-.6)/.32
        else:t=.82-.27*(f-.92)/.08
        return bottom+t*(top-bottom)
    def panel(name,points,rows):
        center=(sum(x for x,z in points)/len(points),sum(z for x,z in points)/len(points))
        for scale,lift,group,patch,suffix in [(1,.008,dark,None,'recessed gasket'),(.925,.011,shell,blue,'painted panel')]:
            verts=[]
            for x,z in points:
                x=center[0]+(x-center[0])*scale;z=center[1]+(z-center[1])*scale
                verts.append((.025+x,-z,surface(rows,x,z)+lift))
            verts.append((.025+center[0],-center[1],surface(rows,center[0],center[1])+lift))
            faces=[(i,(i+1)%len(points),len(points)) for i in range(len(points))]
            obj=make(name+' '+suffix,verts,faces,group,patch)
