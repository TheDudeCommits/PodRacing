...
"""
assert 'BLOCKRUNNER_AUDIT' in globals(), 'Inject the actual successful audit receipt.'
assert 'BLOCKRUNNER_VIEW' in globals() and BLOCKRUNNER_VIEW in {'fullcraft', 'front', 'rear', 'driver'}
assert 'BLOCKRUNNER_RENDER_TOKEN' in globals()
...
try:
    ...
    copied_materials = {}
    for original in sorted(source.objects, key=lambda ob: ob.name):
    ...
    camera_data.ortho_scale = max(height, width / aspect) * 1.22
    ...
    report['camera'] = {'location': list(camera.location), 'target': list(target_center),
                        ...
                        'orthoScale': camera_data.ortho_scale, 'resolution': [1280, 960]}
