...
async function startDriver(speedScale = 1) {
 ...
 await page!.evaluate(({ samples, totalLength, tune, speedScale, performanceMode, waitPickups }) => {
 ...
 }, { samples, totalLength: course.totalLength, tune, speedScale, performanceMode, waitPickups });
}
...
try{
 ...
 await startDriver();
 await page.locator('[data-action="start-race"]:visible').click();receipt.commands.push('Click actual Open Expedition then Start; automated gamepad follows course and authored pickup lanes.');
