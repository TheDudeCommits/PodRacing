...
export class CinematicCamera {
  ...
  snap(subject: CameraSubject): void {
    ...
    this.camera.fov = this.highlightFraming || this.combatFraming
      || (this.mode === 'chase' && subject.wreckChase && !this.comfort.reducedMotion
        && this.hasImpactCombatBack && this.hasCombatBounds(subject))
      ? 60
      : this.mode === 'cockpit'
        ? 84
  ...
  update(dt: number, time: number, subject: CameraSubject): void {
    ...
    const speedT = MathUtils.smoothstep(subject.speed, 70, 220);
    // Keep the fitted authored lease at its solved 60-degree lens through
    // protected recovery. The normal chase snap clears the lease. Replays
    // retain their existing edited-shot lens; generic driving is unchanged.
  ...
  private fitCombatBounds(subject: CameraSubject, direction: Vector3, minimumDistance = 0): void {
    ...
    // Match the authored lease lens; unchanged generic chase fits remain
    // conservative when their normal FOV is wider than 60 degrees.
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      if (impactFrame) {
        // Move the settled source lower by a requested six screen percentage
        // points. Clamp to the original vertical fit interval so portrait or
        // tall geometry can retain less shift without widening or extra queries.
        const phase = !this.comfort.reducedMotion && Number.isFinite(impactFrame.ageSeconds)
          ? MathUtils.smoothstep(impactFrame.ageSeconds!, .13, .65) : 0;
        const currentUp = (upperAim + lowerAim) * .5 - .06 * tangent * distance;
        const lowerUp = upperAim - .70 * tangent * distance;
        const upperUp = lowerAim + .58 * tangent * distance;
        const proposedUp = MathUtils.clamp(currentUp + .12 * phase * tangent * distance, lowerUp, upperUp);
        this.desiredLookAt.addScaledVector(this.boundsRight, (leftAim + rightAim) * .5)
          .addScaledVector(this.boundsUp, proposedUp + .06 * tangent * distance);
      }
