# PodRacing handover — 8 September 2026, round34

> Current frozen technical checkpoint: **round34 foundation material V1**, including Foundry corridor, hoist V3 and solid effects V2. **648 tests / 117 files, typecheck/build, Polwo Time Attack/Canyon Cup/Continue, seven staged sections, Foundry context recovery and three resource cycles PASS.** Fresh world art remains **6.5/10 FAIL**; the separate Blockrunner source cleanup passes its narrow visual gate at **8.5/10**, while hand/control fit is **3/10 FAIL**. Three runtime families, 23 pending. No overhaul commit, push or deployment.

Work in **/Users/amir/Projects/PodRacing**, branch **codex/now-this-is-podracing**, baseline **0f0e8ab209cc5156efaf92b545630bd243b14478**. Remote is TheDudeCommits/PodRacing. The original task folder `/Users/amir/Codex-ThreeJS` is unrelated. Preserve all uncommitted work and prior source/failure receipts.

The user approved recommendations 1–8 and art **B / Inkstorm**, all downloadable catalogue vehicles with drivers, Blender MCP/imagegen, fresh blind criticism after completed work and 40–60fps. **The full request remains incomplete.** Start with [ROUND34](docs/inkstorm-overhaul/ROUND34.md), [CURRENT_VISUALS](docs/inkstorm-overhaul/CURRENT_VISUALS.md), [STATUS](docs/inkstorm-overhaul/STATUS.md) and [current exact performance report](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_FOUNDATION_ROUND34.md).

Current bundle **index-DAcqiho8.js**, **1,617,115 bytes**, SHA256 **3c07cc90bad7f90167eec60765c0f90e62ce88a990e662de6b8651905994433a**. [Frozen inventory](output/gauntlet/round34-foundation-material-v1-frozen-runtime/inventory.json): 474 files / 164,288,407 bytes, SHA256 `24afaafb774da7a98e6f456762cfeee2823ae8fb3e15e6091172f94b35c27f31`. Measured local Apple M4/Chrome tests use adaptive resolution; they are not fixed-DPR or universal-device guarantees. Earlier frozen [round34 V1](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_ROUND34.md), [round33 V5](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_ROUND33.md), [round32](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_ROUND32.md) and [round31](docs/inkstorm-overhaul/FULL_RACE_PERFORMANCE_ROUND31.md) remain authoritative for their own files. Preserve the earlier context failure at exactly ten cadence samples and separate unchanged-gate readiness retry; current foundation checks passed on their first attempts.

Round34 adds ten grounded Foundry landforms through existing render batches, opaque thick hardware and five-chunk rubble, and metre-scaled foundation wear. The latter fixes unit-box paint stretched across 94 m walls. Source geometry, terrain authority, driving/rules, course9 / drive4 / rules2 editions and original layout stay unchanged. Tests include actual source-bound/road clearance, grounding, shared shadow/LOD ownership, pickup recess rays, mine envelope and rubble footprint. All thirteen current PNGs were individually inspected. The [fresh world/source critic](docs/inkstorm-overhaul/BLIND_FOUNDATION_AND_SOURCE_CLEANUP_ROUND34.md) and [solid-effects critic](docs/inkstorm-overhaul/BLIND_SOLID_EFFECTS_V2_ROUND34.md) remain explicit FAILs for full art. Largest gaps: industrial ground contacts, coherent medium-scale materials, repeated process assemblies, coarse impact fragments, and driver/control readability. Original seven world concepts are unchanged; concepts 11–13 supplement construction, Blockrunner paint and effects.

**All 26 vehicle candidates are officially imported and source GLB-preserved**, 748,730,812 bytes, from the retained 29-result search with three exclusions. **Three runtime families: Teemto, Sebulba and Polwo; 23 preparations remain.** No new family was registered in round34. [Catalogue/provenance](docs/inkstorm-overhaul/VEHICLE_CATALOG.md) retains authors, licence restrictions and older 429 failures. Keep the NoDerivs source out of public adaptations and preserve all originals; do not blindly stage the large source archive. Round32 contains the latest three-appearance/six-race matrix; later representative Polwo checks do not repeat that whole matrix.

**Blockrunner source progress:** read [actual cleanup](docs/inkstorm-overhaul/BLOCKRUNNER_CLEANUP_ROUND34.md) and `assets/source/inkstorm/blockrunner-round34/`. Colour source UID `a6f14ae799ab40d7ac425f043f824ff8` retains its original minifigure pilot. Actual Blender audit and four original/four matched neutral renders are complete. Exact opposing-face cleanup removes 4,828 duplicate triangles, 48,384→43,556, in a separate scene; original vertex coordinates, retained normals and source data remain. Fresh narrow cleanup 8.5 PASS does not accept fit/style/runtime. Original floor-rooted full-length control relocation intersects legs; the failed measured candidate is preserved. A bounded controls-and-mount redesign is the next source-only task. No paint, normalization, export, runtime anchors or fourth vehicle yet.

Blender currently has **141 scenes**, including `PodRacing — Blockrunner exact opposing face cleanup round34 V1`. All 140 preexisting scenes were preserved by its creation; all 141 were preserved by each matched render. Restore original scene **Cruise — Going Merry source 4b2cb678**, ViewLayer, active **Sketchfab_model.001**, and its exact 15-object selection after every call. Use isolated copies, clean temporary data and **never save the shared .blend**. Earlier Ben import's unrelated `Icosphere.005` membership observation remains historical and must not be erased. Use actual source receipts, not generated concepts, for geometry and anchor fitting.

All owned capture/performance browsers and ports are closed. **Never adopt or stop unrelated port5211.** No reset/automation/goal was created. Follow up on controls fitting, remaining 23 families, ground/construction/FX critique, then human/controller/audio, longer championship balance and other-device acceptance. Synthesized audio and its 32.43-second capture still lack a critical direct audition. [Prior main documentation](docs/inkstorm-overhaul/evidence/round-34/pre-foundation-documentation/receipt.json) preserves the previous state. The published-product description below is historical and does not describe the current overhaul.
