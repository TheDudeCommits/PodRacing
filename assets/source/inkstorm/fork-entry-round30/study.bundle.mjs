...
//#endregion
//#region src/render/inkstorm/InkstormRockGrounding.ts
/**
...
*/
function groundInkstormButtress(placement, heightAt, fixedCrownY) {
	...
	for (let row = -2; row <= 2; row++) for (let column = -2; column <= 2; column++) {
		...
		minimumGround = Math.min(minimumGround, heightAt(placement.x + cos * x + sin * z, placement.z - sin * x + cos * z));
	}
	const baseY = minimumGround - 1.5 - 18 * placement.sy;
	return {
		baseY,
		scaleY: ((fixedCrownY ?? centerGround - 1.5 + 120 * placement.sy) - baseY) / 120
	};
}
//#endregion
//#region assets/source/inkstorm/fork-entry-round30/study.ts
...
const rows = branch.points.map((p, i) => {
	const { rightX, rightZ } = inkstormRoadCrossSection(branch.points, i, false), main = course.sampleAtProgress(p.canonicalProgress);
	return {
		index: i,
		...p,
		rightX,
		rightZ,
		groundCenter: ground(p.x, p.z),
		edgeGround: [-1, 1].map((s) => ground(p.x + rightX * p.width * s, p.z + rightZ * p.width * s)),
		main: {
			x: main.x,
			z: main.z,
			y: main.y,
		...
		gap: Math.hypot(p.x - main.x, p.z - main.z) - p.width - main.width
	};
});
const dividers = getInkstormForkDividers(course).map((d) => ({
	...d,
	grounding: groundInkstormButtress(d, ground)
}));
writeFileSync(`${folder}/course-contact-study.json`, JSON.stringify({
	seed: course.seed,
	signature: course.signature,
	totalLength: course.totalLength,
	entryProgress: branch.entryProgress,
	exitProgress: branch.exitProgress,
	branchId: branch.id,
	rows,
	dividers
}, null, 2) + "\n");
console.log(JSON.stringify({
	...
	rows: rows.map((r) => ({
		...
		t: r.routeProgress,
		y: r.y,
		ground: r.groundCenter,
		leftGap: r.y - r.edgeGround[0],
		rightGap: r.y - r.edgeGround[1],
		gap: r.gap
	})),
	dividers
}, null, 2));
