...
async function load(path: string, stripImages = false): Promise<Group> {
  ...
  assert(binary.equals(output.subarray(28 + padded)));
...
function fixture(name: typeof variants[number]) {
  ...
  const breakup = TeemtoWreckBreakup.create(root, nodes, original)!; assert(breakup);
  const damageRoot = sources.get(`${name}-damage`)!.clone(true), damage = meshes(damageRoot); root.add(damageRoot);
  assert(breakup.installDamageVariant(damage));
...
export async function prepareHero(){
 sources.set('hero',await load('public/assets/inkstorm/vehicles/teemto-hero-open-v2.glb',true));
 sources.set('hero-damage',await load(privatePath+'teemto-damage-hero-v16.glb'));
 return fixture('hero');
}
export function disposeSources(){for(const root of sources.values())for(const mesh of meshes(root)){
