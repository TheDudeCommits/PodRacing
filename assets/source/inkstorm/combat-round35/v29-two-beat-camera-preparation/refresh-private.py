...
includes=[str((b/'test-runtime/tests/**/*.test.ts').relative_to(r)),str((b/'checks/*.test.ts').relative_to(r)),
 'tests/camera/CinematicCamera.test.ts','tests/camera/GalacticCamera.test.ts','tests/camera/courseJunctionFraming.test.ts',
 'tests/camera/WreckBoundsFraming.test.ts']
(b/'vitest.config.ts').write_text("import{defineConfig}from'vitest/config';import{dirname,resolve}from'node:path';const targets=new Map("+json.dumps(list(paths.items()))+");export default defineConfig({plugins:[{name:'private-v29',enforce:'pre',resolveId(s,i){const p=s.startsWith('.')&&i?resolve(dirname(i),s):s;return targets.get(p)??targets.get(p+'.ts')??null;}}],test:{maxWorkers:1,fileParallelism:false,include:"+json.dumps(includes)+"}});\n")
