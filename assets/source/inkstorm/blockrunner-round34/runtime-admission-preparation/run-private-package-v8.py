#!/usr/bin/env python3
"""Run a fresh V8 contact-color private package/validator sequence against immutable V6.
...
try:
    ...
    run('05-color-only',[sys.executable,str(HERE/'verify-color-only-package-v8.py'),'--package-directory',str(out),'--output',str(out/'color-only-vs-v6-validation.json')])
    ...
    assert hashlib.sha256(a.master.read_bytes()).hexdigest()==a.expected_master_sha256
    print(json.dumps({'status':'private V8 package/validator/color-only sequence PASS; no visual/runtime acceptance','output':str(out),'steps':steps},indent=2))
finally:
