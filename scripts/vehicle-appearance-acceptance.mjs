...
async function checkSebulba() {
  ...
  try {
    ...
    assert(cancelled.selected === 'teemto' && cancelled.racers[0].active === 'teemto'
      && cancelled.racers[0].registeredPrepasses === 10, 'A late Sebulba completion replaced the newer Teemto choice');
    ...
    assert(receipt.requests.every(request => /\/(teemto-(hero-open-v2|rival)|sebulba-hero)\.glb$/.test(request.url)), 'An unused catalogue or Sebulba rival variant was loaded');
...
try {
  ...
  // This is an authored revision fixture, independently pinned to the packaged
  // source receipt, not inferred from whatever the renderer happens to load.
  receipt.teemtoAsset = { url: `${origin}/assets/inkstorm/vehicles/teemto-hero-open-v2.glb`,
    expectedSha256: 'af5d69514a51f7e9c089a1710e1587f46e053412b488268dc433f029576c0a7e',
    triangles: 57618, bodyTriangles: 41839, bodyDraws: 4, pilotDraws: 6 };
  receipt.teemtoAsset.servedSha256 = sha256(new Uint8Array(await (await fetch(receipt.teemtoAsset.url)).arrayBuffer()));
  assert(receipt.teemtoAsset.servedSha256 === receipt.teemtoAsset.expectedSha256, 'Served Teemto GLB differs from the reviewed V2 package');
  ...
  assert(initial.racers[0].statistics.triangles === 57618, 'Hero did not load the verified 57,618-triangle open-cockpit V2 export');
  assert(initial.racers[0].statistics.bodyDraws === 4 && initial.racers[0].statistics.pilotDraws === 6, 'Actual body/driver classification is wrong');
  assert(initial.racers[0].embeddedPilot, 'Imported driver did not suppress the procedural pilot');
  assert(initial.racers[0].registeredPrepasses === 10, 'Imported geometry did not replace the procedural prepass registration');
  ...
  assert((await artState(page)).racers[0].registeredPrepasses === 10, 'Returning to Teemto accumulated or omitted prepasses');
  ...
  assert(live.renderer.racerShadow.drawCalls === 4 && live.renderer.racerShadow.drawnTriangles === 41839,
    `Shadow did not use four actual V2 body meshes: ${JSON.stringify(live.renderer.racerShadow)}`);
  ...
  await failedPage.route('**/assets/inkstorm/vehicles/teemto-hero-open-v2.glb', failAsset);
  ...
  await failedPage.unroute('**/assets/inkstorm/vehicles/teemto-hero-open-v2.glb', failAsset);
  stage = 'retry';
  ...
  assert(receipt.requests.every((request) => /\/teemto-(hero-open-v2|rival)\.glb$/.test(request.url)), 'Unselected catalogue models were loaded');
