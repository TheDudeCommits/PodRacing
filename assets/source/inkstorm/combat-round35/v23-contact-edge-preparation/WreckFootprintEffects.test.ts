...
import { GalacticEffectsView } from '/Users/amir/Projects/PodRacing/assets/source/inkstorm/combat-round35/v23-contact-edge-preparation/test-runtime/GalacticEffectsView.ts';
  ...
  try{
   ...
   for(const time of[4.05,4.15]){
    ...
    for(let i=0;i<2;i++){
     ...
     const age=time-4-i*.04;
     expect(p.x).toBeCloseTo(i===0?-2-age*3.2:2.3+age*4.8,5);expect(p.z).toBeCloseTo(i===0?1:2,5);
     const bottom=new Vector3(0,-1,0).applyMatrix4(m);expect(bottom.y).toBeCloseTo((i===0?2.6:3.4)+.08+s.y*.05,5);
     expect(Math.abs(new Vector3(1,0,0).applyQuaternion(q).z)).toBeCloseTo(1,6);
     expect(view.explosionPlates.geometry.getAttribute('aEffectSurface').getX(i)).toBe(5);
    }
    positions.push(xs);
