...
const beamFragment = /* glsl */ `
  ...
  void main() {
    ...
    float core = step(abs(vUv.y - 0.5), 0.10);
    float alpha = edge * (0.38 + core * 0.6 + movingBand * 0.3) * (0.68 + uPower * 0.4);
    vec3 beamColor = mix(uColor, vec3(1.0, 0.98, 0.82), core * 0.86);
    gl_FragColor = vec4(beamColor * (1.0 + core * 0.82), alpha);
  }
`;
...
const exhaustFragment = /* glsl */ `
  ...
  void main() {
    ...
    float alpha = lengthMask * silhouette
      * (0.7 + ignitionPlate * 0.2 + drivePlate * 0.08);
...
function createEngine(materials: PodracerMaterials, side: number): {
  ...
  // Long, segmented engine silhouettes: deliberately chunky at chase distance.
  addMesh(root, new CylinderGeometry(1.48, 1.75, 9.4, 10, 1, false), materials.shell,
    [0, 0, 0], [Math.PI / 2, 0, 0], [1, 1, 1], 'engine-shell');
  addMesh(root, new CylinderGeometry(1.88, 1.54, 2.4, 10), materials.secondary,
  ...
  addMesh(root, new ConeGeometry(1.48, 3.4, 10, 1, false), materials.metal,
    [0, 0, -6.25], [Math.PI / 2, 0, 0], [1, 1, 1], 'nozzle');
  ...
  for (const z of [-3.75, -0.5, 2.7, 4.2]) {
    addMesh(root, new TorusGeometry(1.7, 0.18, 5, 10), z === 2.7 ? materials.secondary : materials.ink,
      [0, 0, z]);
  }
  ...
  addMesh(root, new TorusGeometry(1.08, 0.14, 5, 10), materials.ink,
    [0, 0, -6.25]);
  addMesh(root, new CylinderGeometry(1.06, 1.22, 0.48, 10), materials.ink,
    [0, 0, -8.02], [Math.PI / 2, 0, 0], [1, 1, 1], 'nozzle-mouth');
  ...
  addMesh(root, new BoxGeometry(0.18, 2.6, 2.7), materials.secondary,
    [side * -1.54, 1.9, -1.35], [0.12, 0, side * 0.18], [1, 1, 1], 'engine-fin');
  ...
  const exhaust = addMesh(root, new ConeGeometry(1.3, 8.5, 9, 4, true), exhaustMaterial,
    [0, 0, -10.2], [-Math.PI / 2, 0, 0], [1, 1, 1], 'engine-exhaust') as Mesh<BufferGeometry, ShaderMaterial>;
...
export class PodracerView extends Group {
  ...
  constructor(
    ...
    addMesh(cockpit, new SphereGeometry(1, 12, 6), palette.shell,
      [0, 0, 0], [0, 0, 0], [2.25, 0.92, 3.4], 'cockpit-shell');
    addMesh(cockpit, new SphereGeometry(1, 10, 5), palette.canopy,
      [0, 1.0, 0.15], [-0.12, 0, 0], [1.34, 0.72, 1.75], 'canopy');
    addMesh(cockpit, new ConeGeometry(1.18, 3.5, 8), palette.secondary,
      [0, -0.1, 3.7], [Math.PI / 2, 0, 0], [1, 1, 1], 'cockpit-nose');
    addMesh(cockpit, new BoxGeometry(4.4, 0.22, 1.3), palette.ink,
      [0, -0.58, -1.4], [0, 0, 0], [1, 1, 1]);
    ...
    for (const vehicleClass of RACER_VEHICLE_CLASSES) {
      ...
      for (const y of presentation.couplingY) {
        ...
        const beam = new Mesh(
          new TubeGeometry(curve, 36, y < 1 ? 0.26 : 0.16, 6, false),
  ...
  update(pose: PodracerPose, time: number): void {
    ...
    for (const exhaust of [this.leftEngine.exhaust, this.rightEngine.exhaust]) {
      ...
      exhaust.material.uniforms.uHot!.value.set(redline > 0 ? '#ff321d' : '#ff9a43');
      exhaust.scale.x = 1 + redlinePulse * 0.42;
      exhaust.scale.z = 1 + redlinePulse * 0.42;
      exhaust.scale.y = 0.72 + power * 0.72 + Math.sin(time * 41 + this.racerIndex) * 0.08;
    }
