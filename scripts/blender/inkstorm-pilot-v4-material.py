...
ACTION = 'setup'
SOURCE = 'PodRacing — Teemto pilot construction v4'
STAGE = 'PodRacing — Teemto pilot material v4'
EXPORT_SCENE = 'PodRacing — Teemto pilot material v4 normalized'
BASE = '/Users/amir/Projects/PodRacing/assets/source/inkstorm/vehicles/4eff45899ada40bb920c5c744663db90/processed/pilot-v4/'
REVIEW = BASE+'review/'
ATLAS_NAME = 'Inkstorm original pilot color atlas v4 2048'
NORMAL_NAME = 'Inkstorm pilot v4 normal atlas 1024'
ROUGHNESS_NAME = 'Inkstorm pilot v4 roughness atlas 1024'
...
def make_bake_material(role,atlas):
    material=bpy.data.materials.new('Pilot v4 source '+role)
    ...
    variation=ramp(material,broad,[(.16,(.68,.68,.68)),(.50,(1,1,1)),(.82,(1.25,1.25,1.25))])
    ...
    if role in ('suit','webbing','accent'):
        ...
        for direction in ('X','Z'):
            ...
            wave.inputs['Scale'].default_value=310 if role!='webbing' else 440
        ...
        color=mix(material,color,(.14,.095,.046),math_node(material,'MULTIPLY',wear,.73))
    elif role=='shell':
    ...
    bump.inputs['Strength'].default_value={'suit':.38,'webbing':.46,'accent':.35,'rubber':.24,'shell':.13,'hardware':.10}[role]
...
def pilot_meshes(scene):
    return [o for o in scene.objects if o.type=='MESH' and o.name.startswith('pilot-material-v4-')]
...
try:
    if ACTION=='setup':
        ...
        for original,matrix in originals:
            ...
            is_pilot=original.type=='MESH' and original.name.startswith('teemto-pilot-v4-')
            key=original.name.split('teemto-pilot-v4-')[-1].split('.')[0] if is_pilot else None
            name='pilot-material-v4-'+key if is_pilot else 'pilot-material-study-copy-'+original.name
        ...
        atlas.filepath_raw=BASE+'pilot-color-atlas-v4.png'
        ...
        normal.filepath_raw=BASE+'pilot-normal-atlas-v4.png';normal.file_format='PNG'
        ...
        roughness.filepath_raw=BASE+'pilot-roughness-atlas-v4.png';roughness.file_format='PNG'
        ...
        if ACTION=='bake':
            ...
            roughness.save()
            for obj in pilots:
                key=obj.name.split('pilot-material-v4-')[-1].split('.')[0]
                material=bpy.data.materials.new('Pilot atlas v4 runtime '+key)
                material.use_nodes=True
                bsdf=material.node_tree.nodes.get('Principled BSDF')
            ...
            for obj in destination.objects:
                obj.select_set(True)
            path=BASE+'teemto-pilot-material-v4-normalized.glb'
