...
export function createPitPadField(placements: readonly PitPlacement[], route: readonly RoutePoint[],
  ...
  for (let row = 1; row < rows - 1; row++) for (let column = 1; column < columns - 1; column++) {
    ...
    let weight = 0, targetSum = 0, targetWeight = 0;
    for (const { pad, c, s } of frames) {
      ...
      if (w > 0) { weight = Math.max(weight, w); targetSum += w ** 4 * pad.targetHeight; targetWeight += w ** 4; }
    }
    if (weight === 0) continue;
    const roadRoom = Math.max(0, roadClearance(nearby(x, z), x, z) - radius);
    // The floor now follows the roadside grade, so keep its mesh-safe apron
    // near full influence until the unchanged protected band. Remaining
    // edge slopes are measured; the previous high-anchor two-metre bank
    // rejection is not treated as acceptance of this revised composition.
    const roadWeight = smooth(roadRoom / PROTECTION_FADE);
