...
import { BoxGeometry, BufferAttribute, BufferGeometry, Color, InstancedBufferAttribute, InstancedMesh, Matrix4, Quaternion, ShaderMaterial, Vector2, Vector3 } from "three";
...
//#endregion
//#region src/render/inkstorm/InkstormGeologyShader.ts
/** Shared world-space stone, lighting and atmosphere for terrain and rock meshes. */
const INKSTORM_GEOLOGY_GLSL = `
...
`;
//#endregion
//#region src/render/inkstorm/InkstormRacerShadow.ts
function createInkstormRacerShadowUniforms() {
	return {
		uRacerShadow: { value: null },
...
const INKSTORM_RACER_SHADOW_GLSL = `
...
//#endregion
//#region src/render/inkstorm/InkstormSunShadow.ts
function createInkstormShadowUniforms() {
	return {
		uWorldShadow: { value: null },
		uWorldShadowMatrix: { value: new Matrix4() },
		uWorldShadowReady: { value: 0 },
		uWorldShadowTexel: { value: new Vector2(1 / 4096, 1 / 4096) },
		uWorldShadowBias: { value: 4e-4 }
	};
}
/** RGBA depth from the actual static scenery, independent of the racing camera. */
const INKSTORM_SHADOW_GLSL = `
...
//#endregion
//#region src/render/inkstorm/InkstormSurfaceMaterial.ts
const rockPaintUniforms = {
	uRockPaint: { value: null },
	uRockBeds: { value: null },
	uRockPaintReady: { value: 0 }
};
const machineryPaintUniforms = {
	uMachineryPaint: { value: null },
	uMachineryPaintReady: { value: 0 }
};
/** Vertex-painted Blender surfaces with broad violet shade and eroded strata. */
var InkstormSurfaceMaterial = class extends ShaderMaterial {
	constructor(stone = true, workshopFamily = null, workshopBake = null) {
		super({
			name: stone ? "Inkstorm painted sandstone" : "Inkstorm worn machinery",
			vertexColors: true,
			toneMapped: false,
			defines: !stone && workshopFamily ? {
				INKSTORM_WORKSHOP_FAMILY: workshopFamily === "pit-complex" ? 1 : 2,
				...workshopBake ? { INKSTORM_WORKSHOP_BAKE: 1 } : {}
			} : {},
			uniforms: {
				uWorkshopBake: { value: workshopBake?.texture ?? null },
				uWorkshopDecodeRange: { value: workshopBake?.decodeRange ?? 4 },
				uWorkshopIntensity: { value: workshopBake?.intensity ?? 1 },
				...createInkstormShadowUniforms(),
				...createInkstormRacerShadowUniforms(),
				...machineryPaintUniforms,
				uPaint: rockPaintUniforms.uRockPaint,
				uRockBeds: rockPaintUniforms.uRockBeds,
				uPaintReady: rockPaintUniforms.uRockPaintReady,
				uStone: { value: stone ? 1 : 0 },
				uSun: { value: new Vector3(-.42, .76, -.5).normalize() },
				uHaze: { value: new Color("#b79cb8") }
			},
			vertexShader: `
        #include <common>
...
uniform float uWorkshopIntensity;
#else
varying vec3 vWorkshopScale;
...
float inkstormWorkshopBakedLens() {
  ...
  return inkstormWorkshopLens(center);
        ...
        void main(){
          ...
          color+=base*texture2D(uWorkshopBake,vWorkshopUv).rgb*uWorkshopDecodeRange*uWorkshopIntensity*(1.-service);
...
//#endregion
//#region src/render/inkstorm/InkstormBridge.ts
/** Joined slab and low trim share the road's rows; branch points remain physics authority. */
function createInkstormBridge$1(branch, heightAt) {
	const vertices = [], colors = [], pieces = [];
	const steel = new Color("#303349"), trim = new Color("#d39865");
	const rows = branch.points.map((point, index) => ({
		point,
	...
	const at = (index, lateral, height = 0) => {
		...
		return [
			row.point.x + row.rightX * lateral,
			row.point.y + height,
	...
	const strip = (left, right, bottom, top, color, across = 1) => {
		for (let i = 0; i < rows.length - 1; i++) {
			...
			quad(at(i, right(i), bottom), at(i + 1, right(i + 1), bottom), at(i + 1, left(i + 1), bottom), at(i, left(i), bottom), color);
			quad(at(i, left(i), bottom), at(i + 1, left(i + 1), bottom), at(i + 1, left(i + 1), top), at(i, left(i), top), color);
			quad(at(i, right(i), top), at(i + 1, right(i + 1), top), at(i + 1, right(i + 1), bottom), at(i, right(i), bottom), color);
		}
		if (rows.length < 2) return;
		const last = rows.length - 1;
		quad(at(0, left(0), bottom), at(0, left(0), top), at(0, right(0), top), at(0, right(0), bottom), color);
		quad(at(last, right(last), bottom), at(last, right(last), top), at(last, left(last), top), at(last, left(last), bottom), color);
	};
	const width = (i) => rows[i].point.width;
	strip((i) => -width(i), width, -1.5, 0, steel, 16);
	for (const side of [-1, 1]) strip((i) => width(i) * side - .35, (i) => width(i) * side + .35, 0, .32, trim);
	...
	for (let i = 1; i < branch.points.length - 1; i += 3) {
		...
		matrix.compose(new Vector3(x, base + height / 2, z), rotation.setFromAxisAngle(up, Math.atan2(b.x - a.x, b.z - a.z)), new Vector3(6, height, 7));
		const box = new BoxGeometry(1, 1, 1);
		const pier = box.toNonIndexed().applyMatrix4(matrix);
		box.dispose();
	...
	const mesh = new InstancedMesh(geometry, new InkstormSurfaceMaterial(false), 1);
...
//#endregion
//#region assets/source/inkstorm/fork-entry-round30/InkstormBridge.candidate.ts
/** Joined slab and low trim share the road's rows; branch points remain physics authority. */
function createInkstormBridge(branch, heightAt, course) {
	const vertices = [], colors = [], pieces = [];
	const steel = new Color("#303349"), trim = new Color("#d39865");
	const rows = branch.points.map((point, index) => ({
		point,
	...
	const material = foundation ? [new InkstormSurfaceMaterial(false), new InkstormSurfaceMaterial(true)] : new InkstormSurfaceMaterial(false);
...
//#endregion
//#region assets/source/inkstorm/fork-entry-round30/hook-audit.ts
let field = null;
...
const old = createInkstormBridge$1(branch, ground);
const start = performance.now();
const next = createInkstormBridge(branch, ground, course);
const elapsedMs = performance.now() - start;
const attributes = Object.entries(old.geometry.attributes).map(([name, a]) => {
	const b = next.geometry.getAttribute(name);
	return {
		name,
		originalComponents: a.array.length,
		candidateComponents: b.array.length,
		byteExactPrefix: Array.from(a.array).every((v, i) => v === b.array[i])
	};
});
const fallback = createInkstormBridge(branch, ground);
const fallbackExact = Object.entries(old.geometry.attributes).every(([name, a]) => {
	const b = fallback.geometry.getAttribute(name);
	return a.array.length === b.array.length && Array.from(a.array).every((v, i) => v === b.array[i]);
});
const materialCount = Array.isArray(next.material) ? next.material.length : 1;
const receipt = {
	status: attributes.every((a) => a.byteExactPrefix) && fallbackExact && materialCount === 2 && next.geometry.groups.length === 2 ? "PASS" : "FAIL",
	attributes,
	fallbackExact,
	materialCount,
	groups: next.geometry.groups,
	originalTriangles: old.geometry.getAttribute("position").count / 3,
	candidateTriangles: next.geometry.getAttribute("position").count / 3,
	constructionMilliseconds: elapsedMs,
	limits: "CPU source candidate instantiation and buffer preservation; no GPU compile or actual racing-view acceptance."
};
