...
try {
  ...
  // Retain the native endpoints so a reviewer can recompute the recovery
  // distances and final step gap without trusting the derived assertion.
  const recoveryEndpoints={guestId,beforeDriveHost:beforeDrive,afterDriveHost:end[0],afterDriveGuest:end[1]};
  await writeFile(`${out}/recovery-endpoints.json`,JSON.stringify(recoveryEndpoints,null,2));
  receipt.recoveryEndpoints={path:'recovery-endpoints.json',sha256:createHash('sha256').update(JSON.stringify(recoveryEndpoints,null,2)).digest('hex')};
