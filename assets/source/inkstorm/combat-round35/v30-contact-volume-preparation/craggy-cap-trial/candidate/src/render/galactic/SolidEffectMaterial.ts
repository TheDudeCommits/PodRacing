...
export function createContactDustMaterial(): MeshBasicMaterial {
  ...
  result.onBeforeCompile = shader => {
    ...
    `).replace('#include <color_fragment>', `
      ...
      vec3 p = vContactLocal * vec3(4.5, 6., 4.5) - vec3(0., vContactAge * 1.3, 0.);
      float billow = contactNoise(p) * .74 + contactNoise(p * 2.3 + 9.) * .26;
      ...
      n = normalize(n - .16 * gradient / max(abs(det), .000001));
      ...
      diffuseColor.rgb = mix(trough, lit, light) * (.64 + .60 * billow);
    `);
  };
